...
use p3_field::{
    extension::BinomialExtensionField, AbstractExtensionField, AbstractField, Field, PrimeField64,
    TwoAdicField,
};
...
use crate::config::{Challenge, Domain, OpeningError, StarkGenericConfig, Val};
use crate::{
...
impl<SC: StarkGenericConfig, A: MachineAir<Val<SC>>> Verifier<SC, A> {
        ...
        A: for<'a> Air<VerifierConstraintFolder<'a, SC>>,
    {
        ...
        for (opening, chip) in opened_values.chips.iter().zip_eq(chips.iter()) {
            ...
            challenger.observe_slice(<BinomialExtensionField<<SC as StarkGenericConfig>::Val, 4> as AbstractExtensionField<Val<SC>>>::as_base_slice(&local_sum));
        ...
        let expected_local_sum = compute_expected_state_imbalance::<SC>(
            public_values,
            &permutation_challenges,
