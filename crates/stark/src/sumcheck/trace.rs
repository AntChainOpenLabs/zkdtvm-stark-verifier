...
///   - `padding_row` and main are both extension field `Challenge<SC>`
pub(crate) enum ChipTrace<'a, SC: SCStarkGenericConfig> {
...
impl<SC: SCStarkGenericConfig> ChipTrace<'_, SC> {
    ...
    pub(crate) fn get_sum_perm_rows_linear(&self, point: usize) -> Challenge<SC> {
        match point {
            0 | 1 => match self {
    ...
    pub(crate) fn get_summation_input_base(
        &self,
        point: usize,
        var_degree: usize,
    ...
    pub(crate) fn get_summation_input_ext(
        &self,
        point: usize,
        var_degree: usize,
        bit_expand_poly: Option<&BitExpandPoly<Val<SC>>>,
    ...
    pub(crate) fn get_summation_input_hybrid(
        &self,
        point: usize,
        var_degree: usize,
        bit_expand_poly: Option<&BitExpandPoly<Val<SC>>>,
    ...
    pub(crate) fn update(&mut self, challenge: Challenge<SC>) {
        match self {
            ChipTrace::FirstRound(mat) => {
    ...
    /// occur.
    pub(crate) fn get_row_for_opening(&self, idx: usize) -> Vec<Challenge<SC>>
    where
