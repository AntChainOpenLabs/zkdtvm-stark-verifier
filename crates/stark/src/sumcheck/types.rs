...
impl<F: Field, EF: ExtensionField<F>> EqPoly<F, EF> {
    ...
    #[allow(dead_code)]
    pub(crate) fn new(eq_challenges: Vec<EF>, num_rounds_linear: usize, degree: usize) -> Self {
    ...
    #[allow(dead_code)]
    pub(crate) fn update(&mut self, challenge: EF) {
    ...
    /// order used in sumcheck v2.
    ///
    /// In the original (big-endian) version, the Kronecker product is `cur ⊗ prev`, placing the
    /// newest variable in the high-order (outer) position. In the v2 (small-endian) version, we
    ...
    pub fn compute_weights(degree: usize) -> Vec<F> {
        (0..=degree)
            .map(|i| {
                (0..=degree)
                    .filter(|j| *j != i)
