...
use crate::{
    ...
    InteractionKind, MachineChip, OpeningError, StarkGenericConfig, Val,
};
...
impl<SC: SCStarkGenericConfig, A: MachineAir<Val<SC>>> Verifier<SC, A> {
        ...
        A: for<'a> Air<SumcheckVerifierConstraintFolder<'a, SC>>,
    {
        ...
        // println!("CHIYUE, finish verify sumcheck unipolys");
        ...
        // Only core proofs (contains_global_bus) have dangling State/MemoryGlobalAddr/
        // GlobalDigestChain interactions. Recursion proofs have all interactions balanced.
        let expected_local_sum = if contains_global_bus {
            compute_expected_state_imbalance::<SC>(
                public_values,
                &permutation_challenges,
