...
use p3_field::{
    ...
    packed_ext::PackedExtension,
    AbstractField, Field, PrimeField32, TwoAdicField,
};
use serde::{de::DeserializeOwned, Serialize};

pub type Domain<SC> = <<SC as StarkGenericConfig>::Pcs as Pcs<
    BinomialExtensionField<Val<SC>, 4>,
...
pub type PackedExt<SC> = PackedExtension<<Val<SC> as Field>::Packing, 4>;

// pub type PackedChallenge<SC> =
//     <<SC as StarkGenericConfig>::Challenge as ExtensionField<Val<SC>>>::ExtensionPacking;

pub type Challenge<SC> = BinomialExtensionField<Val<SC>, 4>;

pub type PackedChallenge<SC> =
    BinomialExtensionField<<<SC as StarkGenericConfig>::Val as Field>::Packing, 4>;
...
pub trait StarkGenericConfig: 'static + Send + Sync + Serialize + DeserializeOwned + Clone {
    ...
    type Pcs: Pcs<BinomialExtensionField<Self::Val, 4>, Self::Challenger, Domain = Self::Domain>
        + Sync
        + ZeroCommitment<Self>;
    ...
    type Challenger: FieldChallenger<Val<Self>>
        + CanObserve<
            <Self::Pcs as Pcs<BinomialExtensionField<Self::Val, 4>, Self::Challenger>>::Commitment,
        > + CanSample<BinomialExtensionField<Self::Val, 4>>
        + Serialize
        + DeserializeOwned;
