...
pub enum InteractionKind {
    ...
    /// Interaction for chaining the septic digest accumulation in `GlobalChip`
    /// without next-row access.
    GlobalDigestChain = 15,
}
...
impl InteractionKind {
    ...
    pub fn all_kinds() -> Vec<InteractionKind> {
        vec![
            ...
            InteractionKind::GlobalDigestChain,
        ]
    }
}
...
impl Display for InteractionKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            InteractionKind::GlobalDigestChain => write!(f, "GlobalDigestChain"),
        }
    }
}
