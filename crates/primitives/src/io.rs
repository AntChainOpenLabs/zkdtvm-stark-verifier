...
pub struct DTPublicValues {
    buffer: Buffer,
}

impl DTPublicValues {
    /// Create a new `DTPublicValues`.
    ...
    /// Create a `DTPublicValues` from a slice of bytes.
...
impl AsRef<[u8]> for DTPublicValues {
    fn as_ref(&self) -> &[u8] {
        &self.buffer.data
    }
}
...
mod tests {
    ...
    fn test_hash_public_values() {
        ...
        let mut public_values = DTPublicValues::new();
