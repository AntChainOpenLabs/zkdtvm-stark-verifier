...
pub fn runtime_sc_poseidon2_init(
    ...
    let mut external_round_constants: Vec<[KoalaBear; 24]> = KB24_RC_EXTERNAL_INITIAL
        .iter()
        .map(|row| row.map(KoalaBear::from_canonical_u32))
        .collect();
    external_round_constants.extend(
        KB24_RC_EXTERNAL_FINAL
            .iter()
            .map(|row| row.map(KoalaBear::from_canonical_u32)),
    );
    let internal_round_constants: Vec<KoalaBear> = KB24_RC_INTERNAL
        .iter()
