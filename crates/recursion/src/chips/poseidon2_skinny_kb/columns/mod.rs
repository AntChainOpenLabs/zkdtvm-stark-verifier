...
///       - `round_witness`  : first `WIDTH` (16) cells carry the state after the first
///                            of the two external rounds; remaining 3 cells unused (zero).
///       - `state_out`      : output state after 2 external rounds.
///
///   * **Internal-rounds row** (row 2):
///       - `state_in`       : input state
///       - `round_witness`  : `round_witness[k]` for k=0..18 holds the post-S-box value
///                            of `state[0]` produced by internal round `k`. Round 19 (the
///                            last) is computed inline without a witness.
