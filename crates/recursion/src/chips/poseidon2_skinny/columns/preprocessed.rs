...
///   - `is_first_round`    : 1 only on the very first row of a permutation; selects an
///                            extra `external_linear_layer` on the input state.
///   - `is_real`           : 1 on real rows, 0 on padding rows. AIR-side selector for
///                            transition constraints.
///   - `state_in_neg_mult` : -1 on real rows, 0 on padding rows. Used as the `send_single`
///                            multiplicity for `state_in` (a `send` with negative mult is
///                            equivalent to a `receive` with positive mult; this matches
///                            the wide chip's "send-only" lookup convention so padding
///                            rows never inject any net interaction into the memory bus).
