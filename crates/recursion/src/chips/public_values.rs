...
pub const PUB_VALUES_TARGET_LOG_HEIGHT: usize = 4;
pub const PUB_VALUES_LOG_HEIGHT: usize =
    align_log_height_for_skip_rounds(PUB_VALUES_TARGET_LOG_HEIGHT);

const fn align_log_height_for_skip_rounds(log_height: usize) -> usize {
    if log_height < CHIP_LOG_HEIGHT_THRESHOLD {
        log_height.div_ceil(NUM_SKIP_ROUNDS) * NUM_SKIP_ROUNDS
    } else {
        log_height
    }
}
...
mod tests {
    ...
    use crate::utils::{pad_rows_fixed, setup_logger};
