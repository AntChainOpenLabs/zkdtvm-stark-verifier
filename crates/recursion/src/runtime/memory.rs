...
impl<F: PrimeField64> MemVec<F> {
    ...
    /// Returns a raw pointer to the memory slot at the given index, for prefetching.
    #[inline]
    pub fn slot_ptr(&self, idx: usize) -> *const u8 {
        unsafe { self.0.as_ptr().add(idx) as *const u8 }
    }

    /// Read memory at a pre-computed usize index. Avoids Montgomery reduction.
    #[inline]
    pub unsafe fn mr_at(&self, idx: usize) -> &MemoryEntry<F> {
        let entry = unsafe { &*self.0.get_unchecked(idx).0.get() };
        unsafe { entry.assume_init_ref() }
    }

    /// Write memory at a pre-computed usize index. Avoids Montgomery reduction.
    #[inline]
    pub unsafe fn mw_at(&self, idx: usize, val: Block<F>) {
        let entry = unsafe { &mut *self.0.get_unchecked(idx).0.get() };
        drop(entry.write(MemoryEntry { val }));
    }
}
