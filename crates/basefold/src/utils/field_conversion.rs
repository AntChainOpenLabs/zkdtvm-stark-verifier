...
pub fn into_ef_unpacked<F: Field, EF: ExtensionField<F>>(
    x: EF::ExtensionPacking,
) -> Vec<EF> {
...
pub fn ef_vector_add_with_scale<F: Field, EF: ExtensionField<F>>(
    x: &mut [EF],
    y: &[EF],
    s: EF,
) {
    ...
    let len = x.len();
    let packed_s = EF::ExtensionPacking::from_base_fn(|i| {
        F::Packing::from(s.as_base_slice()[i])
    });
        ...
        .for_each(|(index, (chunk_x, chunk_y))| {
            ...
            for i in 0..F::Packing::WIDTH {
                if i + index * F::Packing::WIDTH < len {
                    chunk_x[i] = EF::from_base_fn(|j| {
                        packed_x.as_base_slice()[j].as_slice()[i]
                    });
                }
            }
        });
...
mod tests {
    ...
    fn flatten_to_base_test() {
        ...
        let vec_f_packed: Vec<<F as Field>::Packing> =
            unsafe { flatten_to_base(vec_ef_packed) };
