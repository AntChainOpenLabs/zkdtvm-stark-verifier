...
use p3_field::{Field, AbstractField, AbstractExtensionField, PackedValue};
use std::fmt::Debug;
use rand::Rng;
...
impl<F: Field> MultilinearPolynomial<F> {
    ...
    pub fn random_linear_combine_single_matrix_columns<EF: ExtensionField<F>>(polys: &RowMajorMatrix<F>, coeff: &Vec<EF>, result: &mut Vec<EF>) {
        let height = polys.height();
        if polys.width == 0 || polys.height() == 0 { return; }
                ...
                .for_each(|(idx, item)| {
                    *item += if is_from_ef {
                        polys.row_slice(idx)
                            .chunks(EF::D)
                            .zip_eq(coeff.iter())
                            .map(|(poly_val, coeff_val)| {
                                EF::from_base_slice(poly_val) * *coeff_val
                            })
                            .fold(EF::zero(), |acc: EF, val: EF| acc + val)
                    } else {
                        (0..polys.width())
                            .map(|poly_idx| {
                                coeff[poly_idx] * polys.values[idx * width + poly_idx]
                            })
                            .fold(EF::zero(), |acc: EF, val: EF| acc + val)
                    };
                });
        ...
        result.par_chunks_mut(F::Packing::WIDTH)
            .enumerate()
            .for_each(|(i_start, chunk)| {
                let i_start = i_start * F::Packing::WIDTH;
                let packed_vals = polys.vertically_packed_row::<F::Packing>(i_start).collect_vec();
                let mut packed_result = EF::ExtensionPacking::zero();
                if is_from_ef {
                    packed_vals.chunks(EF::D)
                        .zip_eq(coeff.iter())
                        .for_each(|(poly_vals, coeff_val)| {
                ...
                for i in 0..F::Packing::WIDTH {
                    if i_start + i < height {
                        chunk[i] += EF::from_base_fn(|j| packed_result.as_base_slice()[j].as_slice()[i]);
                    }
                }
            });
    }
    ...
    pub fn random_linear_combine_columns_compressed<EF: ExtensionField<F>>(
        ...
        for (mat_idx, mat) in matrices.iter().enumerate() {
            Self::random_linear_combine_single_compressed_matrix_columns(mat, &coefficients[mat_idx], result);
        }
    }
    ...
    pub fn random_linear_combine_single_compressed_matrix_columns<EF: ExtensionField<F>>(
        ...
        if width == 0 || total_height == 0 { return; }
        ...
        if stored_height > 0 {
            ...
            result[..stored_height].par_iter_mut()
                .enumerate()
                .for_each(|(idx, item)| {
                    *item += if is_from_ef {
                        ...
                        (0..width)
                            .map(|poly_idx| {
                                coeff[poly_idx] * main_values[idx * width + poly_idx]
                            })
                            .fold(EF::zero(), |acc: EF, val: EF| acc + val)
                    };
                });
        ...
        if stored_height < total_height {
            let padding_contribution: EF = match &compressed.padding_row {
                ...
                PaddingRow::Constant { value, .. } => {
                    if is_from_ef {
                        ...
                        coeff.iter().map(|c| *c * *value).fold(EF::zero(), |acc, val| acc + val)
                    }
                }
            ...
            result[stored_height..total_height].par_iter_mut().for_each(|val| {
...
impl<F: Field> AddAssign<&MultilinearPolynomial<F>> for MultilinearPolynomial<F> {
    fn add_assign(&mut self, rhs: &MultilinearPolynomial<F>) {
        ...
        let summed_evaluations: Vec<F> = self.evals.iter().zip(&rhs.evals).map(|(a, b)| *a + *b).collect();
...
impl<'a, F: Field> AddAssign<(&'a F, &'a MultilinearPolynomial<F>)>
...
impl<'lhs, 'rhs, F: Field> Add<&'rhs MultilinearPolynomial<F>> for &'lhs MultilinearPolynomial<F> {
    ...
    fn add(self, rhs: &'rhs MultilinearPolynomial<F>) -> Self::Output {
        if rhs.is_zero() {
            return self.clone();
        }
        if self.is_zero() {
        ...
        assert_eq!(self.num_vars, rhs.num_vars);
        let result: Vec<F> = self.evals.iter()
            .zip(rhs.evals.iter())
            .map(|(a, b)| *a + *b)
            .collect();
...
impl<'lhs, 'rhs, F: Field> Sub<&'rhs MultilinearPolynomial<F>> for &'lhs MultilinearPolynomial<F> {
    ...
    fn sub(self, rhs: &'rhs MultilinearPolynomial<F>) -> Self::Output {
        self + &rhs.clone().neg()
    }
}
...
pub trait MultilinearExtension<F: Field>: Clone + Debug + for<'a> AddAssign<(&'a F, &'a Self)> {
...
impl<F: Field> MultilinearExtension<F> for MultilinearPolynomial<F> where Standard: Distribution<F> {
    type Point = Vec<F>;
