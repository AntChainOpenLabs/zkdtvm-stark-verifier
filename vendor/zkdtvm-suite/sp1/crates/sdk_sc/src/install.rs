//! # zkDTVM Install
//!
//! A library for installing the zkDTVM circuit artifacts.
...
use crate::DT_CIRCUIT_VERSION;

/// The base URL for the S3 bucket containing the circuit artifacts.
pub const CIRCUIT_ARTIFACTS_URL_BASE: &str = "https://dt-circuits.s3-us-east-2.amazonaws.com";

/// The directory where the groth16 circuit artifacts will be stored.
#[must_use]
pub fn groth16_circuit_artifacts_dir() -> PathBuf {
    std::env::var("DT_GROTH16_CIRCUIT_PATH")
        .map_or_else(
            |_| dirs::home_dir().unwrap().join(".dt").join("circuits/groth16"),
            |path| path.parse().unwrap(),
        )
        .join(DT_CIRCUIT_VERSION)
}

/// The directory where the plonk circuit artifacts will be stored.
#[must_use]
pub fn plonk_circuit_artifacts_dir() -> PathBuf {
    std::env::var("DT_PLONK_CIRCUIT_PATH")
        .map_or_else(
            |_| dirs::home_dir().unwrap().join(".dt").join("circuits/plonk"),
            |path| path.parse().unwrap(),
        )
        .join(DT_CIRCUIT_VERSION)
}
...
pub fn try_install_circuit_artifacts(artifacts_type: &str) -> PathBuf {
    ...
    if build_dir.exists() {
        eprintln!(
            "[dt] {} circuit artifacts already seem to exist at {}. if you want to re-download them, delete the directory",
            artifacts_type,
            build_dir.display()
        );
    } else {
        cfg_if! {
            if #[cfg(any(feature = "network", feature = "network"))] {
                eprintln!(
                    "[dt] {} circuit artifacts for version {} do not exist at {}. downloading...",
                    artifacts_type,
                    DT_CIRCUIT_VERSION,
                    build_dir.display()
                );
...
pub fn install_circuit_artifacts(build_dir: PathBuf, artifacts_type: &str) {
    ...
    let download_url =
        format!("{CIRCUIT_ARTIFACTS_URL_BASE}/{DT_CIRCUIT_VERSION}-{artifacts_type}.tar.gz");
    ...
    eprintln!("[dt] downloaded {} to {:?}", download_url, build_dir.to_str().unwrap(),);
}
