...
use anyhow::Result;
use dt_core_executor::{DTContextBuilder, ExecutionReport, HookEnv, IoWriter};
...
///
/// This builder providers a typed interface for configuring the zkDTVM RISC-V executor. The builder
/// is used for all the different variants of the [`crate::ProverClient`].
pub struct CpuExecuteBuilder<'a> {
    pub(crate) elf: &'a [u8],
    pub(crate) stdin: DTStdin,
    pub(crate) prover: &'a DTProver<SCCpuProverComponents>,
    pub(crate) context_builder: DTContextBuilder<'a>,
}

impl<'a> CpuExecuteBuilder<'a> {
    /// Add a executor [`dt_core_executor::Hook`] into the context.
    ///
    /// # Arguments
    ...
    /// [`dt_core_executor::ExecutionError::ExceededCycleLimit`]. This is useful for preventing
    ...
    /// ```
    pub fn run(self) -> Result<(DTPublicValues, ExecutionReport)> {
