//! # zkDTVM SDK Utilities
//!
//! A collection of utilities for the zkDTVM SDK.

use dt_core_machine::io::DTStdin;
pub use dt_core_machine::utils::setup_logger;

/// Dump the program and stdin to files for debugging if `DT_DUMP` is set.
pub(crate) fn dt_dump(elf: &[u8], stdin: &DTStdin) {
    if std::env::var("DT_DUMP").map(|v| v == "1" || v.to_lowercase() == "true").unwrap_or(false) {
...
/// Check that zkDTVM SDK was built in release mode. Ensures that the prover and executor
/// will be performant, which is important for benchmarking.
pub(crate) fn check_release_build() {
    #[cfg(debug_assertions)]
    panic!("dt-sdk must be built in release mode, please compile with the --release flag.");
}
