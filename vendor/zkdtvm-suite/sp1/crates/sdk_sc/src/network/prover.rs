...
use crate::{
    ...
    prover::verify_proof,
    DTProofMode, DTProofWithPublicValues, DTProvingKey, DTVerifyingKey, ProofFromNetwork, Prover,
};
use alloy_primitives::{Address, B256, U256};
use anyhow::{Context, Result};
use dt_core_executor::{DTContext, DTContextBuilder};
use dt_core_machine::io::DTStdin;
use dt_prover::{components::SCCpuProverComponents, DTProver, HashableKey, DT_CIRCUIT_VERSION};
...
impl NetworkProver {
    ...
    /// ```
    pub async fn register_program(&self, vk: &DTVerifyingKey, elf: &[u8]) -> Result<B256> {
    ...
    /// # Example
    /// ```rust,no_run
    /// use dt_sdk::{network::B256, ProverClient};
    ///
    /// tokio_test::block_on(async {
    ...
    pub async fn get_proof_status(
        &self,
        request_id: B256,
    ) -> Result<(GetProofRequestStatusResponse, Option<DTProofWithPublicValues>)> {
    ...
    ) -> Result<(Option<DTProofWithPublicValues>, FulfillmentStatus)> {
        // Get the status.
        let (status, maybe_proof): (GetProofRequestStatusResponse, Option<ProofFromNetwork>) =
    ...
    pub(crate) async fn request_proof(
        ...
        tracing::info!("└─ Circuit version: {}", DT_CIRCUIT_VERSION);
    ...
    fn get_execution_limits(
        ...
        let execute_result = self
            .prover
            .inner()
            .execute(elf, stdin, DTContext::builder().calculate_gas(true).build())
...
impl Prover<SCCpuProverComponents> for NetworkProver {
    fn setup(&self, elf: &[u8]) -> (DTProvingKey, DTVerifyingKey) {
        self.prover.setup(elf)
    }

    fn inner(&self) -> &DTProver {
        self.prover.inner()
    }
    ...
    fn verify(
        ...
        vkey: &DTVerifyingKey,
    ) -> Result<(), crate::DTVerificationError> {
        if let Some(tee_proof) = &bundle.tee_proof {
            if self.tee_signers.is_empty() {
                return Err(crate::DTVerificationError::Other(anyhow::anyhow!(
            ...
            let version_hash =
                alloy_primitives::keccak256(crate::network::tee::DT_TEE_VERSION.to_le_bytes());
            ...
            if self.tee_signers.contains(&address) {
                ...
                Err(crate::DTVerificationError::Other(anyhow::anyhow!(
...
impl From<DTProofMode> for ProofMode {
    fn from(value: DTProofMode) -> Self {
        match value {
            DTProofMode::Core => Self::Core,
            DTProofMode::Compressed => Self::Compressed,
            // The network protocol's `ProofMode` enum has no dedicated `Shrink`
            // variant; map shrink requests to `Compressed` for wire purposes
            // (the local SDK still distinguishes the two stages internally).
            DTProofMode::Shrink => Self::Compressed,
            DTProofMode::Plonk => Self::Plonk,
            DTProofMode::Groth16 => Self::Groth16,
        }
    }
}
