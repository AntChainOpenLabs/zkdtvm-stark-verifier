use crate::network::utils::sign_raw;
...
impl TEEResponse {
    ...
    pub fn as_prefix_bytes(&self) -> Vec<u8> {
        ...
        // Include the version of the zkDTVM circuit.
        let version_bytes = super::DT_TEE_VERSION.to_le_bytes();
    ...
    /// The selector for the TEE verifier.
    fn selector() -> [u8; 4] {
        alloy_primitives::keccak256("DTTeeVerifier")[0..4].try_into().unwrap()
    }
}
