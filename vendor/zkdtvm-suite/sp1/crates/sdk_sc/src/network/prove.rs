...
use crate::{
    utils::{block_on, dt_dump},
    DTProofMode, DTProofWithPublicValues, NetworkProver,
};
...
impl NetworkProveBuilder<'_> {
    ...
    /// # Example
    /// ```rust,no_run
    /// use dt_sdk::{network::FulfillmentStrategy, DTStdin, Prover, ProverClient};
    ///
    /// let elf = &[1, 2, 3];
    /// let stdin = DTStdin::new();
    ///
    ...
    /// ```
    pub async fn run_async(mut self) -> Result<DTProofWithPublicValues> {
        ...
        dt_dump(&self.pk.elf, &self.stdin);
...
impl<'a> IntoFuture for NetworkProveBuilder<'a> {
    type Output = Result<DTProofWithPublicValues>;
