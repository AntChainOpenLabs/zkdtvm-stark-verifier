...
//! curve.
//!
//! This implementation is specifically for use inside of zkDTVM, and internally uses zkDTVM's
//! Weierstrass precompiles. Weierstrass precompiles.
//!
//! In summary, zkDTVM overrides curve arithmetic entirely, and patches upstream field operations
//! to be more efficient in the VM, such as `sqrt` or `inverse`.

use crate::utils::AffinePoint as DTAffinePointTrait;
...
/// The affine point type for zkDTVM.
pub mod affine;
pub use affine::AffinePoint;

/// The projective point type for zkDTVM.
pub mod projective;
...
/// A [`CurveArithmetic`] extension for zkDTVM acceleration.
///
/// Patched crates implement this trait to take advantage of zkDTVM-specific acceleration in the
/// zkVM context.
///
    ...
    >,
{
    ...
    /// The underlying [`DTAffinePointTrait`] implementation.
    type DTAffinePoint: ECDSAPoint;
...
/// Alias trait for the [`DTAffinePointTrait`] with 32 byte field elements.
pub trait ECDSAPoint: DTAffinePointTrait<POINT_LIMBS> + Clone + Copy + Debug + Send + Sync {
    #[inline]
    fn from(x: &[u8], y: &[u8]) -> Self {
        <Self as DTAffinePointTrait<POINT_LIMBS>>::from(x, y)
    }
}

impl<P> ECDSAPoint for P where
    P: DTAffinePointTrait<POINT_LIMBS> + Clone + Copy + Debug + Send + Sync
{
}
