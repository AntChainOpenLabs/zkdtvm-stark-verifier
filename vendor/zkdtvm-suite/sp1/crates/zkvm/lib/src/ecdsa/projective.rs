//! Implementation of the zkDTVM accelerated projective point. The projective point wraps the affine
//! point.
//!
//! This type is mainly used in the `ecdsa-core` algorithms.
//!
//! Note: When performing curve operations, accelerated crates for zkDTVM use affine arithmetic
//! instead of projective arithmetic for performance.

use super::{AffinePoint, DTAffinePointTrait, ECDSACurve};
...
/// The zkDTVM accelerated projective point.
#[derive(Clone, Copy, Debug)]
pub struct ProjectivePoint<C: ECDSACurve> {
    /// The inner affine point.
    ///
    /// zkDTVM uses affine arithmetic for all operations.
...
impl<C: ECDSACurve> ProjectivePoint<C> {
    ...
    fn to_zkvm_point(self) -> C::DTAffinePoint {
        self.inner.inner
    }

    fn as_zkvm_point(&self) -> &C::DTAffinePoint {
        &self.inner.inner
    }

    fn as_mut_zkvm_point(&mut self) -> &mut C::DTAffinePoint {
    ...
    fn from_zkvm_point(p: C::DTAffinePoint) -> Self {
...
impl<C: ECDSACurve> LinearCombination for ProjectivePoint<C> {
    fn lincomb(x: &Self, k: &Self::Scalar, y: &Self, l: &Self::Scalar) -> Self {
        ...
        let dt_point = C::DTAffinePoint::multi_scalar_multiplication(&a_bits_le, x, &b_bits_le, y);

        Self::from_zkvm_point(dt_point)
    }
}
...
impl<C: ECDSACurve, T: Borrow<C::Scalar>> Mul<T> for ProjectivePoint<C> {
    ...
    fn mul(mut self, rhs: T) -> Self::Output {
        let dt_point = self.as_mut_zkvm_point();
        dt_point.mul_assign(&be_bytes_to_le_words(rhs.borrow().to_repr()));
