//! Implementation of an affine point, with acceleration for operations in the context of zkDTVM.
//!
...
use super::{
    DTAffinePointTrait, ECDSACurve, ECDSAPoint, Field, FieldElement, FIELD_BYTES_SIZE_USIZE,
};
...
impl<C: ECDSACurve> AffinePoint<C> {
    ...
    pub fn from_field_elements_unchecked(x: FieldElement<C>, y: FieldElement<C>) -> Self {
        ...
        AffinePoint { inner: <C::DTAffinePoint as ECDSAPoint>::from(x_slice, y_slice) }
    }
    ...
    pub fn generator() -> Self {
        AffinePoint { inner: C::DTAffinePoint::GENERATOR_T }
    }

    /// Get the identity point.
    pub fn identity() -> Self {
        AffinePoint { inner: C::DTAffinePoint::identity() }
    }
...
impl<C: ECDSACurve> ConditionallySelectable for AffinePoint<C> {
    fn conditional_select(a: &Self, b: &Self, choice: Choice) -> Self {
        ...
        //
        // In the zkDTVM vm, there are no attempts made to prevent side channel attacks.
        if choice.into() {
            *b
        } else {
            *a
