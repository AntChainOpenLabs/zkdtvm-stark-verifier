...
use dt_recursion_core::air::{RecursionPublicValues, NUM_PV_ELMS_TO_HASH};
#[cfg(feature = "babybear")]
use dt_stark::baby_bear_poseidon2::MyHash as InnerHash;
#[cfg(feature = "koalabear")]
use dt_stark::koalabear_poseidon2::koala_bear_poseidon2::MyHash as InnerHash;
use dt_stark::{DTCoreOpts, Word};
use itertools::Itertools;
use p3_bn254_fr::Bn254Fr;
use p3_field::{AbstractField, PrimeField32};
use p3_symmetric::CryptographicHasher;

use crate::{DTCoreProofData, InnerSC, OuterSC};

/// Get the zkDTVM vkey BabyBear Poseidon2 digest this reduce proof is representing.
pub fn dt_vkey_digest_babybear(proof: &DTReduceProof<OuterSC>) -> [SCField; 8] {
    let proof = &proof.proof;
    let pv: &RecursionPublicValues<SCField> = proof.public_values.as_slice().borrow();
    pv.dt_vk_digest
}

/// Get the zkDTVM vkey Bn Poseidon2 digest this reduce proof is representing.
pub fn dt_vkey_digest_bn254(proof: &DTReduceProof<OuterSC>) -> Bn254Fr {
    babybears_to_bn254(&dt_vkey_digest_babybear(proof))
}

/// Compute the digest of the public values.
pub fn recursion_public_values_digest(
    config: &InnerSC,
    public_values: &RecursionPublicValues<SCField>,
) -> [SCField; 8] {
...
pub fn root_public_values_digest(
    config: &InnerSC,
    public_values: &RootPublicValues<SCField>,
) -> [SCField; 8] {
    let hash = InnerHash::new(config.perm.clone());
    let input = (*public_values.dt_vk_digest())
        .into_iter()
        .chain(
...
pub fn dt_committed_values_digest_bn254(proof: &DTReduceProof<OuterSC>) -> Bn254Fr {
    let proof = &proof.proof;
    let pv: &RecursionPublicValues<SCField> = proof.public_values.as_slice().borrow();
...
pub fn get_cycles(elf: &[u8], stdin: &DTStdin) -> u64 {
...
pub fn babybears_to_bn254(digest: &[SCField; 8]) -> Bn254Fr {
...
pub fn babybear_bytes_to_bn254(bytes: &[SCField; 32]) -> Bn254Fr {
    let mut result = Bn254Fr::zero();
    for (i, byte) in bytes.iter().enumerate() {
        debug_assert!(byte < &SCField::from_canonical_u32(256));
        if i == 0 {
