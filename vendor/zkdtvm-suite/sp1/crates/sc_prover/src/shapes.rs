use crate::{
    components::DTProverComponents, types::HashableKey, CompressAir, DTProver, InnerSC, Instant,
    ShrinkAir, ShrinkVerifyMachine,
};
use dt_core_machine::shape::CoreShapeConfig;
use dt_primitives::SCField;
pub use dt_recursion_circuit::machine::{
    DTCompressWithVkeyShape, DTDeferredShape, DTRecursionShape,
};
use dt_recursion_circuit::sc_machine::SCDTCompressWithVKeyWitnessValues;
use eyre::Result;
use p3_field::AbstractField;
use serde::{Deserialize, Serialize};

use dt_recursion_circuit::sc_machine::{SCDTDeferredWitnessValues, SCDTRecursionWitnessValues};

use dt_recursion_core::{
    shape::{RecursionShape, RecursionShapeConfig},
    RecursionProgram,
};
use dt_stark::{shape::OrderedShape, DIGEST_SIZE};
...
pub enum DTCompressProgramShape {
    Recursion(DTRecursionShape),
    Compress(DTCompressWithVkeyShape),
    Deferred(DTDeferredShape),
    Shrink(DTCompressWithVkeyShape),
}

impl DTCompressProgramShape {
...
pub fn check_shapes<C: DTProverComponents>(
    reduce_batch_size: usize,
    no_precompiles: bool,
    num_compiler_workers: usize,
    prover: &mut DTProver<C>,
) -> bool {
    let (shape_tx, shape_rx) =
        std::sync::mpsc::sync_channel::<DTCompressProgramShape>(num_compiler_workers);
    ...
    let all_maximal_shapes = DTProofShape::generate_maximal_shapes(
        &core_shape_config,
        recursion_shape_config,
        reduce_batch_size,
        no_precompiles,
    )
    .collect::<BTreeSet<DTProofShape>>();
    ...
    let compress_ok = std::thread::scope(|s| {
        ...
        all_maximal_shapes.into_iter().for_each(|program_shape| {
            shape_tx.send(DTCompressProgramShape::from_proof_shape(program_shape, height)).unwrap();
        });
...
pub fn build_vk_map<C: DTProverComponents + 'static>(
    reduce_batch_size: usize,
    dummy: bool,
    num_compiler_workers: usize,
    num_setup_workers: usize,
    indices: Option<Vec<usize>>,
) -> (BTreeSet<[SCField; DIGEST_SIZE]>, Vec<usize>, usize) {
    // Setup the prover.
    let mut prover = DTProver::<C>::new();
    ...
    let (vk_set, panic_indices, height) = if dummy {
        ...
        let dummy_set = DTProofShape::dummy_vk_map(
            &core_shape_config,
            recursion_shape_config,
            reduce_batch_size,
        )
        ...
        let (shape_tx, shape_rx) =
            std::sync::mpsc::sync_channel::<(usize, DTCompressProgramShape)>(num_compiler_workers);
        ...
        for shape in
            DTProofShape::generate(&core_shape_config, recursion_shape_config, reduce_batch_size)
        {
        ...
        std::thread::scope(|s| {
            ...
            for _ in 0..num_compiler_workers {
                ...
                s.spawn(move || {
                    while let Ok((i, shape)) = shape_rx.lock().unwrap().recv() {
                        tracing::trace!("shape: {:?}", shape);
                        let is_shrink = matches!(shape, DTCompressProgramShape::Shrink(_));
                        let prover = prover.clone();
            ...
            subset_shapes
                .clone()
                .into_iter()
                .map(|(i, shape)| (i, DTCompressProgramShape::from_proof_shape(shape, height)))
...
pub fn build_vk_map_to_file<C: DTProverComponents + 'static>(
    build_dir: PathBuf,
    reduce_batch_size: usize,
    dummy: bool,
    num_compiler_workers: usize,
...
impl DTProofShape {
    pub fn generate<'a>(
        core_shape_config: &'a CoreShapeConfig<SCField>,
        recursion_shape_config: &'a RecursionShapeConfig<SCField, CompressAir<SCField>>,
        reduce_batch_size: usize,
    ...
    pub fn generate_compress_shapes(
        recursion_shape_config: &'_ RecursionShapeConfig<SCField, CompressAir<SCField>>,
        reduce_batch_size: usize,
    ...
    pub fn generate_maximal_shapes<'a>(
        ...
        let core_shape_iter = if no_precompiles {
            core_shape_config.maximal_core_shapes(22).into_iter()
        } else {
            core_shape_config.maximal_core_plus_precompile_shapes(22).into_iter()
        };
        core_shape_iter
            .map(|core_shape| {
    ...
    pub fn dummy_vk_map<'a>(
        ...
        recursion_shape_config: &'a RecursionShapeConfig<SCField, CompressAir<SCField>>,
        reduce_batch_size: usize,
    ) -> BTreeMap<[SCField; DIGEST_SIZE], usize> {
        Self::generate(core_shape_config, recursion_shape_config, reduce_batch_size)
            .enumerate()
            .map(|(i, _)| ([SCField::from_canonical_usize(i); DIGEST_SIZE], i))
            .collect()
    }
}

impl DTCompressProgramShape {
    pub fn from_proof_shape(shape: DTProofShape, height: usize) -> Self {
        match shape {
            DTProofShape::Recursion(proof_shape) => Self::Recursion(proof_shape.into()),
            DTProofShape::Deferred(proof_shape) => {
                Self::Deferred(DTDeferredShape::new(vec![proof_shape].into(), height))
            }
            DTProofShape::Compress(proof_shapes) => Self::Compress(DTCompressWithVkeyShape {
                compress_shape: proof_shapes.into(),
                merkle_tree_height: height,
            }),
            DTProofShape::Shrink(proof_shape) => Self::Shrink(DTCompressWithVkeyShape {
...
impl<C: DTProverComponents> DTProver<C> {
    pub fn program_from_shape(
        &self,
        shape: DTCompressProgramShape,
        shrink_shape: Option<RecursionShape>,
    ) -> Arc<RecursionProgram<SCField>> {
        match shape {
            DTCompressProgramShape::Recursion(shape) => {
                let input = SCDTRecursionWitnessValues::<InnerSC>::dummy_polyair(
                    self.core_prover.machine(),
                    &shape,
                );
                self.recursion_program(&input)
            }
            DTCompressProgramShape::Deferred(shape) => {
                let input = SCDTDeferredWitnessValues::<InnerSC>::dummy_polyair(
                    self.compress_prover.machine(),
                    &shape,
                );
                self.deferred_program(&input)
            }
            DTCompressProgramShape::Compress(shape) => {
            ...
            DTCompressProgramShape::Shrink(shape) => {
                let input = SCDTCompressWithVKeyWitnessValues::<InnerSC>::dummy_polyair(
                    self.compress_prover.machine(),
                    &shape,
                );
                self.shrink_program(
                    shrink_shape.unwrap_or_else(ShrinkAir::<SCField>::shrink_shape),
                    &input,
                    ShrinkVerifyMachine::Compress,
                    false,
                    false,
                )
            }
