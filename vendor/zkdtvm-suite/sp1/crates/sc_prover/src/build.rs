...
pub use dt_recursion_circuit::witness::{OuterWitness, Witnessable};
pub use dt_recursion_core::stark::dt_dev_mode;
...
pub fn try_build_plonk_bn254_artifacts_dev(
    template_vk: &SCStarkVerifyingKey<OuterSC>,
    template_proof: &SCShardProof<OuterSC>,
) -> PathBuf {
    let build_dir = plonk_bn254_artifacts_dev_dir();
    println!("[dt] building plonk bn254 artifacts in development mode");
...
pub fn try_build_groth16_bn254_artifacts_dev(
    ...
    println!("[dt] building groth16 bn254 artifacts in development mode");
...
pub fn plonk_bn254_artifacts_dev_dir() -> PathBuf {
    dirs::home_dir().unwrap().join(".dt").join("circuits").join("dev")
}
...
/// gnark-wrap is quartic-only (BN254); ext5 is shrink-only, so this is gated off.
#[cfg(not(feature = "ext5"))]
pub fn build_constraints_and_witness(
    ...
    let template_input = SCDTCompressWitnessValues {
    ...
    let pv: &RecursionPublicValues<SCField> = template_proof.public_values.as_slice().borrow();
    let vkey_hash = babybears_to_bn254(&pv.dt_vk_digest);
    let committed_values_digest_bytes: [SCField; 32] =
...
/// the proof.
pub fn dummy_proof() -> (SCStarkVerifyingKey<OuterSC>, SCShardProof<OuterSC>) {
    let elf = include_bytes!("../elf/riscv32im-succinct-zkvm-elf");

    tracing::info!("initializing prover");
    let prover: DTProver = DTProver::new();
    ...
    #[cfg(feature = "ext5")]
    {
        ...
        panic!("ext5 root_shrink proofs use a final SHA256 PCS and are not wrapped recursively")
    }
...
#[cfg(not(feature = "ext5"))]
fn build_outer_circuit(template_input: &SCDTCompressWitnessValues<OuterSC>) -> Vec<Constraint> {
    ...
    // Verify the proof.
    SCDTWrapVerifier::verify(&mut builder, &wrap_machine, input);
