...
use dt_core_machine::{io::DTStdin, riscv::RiscvAir, shape::CoreShapeConfig, utils::setup_logger};
    ...
    for s3_path in program_list {
        ...
        let status = std::process::Command::new("aws")
            .args([
                "s3",
                "cp",
                &format!("s3://dt-testing-suite/{}/program.bin", s3_path),
                "program.bin",
            ])
            .status()
        ...
        let status = std::process::Command::new("aws")
            .args([
                "s3",
                "cp",
                &format!("s3://dt-testing-suite/{}/stdin.bin", s3_path),
                "stdin.bin",
            ])
            .status()
