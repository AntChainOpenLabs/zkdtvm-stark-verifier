...
use clap::Parser;
use dt_core_executor::{DTContext, Executor, Program, RiscvAirId};
use dt_core_machine::{io::DTStdin, riscv::RiscvAir, utils::setup_logger};
use dt_stark::{shape::Shape, DTCoreOpts};
...
fn main() {
    ...
    // Setup the options.
    let mut opts = DTCoreOpts { shard_batch_size: 1, ..Default::default() };
    ...
    for s3_path in program_list {
        ...
        let status = std::process::Command::new("aws")
            .args([
                "s3",
                "cp",
                &format!("s3://dt-testing-suite/{s3_path}/program.bin"),
                "program.bin",
            ])
            .status()
        ...
        // Download stdin.bin.
        let status = std::process::Command::new("aws")
            .args(["s3", "cp", &format!("s3://dt-testing-suite/{s3_path}/stdin.bin"), "stdin.bin"])
            .status()
        ...
        let stdin: DTStdin = bincode::deserialize(&stdin).expect("failed to deserialize stdin");
        ...
        for &log_shard_size in args.shard_sizes.iter() {
            ...
            let new_context = DTContext::default();
