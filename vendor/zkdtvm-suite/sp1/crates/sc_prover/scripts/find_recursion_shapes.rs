...
use dt_prover::{
    ...
    shapes::{check_shapes, DTProofShape},
    SCDTProver, ShrinkAir, REDUCE_BATCH_SIZE,
};
use dt_recursion_core::shape::RecursionShapeConfig;
use dt_stark::{shape::OrderedShape, MachineProver};
...
fn main() {
    ...
    let mut prover = SCDTProver::<SCCpuProverComponents>::new();
    ...
    assert!({
        ...
        catch_unwind(AssertUnwindSafe(|| {
            prover.shrink_prover.setup(&prover.program_from_shape(
                dt_prover::shapes::DTCompressProgramShape::from_proof_shape(
                    DTProofShape::Shrink(OrderedShape {
