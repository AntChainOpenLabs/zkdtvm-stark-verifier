use dt_recursion_compiler::ir::{Builder, DslIr, Felt, IrIter, SymbolicExt};
use dt_recursion_core::DIGEST_SIZE;
use dt_stark::{InnerChallenge, InnerChallengeMmcs, InnerPcsProof, InnerVal};
...
impl PolynomialShape {
    pub fn dimensions(&self) -> Dimensions {
        Dimensions { width: self.width, height: 1 << self.log_degree }
    }
    pub fn perm_dimensions(&self) -> Dimensions {
        Dimensions { width: self.width / 4, height: 1 << self.log_degree }
    }
}
#[derive(Debug, Clone)]
...
>(
    ...
    for coeff in proof.final_poly.iter() {
        let final_poly_felts = C::ext2felt(builder, *coeff);
    ...
    challenger.check_witness(builder, config.grinding_bits_query, proof.pow_witness);
...
pub fn verify_challenges<C: CircuitConfig<F = SC::Val>, SC: BabyBearFriConfigVariable<C>>(
    ...
    challenges
        ...
        .ir_par_map_collect::<(), _, _>(builder, |sub_builder, ((index_bits, query_proof), ro)| {
            ...
            assert!(!proof.final_poly.is_empty(), "final_poly must be non-empty");
            sub_builder.assert_ext_eq(folded_eval, proof.final_poly[0]);
        });
}
...
pub fn kb_dummy_hash() -> Hash<p3_koala_bear::KoalaBear, p3_koala_bear::KoalaBear, DIGEST_SIZE> {
    [p3_koala_bear::KoalaBear::zero(); DIGEST_SIZE].into()
}
            ...
            .map(|query_proof| {
                let commit_phase_openings = query_proof
                    ...
                    .map(|commit_phase_opening| {
                        ...
                        let leaf_values = commit_phase_opening
                            .opened_values
                            .iter()
                            .map(|&value| builder.eval(SymbolicExt::from_f(value)))
                            .collect::<Vec<_>>();
