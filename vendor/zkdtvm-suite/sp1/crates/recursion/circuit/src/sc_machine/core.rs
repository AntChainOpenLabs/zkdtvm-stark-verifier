use crate::{hash::FieldHasherVariable, sumcheck::SCBabyBearFriConfig};
use dt_core_machine::{
    riscv::{RiscvAir, MAX_CPU_LOG_DEGREE, MAX_LOG_NUMBER_OF_SHARDS},
...
use crate::{
    ...
    sumcheck::{
        ...
        sc_dummy_vk_and_shard_proof, verifier::SumcheckVerifier,
    },
};
...
use dt_stark::{
    air::{FullAir, MachineAir, PolyAirExtendable, PublicValues, POSEIDON_NUM_WORDS},
    sumcheck::{config::SCStarkGenericConfig, keys::SCStarkVerifyingKey, proof::SCShardProof},
    Challenge, Dom, SCStarkMachine, Word,
};
...
use super::public_values::sc_recursion_public_values_digest;
use crate::{
...
pub struct SCDTRecursiveVerifier<C: Config, SC: SCBabyBearFriConfig> {
    _phantom: PhantomData<(C, SC)>,
}

impl<C, SC> SCDTRecursiveVerifier<C, SC>
where
    C: CircuitConfig<F = SC::Val, EF = Challenge<SC>, Bit = Felt<SC::Val>>,
    SC: SCBabyBearFriConfigVariable<C>,
    SC::ValMmcs: Mmcs<SC::Val, ProverData<RowMajorMatrix<SC::Val>>: Clone>,
    <SC as FieldHasherVariable<C>>::DigestVariable: IntoIterator<Item = Felt<C::F>> + Copy,
    Builder<C>: CircuitV2Builder<C>,
{
    /// Verify a batch of zkDTVM shard proofs and aggregate their public values.p
    ///
    ...
    pub fn verify(
        ...
        machine: &SCStarkMachine<SC, RiscvAir<SC::Val>, RiscvAir<Challenge<SC>>>,
        input: SCDTRecursionWitnessVariable<C, SC>,
    ) {
        // Read input.
        let SCDTRecursionWitnessVariable {
            vk,
            shard_proofs,
            is_complete,
            is_first_shard,
        ...
        // Write all values to the public values struct and commit to them.
        {
            ...
            recursion_public_values.dt_vk_digest =
                vk_digest.into_iter().collect::<Vec<_>>().try_into().unwrap();
            ...
            recursion_public_values.digest =
                sc_recursion_public_values_digest::<C, SC>(builder, recursion_public_values);
    ...
    pub fn verify_polyair<A, const D: usize>(
        builder: &mut Builder<C>,
        machine: &PolyAirStarkMachine<SC, A, D>,
        input: SCDTRecursionWitnessVariable<C, SC>,
    ) where
        A: MachineAir<SC::Val>,
        SC::Val: PolyAirExtendable<D> + p3_field::PrimeField32,
...
impl<SC: SCBabyBearFriConfig> SCDTRecursionWitnessValues<SC> {
...
impl SCDTRecursionWitnessValues<SCBabyBearPoseidon2> {
    pub fn dummy(
        ...
        let (mut vks, shard_proofs): (Vec<_>, Vec<_>) = shape
            .proof_shapes
            .iter()
            .map(|shape| sc_dummy_vk_and_shard_proof(machine, shape))
            .unzip();
        ...
        BabyBear: PolyAirExtendable<D>,
    {
        let (mut vks, shard_proofs): (Vec<_>, Vec<_>) = shape
            .proof_shapes
            .iter()
            .map(|shape| polyair_sc_dummy_vk_and_shard_proof(machine, shape))
            .unzip();
        let vk = vks.pop().unwrap();
        Self {
            vk,
    ...
    >
{
    pub fn dummy(
        machine: &SCStarkMachine<
            dt_stark::koalabear_poseidon2::koala_bear_poseidon2::SCKoalaBearPoseidon2,
            RiscvAir<p3_koala_bear::KoalaBear>,
            RiscvAir<
                Challenge<
                    dt_stark::koalabear_poseidon2::koala_bear_poseidon2::SCKoalaBearPoseidon2,
                >,
        ...
        let (mut vks, shard_proofs): (Vec<_>, Vec<_>) = shape
            .proof_shapes
            .iter()
            .map(|shape| kb_sc_dummy_vk_and_shard_proof(machine, shape))
            .unzip();
        let vk = vks.pop().unwrap();
        Self {
            vk,
            shard_proofs,
            reconstruct_deferred_digest: [KoalaBear::zero(); DIGEST_SIZE],
            is_complete: shape.is_complete,
            is_first_shard: false,
        ...
        p3_koala_bear::KoalaBear: PolyAirExtendable<D>,
    {
        ...
        let (mut vks, shard_proofs): (Vec<_>, Vec<_>) = shape
            .proof_shapes
            .iter()
            .map(|shape| kb_polyair_sc_dummy_vk_and_shard_proof(machine, shape))
            .unzip();
        let vk = vks.pop().unwrap();
        Self {
            vk,
