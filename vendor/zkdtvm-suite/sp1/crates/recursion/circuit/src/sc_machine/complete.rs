...
///
/// This is a generalized version that works with any field type supported by `CircuitV2Builder`,
/// unlike the BabyBear-specific version in `machine/complete.rs`.
    ...
    Builder<C>: CircuitV2Builder<C>,
{
    ...
    for limb in global_interval_end[0].iter().chain(global_interval_end[2].iter()) {
        builder.assert_felt_eq(is_complete * *limb, C::F::zero());
    }
    for (limb, value) in global_interval_end[1].iter().enumerate() {
