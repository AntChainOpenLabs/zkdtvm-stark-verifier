...
use dt_stark::koalabear_poseidon2::koala_bear_poseidon2::{
    Challenge as KBInnerChallenge, Val as KBInnerVal,
};
use dt_stark::{
...
impl<C: CircuitConfig> Witnessable<C> for bool {
    ...
    fn read(&self, builder: &mut Builder<C>) -> Self::WitnessVariable {
        ...
        #[cfg(feature = "verify")]
        {
            C::read_bit_val(builder, C::N::from_bool(*self))
        }
    }
...
#[cfg(feature = "koalabear")]
impl<C: CircuitConfig<F = KBInnerVal>> Witnessable<C> for KBInnerVal {
    type WitnessVariable = Felt<KBInnerVal>;
...
#[cfg(feature = "koalabear")]
impl<C: CircuitConfig<F = KBInnerVal, EF = KBInnerChallenge>> Witnessable<C> for KBInnerChallenge {
    type WitnessVariable = Ext<KBInnerVal, KBInnerChallenge>;
...
// Under `ext5`, `KBInnerChallenge` is the quintic challenge, so the impl above
// no longer covers the binomial-quartic KoalaBear extension still used by the
// outer/wrap layer (`SCOuterChallenge`). Provide it explicitly here. Gated to
// `ext5` so it does not duplicate the impl above under the default ext4 build
// (where `KBInnerChallenge == BinomialExtensionField<KoalaBear, 4>`).
#[cfg(all(feature = "koalabear", feature = "ext5"))]
impl<
        C: CircuitConfig<
            F = KBInnerVal,
            EF = p3_field::extension::BinomialExtensionField<KBInnerVal, 4>,
        >,
    > Witnessable<C> for p3_field::extension::BinomialExtensionField<KBInnerVal, 4>
{
    type WitnessVariable =
        Ext<KBInnerVal, p3_field::extension::BinomialExtensionField<KBInnerVal, 4>>;
