...
pub use dt_recursion_compiler::ir::Witness as OuterWitness;
...
#[cfg(feature = "koalabear")]
use dt_recursion_core::stark::{SCOuterChallenge, SCOuterVal};
...
#[cfg(feature = "koalabear")]
impl WitnessWriter<SCOuterConfig> for OuterWitness<SCOuterConfig> {
    fn write_bit(&mut self, value: bool) {
        self.vars.push(Bn254Fr::from_bool(value));
    }
    ...
    fn write_felt(&mut self, value: SCOuterVal) {
        self.felts.push(value);
    }

    fn write_ext(&mut self, value: SCOuterChallenge) {
        self.exts.push(value);
    }
}
...
impl<C: CircuitConfig<N = Bn254Fr>> Witnessable<C> for Bn254Fr {
    ...
    fn read(&self, builder: &mut Builder<C>) -> Self::WitnessVariable {
        let mut var = builder.witness_var();
        #[cfg(feature = "verify")]
        {
            var.val = *self;
        }
