...
/// Reference: [dt_core::stark::StarkVerifyingKey]
#[derive(Clone)]
pub struct VerifyingKeyVariable<C: CircuitConfig<F = SC::Val>, SC: BabyBearFriConfigVariable<C>> {
    ...
    pub program_boundary_presence: Felt<C::F>,
    pub program_boundary_xy: [[Felt<C::F>; 11]; 2],
    pub owner_registry_entries: Vec<[Felt<C::F>; 2]>,
    pub owner_registry_digest: [Felt<C::F>; 32],
...
pub struct FriCommitPhaseProofStepVariable<C: CircuitConfig, H: FieldHasherVariable<C>> {
    ...
    /// Both leaf values in the pair `[v_even, v_odd]` for basefold dedup format.
    /// Empty for the TwoAdicFri path, which uses `sibling_value` instead.
    pub leaf_values: Vec<Ext<C::F, C::EF>>,
...
impl<C: CircuitConfig<F = SC::Val>, SC: BabyBearFriConfigVariable<C>> VerifyingKeyVariable<C, SC> {
        ...
        Challenger: CanObserveVariable<C, Felt<C::F>> + CanObserveVariable<C, SC::DigestVariable>,
    {
        ...
        let boundary_tag: Felt<C::F> = builder.eval(C::F::from_canonical_u32(0x0d12));
        challenger.observe(builder, boundary_tag);
        challenger.observe(builder, self.program_boundary_presence);
        for coordinate in &self.program_boundary_xy {
            challenger.observe_slice(builder, *coordinate);
        }

        let owner_count: Felt<C::F> =
            builder.eval(C::F::from_canonical_usize(self.owner_registry_entries.len()));
        challenger.observe(builder, owner_count);
        for entry in &self.owner_registry_entries {
            challenger.observe_slice(builder, *entry);
        }
        challenger.observe_slice(builder, self.owner_registry_digest);
    }

    /// Hash the verifying key + prep domains into a single digest.
    /// Hash the complete direct-key boundary/owner identity and prepared domains.
    pub fn hash(&self, builder: &mut Builder<C>) -> SC::DigestVariable
    where
        C::F: TwoAdicField,
        SC::DigestVariable: IntoIterator<Item = Felt<C::F>>,
    {
        let mut num_inputs = DIGEST_SIZE +
            1 +
            24 +
            1 +
            2 * self.owner_registry_entries.len() +
            32 +
            32 +
            33 +
            (7 * self.chip_information.len());
        ...
        inputs.push(builder.eval(C::F::from_canonical_u32(0x0d12)));
        inputs.push(self.program_boundary_presence);
        inputs.extend(self.program_boundary_xy.into_iter().flatten());
        inputs.push(builder.eval(C::F::from_canonical_usize(self.owner_registry_entries.len())));
        inputs.extend(self.owner_registry_entries.iter().flatten().copied());
        inputs.extend(self.owner_registry_digest);
        for value in dt_stark::global_d11::global146_identity_fields::<C::F>() {
            inputs.push(builder.eval(value));
        }
        inputs.extend(self.program_global_seed.into_iter().flatten());
