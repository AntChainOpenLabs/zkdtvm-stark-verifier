...
{
    fn sample_ext(&mut self, builder: &mut Builder<C>) -> Ext<C::F, C::EF> {
        let limbs = (0..<C::EF as AbstractExtensionField<C::F>>::D)
            .map(|_| self.sample(builder))
            .collect::<Vec<_>>();
        builder.ext_from_base_slice(&limbs)
    }
...
{
    fn observe(&mut self, builder: &mut Builder<C>, value: [Felt<C::F>; N]) {
        for val in value {
            self.observe(builder, val);
        }
    }
}
...
//     };
//     use dt_recursion_core::stark::{outer_perm, BabyBearPoseidon2Outer, OuterCompress, OuterHash};
