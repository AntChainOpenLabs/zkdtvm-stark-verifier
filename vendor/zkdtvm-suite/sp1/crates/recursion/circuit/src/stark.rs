...
use dt_stark::{
    ...
    shape::OrderedShape,
    AirOpenedValues, Challenge, Challenger, Chip, ChipOpenedValues, InnerChallenge,
    InteractionKind, ShardCommitment, ShardOpenedValues, ShardProof, StarkGenericConfig,
    StarkMachine, StarkVerifyingKey, Val, PROOF_MAX_NUM_PVS,
};
...
/// Reference: [dt_core::stark::ShardProof]
...
pub fn dummy_vk_and_shard_proof<A: MachineAir<BabyBear>>(
    ...
    assert_eq!(shard_chips.len(), opened_values.chips.len());
    ...
    let preprocessed_chip_information: Vec<_> = preprocessed_names_and_dimensions
        ...
        .map(|(name, width, log_height)| {
            let domain = <<BabyBearPoseidon2 as StarkGenericConfig>::Pcs as Pcs<
                BinomialExtensionField<<BabyBearPoseidon2 as StarkGenericConfig>::Val, 4>,
    ...
    A: MachineAir<Val<SC>>,
{
    pub fn natural_domain_for_degree(
        ...
        <SC::Pcs as Pcs<Challenge<SC>, SC::FriChallenger>>::natural_domain_for_degree(
            config.pcs(),
            degree,
        )
    }
        ...
        A: for<'a> Air<RecursiveVerifierConstraintFolder<'a, C>>,
    {
        ...
        // Observe the chip-local logup sums. Global interval state is carried once in the claim.
        ...
        // Verify the standard state/address/Global-interval expected imbalance.
        ...
        builder.assert_ext_eq(local_cumulative_sum, expected);
...
impl<C: CircuitConfig<F = SC::Val>, SC: BabyBearFriConfigVariable<C>> ShardProofVariable<C, SC> {
    pub fn contains_cpu(&self) -> bool {
        self.chip_ordering.contains_key("Add") ||
            self.chip_ordering.contains_key("AddPolyAir") ||
            self.chip_ordering.contains_key("Addi") ||
            self.chip_ordering.contains_key("AddiPolyAir") ||
            self.chip_ordering.contains_key("Jal") ||
            self.chip_ordering.contains_key("JalPolyAir")
    }
...
pub mod tests {
    ...
    use dt_core_machine::{io::DTStdin, riscv::RiscvAir, utils::setup_logger};
    use dt_recursion_compiler::{
    ...
    use dt_recursion_core::{air::Block, machine::RecursionAir, stark::BabyBearPoseidon2Outer};
}
