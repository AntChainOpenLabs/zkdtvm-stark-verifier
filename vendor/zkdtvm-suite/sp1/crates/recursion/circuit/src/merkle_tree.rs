...
use dt_core_machine::utils::log2_strict_usize;
...
mod kb_tests {
    ...
    type C = SCInnerConfig;
    type F = KoalaBear;
    type HV = SCKoalaBearPoseidon2;

    #[test]
    fn test_merkle_tree_inner_koalabear() {
        let mut rng = OsRng;
        let mut builder = Builder::<SCInnerConfig>::default();
        // Run five times with different randomness.
        for _ in 0..1 {
            // Test with different number of leaves.
            for j in 2..8 {
                ...
                for (i, leaf) in leaves.iter().enumerate() {
                    ...
                    let proof_variable = MerkleProofVariable::<SCInnerConfig, SCKoalaBearPoseidon2> {
                        index: index_bits,
                        path: path_variable,
                    };

                    verify::<SCInnerConfig, SCKoalaBearPoseidon2>(
                        &mut builder,
                        proof_variable,
                        value_variable,
                        root_variable,
