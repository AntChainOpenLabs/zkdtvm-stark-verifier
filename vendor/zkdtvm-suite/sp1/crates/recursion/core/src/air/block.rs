...
use dt_stark::air::{ChallengeExtension, DTAirBuilder, ExtensionAirBuilder};
...
impl<T: Clone> Block<T> {
    pub fn as_extension<AB: ExtensionAirBuilder<Var = T>>(&self) -> ChallengeExtension<AB::Expr> {
        let arr: [AB::Expr; D] = self.0.clone().map(|x| AB::Expr::zero() + x);
        ChallengeExtension(arr)
    }

    pub fn as_extension_from_base<AB: DTAirBuilder<Var = T>>(
        &self,
        base: AB::Expr,
    ) -> ChallengeExtension<AB::Expr> {
...
impl<T: AbstractField> From<T> for Block<T> {
    fn from(value: T) -> Self {
        let mut arr = core::array::from_fn(|_| T::zero());
        arr[0] = value;
        Self(arr)
    }
}
