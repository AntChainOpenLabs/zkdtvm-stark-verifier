...
use core::fmt::Debug;
use dt_core_machine::utils::indices_arr;
use dt_derive::AlignedBorrow;
use dt_stark::{air::POSEIDON_NUM_WORDS, Word, PROOF_MAX_NUM_PVS};
...
// dt_core should be set to `RECURSIVE_PROOF_NUM_PV_ELTS`.
const_assert_eq!(RECURSIVE_PROOF_NUM_PV_ELTS, PROOF_MAX_NUM_PVS);
const_assert_eq!(RECURSION_PUBLIC_VALUES_COL_MAP.global_has_opening, 82);
const_assert_eq!(RECURSION_PUBLIC_VALUES_COL_MAP.global_count, 83);
const_assert_eq!(RECURSION_PUBLIC_VALUES_COL_MAP.global_interval_start[0][0], 84);
const_assert_eq!(RECURSION_PUBLIC_VALUES_COL_MAP.global_interval_end[0][0], 117);
const_assert_eq!(RECURSION_PUBLIC_VALUES_COL_MAP.is_complete, 150);
const_assert_eq!(RECURSION_PUBLIC_VALUES_COL_MAP.contains_execution_shard, 151);
const_assert_eq!(RECURSION_PUBLIC_VALUES_COL_MAP.exit_code, 152);
const_assert_eq!(RECURSION_PUBLIC_VALUES_COL_MAP.digest[0], 153);
...
pub struct RecursionPublicValues<T> {
    ...
    /// The commitment to the dt program being proven.
    pub dt_vk_digest: [T; DIGEST_SIZE],

    /// The root of the vk merkle tree.
    pub vk_root: [T; DIGEST_SIZE],

    /// Canonical proof-system Global interval composed from authenticated child claims.
    pub global_has_opening: T,
    pub global_count: T,
