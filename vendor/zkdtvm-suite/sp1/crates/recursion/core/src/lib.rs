//! Recursion core: the AIR chip definitions and runtime for the recursion VM
//! that executes the compiled verifier circuits (compress / shrink / wrap).
...
///   * KoalaBear : 9-row layout (4 ext + 1 internal-rounds row + 4 ext) -> `SKINNY_NUM_SCRATCH =
///     8`.
///
/// cbindgen:ignore
...
pub const SKINNY_NUM_SCRATCH: usize = 8;
...
/// KoalaBear (9-row layout, 8 groups) via cargo feature gating.
...
/// The event encoding the inputs and outputs of an ext exp-reverse-bits operation. The `len`
/// operand is now stored as the length of the `exp` field.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
