    ...
    AB::Var: 'static,
{
    fn eval(&self, builder: &mut AB) {
        ...
        // ------------------------------------------------------------------
        // 2. External rounds: each external row folds two rounds. Row mapping: is_round[0] →
        //    external pair 0 (BEGIN_EXT_CONSTS[0], [1]) is_round[1] → external pair 1
        //    (BEGIN_EXT_CONSTS[2], [3]) is_round[3] → external pair 2 (END_EXT_CONSTS[0], [1])
        ...
        // ------------------------------------------------------------------
        // 3. Internal rounds: fold all 20 rounds into one row. Rounds 0..18: use round_witness[k]
        //    as witness for (state[0]+RC[k])^3 Round 19: compute inline (no witness needed, degree
        //    stays at 3).
        // ------------------------------------------------------------------
        {
