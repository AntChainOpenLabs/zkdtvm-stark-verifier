    ...
    AB::Var: 'static,
{
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        // ------------------------------------------------------------------
        // 2. External round (split into is_first vs !is_first). BabyBear uses SBOX_DEGREE=7: x^7 =
        //    x^3 * x^3 * x.
        // ------------------------------------------------------------------
        ...
        {
            let add_rc: [AB::Expr; WIDTH] =
                array::from_fn(|i| local_row.state_in[i].into() + prep_local.round_constants[i]);
            let sbox: [AB::Expr; WIDTH] = array::from_fn(|i| {
                let deg3 = add_rc[i].clone() * add_rc[i].clone() * add_rc[i].clone();
                deg3.clone() * deg3.clone() * add_rc[i].clone()
            });
