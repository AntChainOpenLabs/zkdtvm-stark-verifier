...
use dt_stark::{
    ...
    Challenge, Chip, SCStarkMachine, StarkGenericConfig, StarkMachine, PROOF_MAX_NUM_PVS,
};
...
use crate::{
    chips::{
        alu_base::{
            BaseAluChip, NUM_BASE_ALU_ENTRIES_PER_ROW, NUM_BASE_ALU_SHRINK_ENTRIES_PER_ROW,
        },
...
impl<F: PrimeField32 + BinomiallyExtendable<4>, const DEGREE: usize> RecursionAir<F, DEGREE> {
    ...
    pub fn shrink_shape() -> RecursionShape {
        // Tightened to match observed shrink-stage event counts (recorded via
        // `[shrink-diag]` instrumentation):
        //   MemoryConst: 171850 events / 2 entries-per-row = 85925 rows  -> 2^17.
        //   MemoryVar:   264455 events / 2 entries-per-row = 132228 rows -> 2^18.
        //   BaseAlu:     149761 events / 2 entries-per-row = 74881 rows  -> 2^17.
        //   ExtAlu:      308794 events / 3 entries-per-row = 102932 rows -> 2^17.
        //   P2SkinnyKb:  34736 events  * 9 rows-per-permute = 312624     -> 2^19.
        //   Select:      133928 events                                   -> 2^18.
        //   EERB:        27140 events                                    -> 2^15.
        //   PrefixSum:   8 events                                        -> 2^3.
        // Overall max_log_height = 19 is bottlenecked by Poseidon2SkinnyKb;
        // any further reduction requires shrinking the verified compress proof
        // (e.g. via larger blowup / fewer FRI queries upstream).
        // Capacity is verified at runtime in `shrink_program` (see the
        // `shrink height overflow` panic). If event counts grow beyond capacity
        // (e.g. compress FRI parameters change), bump the corresponding entry.
        // [C1] EERB 内联化（ShrinkConfig::exp_reverse_bits_ext 用 4 条 ExtAlu op/bit
        // 替代 ExtExpReverseBitsChip）后的 shape：
        //   - 删除 ExtExpReverseBits chip（shrink_machine 也同步删除注册）
        //   - ExtAlu 17 -> 18：吸纳 22310 bits × 4 op = ~89240 额外 events 新 events ≈ 257722 +
        //     89240 = 346962, /3 = 115654 < 2^18=262144 (44% 余量)
        //   - 其他 chip 沿用 B1' (676 KiB) 配置不动
        // [C2_safe] 当前代码 (合 C1 EERB 内联化 + commit 9551fb7a 的 shrink height
        // overflow panic 校验) 下，shrink 阶段实测 instruction counts:
        //   mem_const_events = 199,182, /2 = 99,591   rows -> 2^17 (76% util)
        //   mem_var_events   = 262,277, /2 = 131,139  rows -> 2^18 (50% util) [原 17 装不下]
        //   base_alu_events  = 148,613, /2 = 74,307   rows -> 2^17 (57% util) [原 16 装不下]
        //   ext_alu_events   = 412,209, /3 = 137,403  rows -> 2^18 (52% util)
        //   p2_skinny_events = 34,128 × 9 = 307,152   rows -> 2^19 (59% util) [原 18 装不下]
        //   select_events    = 131,992                rows -> 2^18 (50% util) [原 17 装不下]
        //   prefix_sum_checks_events = 0  (inlined into ExtAlu via ShrinkConfig::eq_poly)
        // 历史 647.14 KiB 数字 = panic 校验加入前 next_power_of_two 自动 fallback
        // 跑出的物理形态，本质上等价于本配置的最小可行档位。
    ...
    pub fn shrink_heights(program: &RecursionProgram<F>) -> Vec<(String, usize)> {
        ...
        result.extend(vec![
            ...
            (Self::PublicValues(PublicValuesChip).name(), PUB_VALUES_LOG_HEIGHT),
            (Self::PrefixSumChecks(PrefixSumChecksChip).name(), counts.prefix_sum_checks_events),
        ]);
    ...
    //     (
    //         Self::ExtAlu(ExtAluChip),
    //         counts.ext_alu_events.div_ceil(NUM_EXT_ALU_ENTRIES_PER_ROW),
    //     ),
    //     (Self::poseidon2_skinny_chip(), skinny_rows),
    //     (Self::Select(SelectChip), counts.select_events),
    //     (Self::PublicValues(PublicValuesChip), PUB_VALUES_LOG_HEIGHT),
    //     (Self::PrefixSumChecks(PrefixSumChecksChip), counts.prefix_sum_checks_events),
    //     (
    //         Self::ExtExpReverseBits(ExtExpReverseBitsChip::<DEGREE>),
    //         counts.ext_exp_reverse_bits_events,
    //     ),
    // ]
    // .map(|(chip, height)| (chip.name(), height))
    // .to_vec()
}
// }
...
impl<F: PrimeField32 + BinomiallyExtendable<4>, const DEGREE: usize> RecursionAir<F, DEGREE> {
    ...
    pub fn sc_machine_wide_with_all_chips<SC: SCStarkGenericConfig<Val = F>>(
        config: SC,
    ) -> SCStarkMachine<SC, Self, RecursionAir<Challenge<SC>, DEGREE>> {
        let chips = vec![
            RecursionAir::MemoryConst(MemoryConstChip::default()),
            RecursionAir::MemoryVar(MemoryVarChip::default()),
            RecursionAir::BaseAlu(BaseAluChip),
    ...
    /// consistent.
    ///
    /// [C1] `ExtExpReverseBits` chip is intentionally NOT registered here: the shrink-stage
    /// circuit is compiled with `ShrinkConfig` whose `exp_reverse_bits_ext` is inlined into
    /// pure ExtAlu ops, so no `Instruction::ExtExpReverseBits` is emitted and the chip would
    /// cause a runtime shape-lookup mismatch (see `RecursionProgram::fixed_log2_rows`).
    ///
    /// [C3] `PrefixSumChecks` chip is intentionally NOT registered here: the shrink-stage
    /// circuit is compiled with `ShrinkConfig` whose `eq_poly` is inlined into pure ExtAlu
    /// ops, so no `Instruction::PrefixSumChecks` is emitted and the chip would cause a
...
pub mod tests {
    ...
    pub fn run_recursion_test_machines(program: RecursionProgram<F>) {
        ...
        runtime.run().unwrap();
        // println!("poly eval events: {:?}, total num is: {:?}", runtime.record.poly_eval_events,
        // runtime.record.poly_eval_events.len());
