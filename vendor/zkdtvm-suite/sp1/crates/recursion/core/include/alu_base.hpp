...
namespace dt_recursion_core_sys::alu_base {
template <class F>
__DT_HOSTDEV__ void event_to_row(const BaseAluEvent<F>& event,
                                  BaseAluValueCols<F>& cols) {
  cols.vals = event;
}

template <class F>
__DT_HOSTDEV__ void instr_to_row(const BaseAluInstr<F>& instr,
