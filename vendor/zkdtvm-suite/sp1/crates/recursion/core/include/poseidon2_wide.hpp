...
namespace dt_recursion_core_sys::poseidon2_wide {
using namespace constants;
using namespace poseidon2;

template <class F>
__DT_HOSTDEV__ __DT_INLINE__ void populate_external_round(
  ...
  F add_rc[WIDTH];
  for (size_t i = 0; i < WIDTH; i++) {
    add_rc[i] = round_state[i] + F(F::to_monty(rc<F>(round, i)));
  }
  ...
  for (size_t i = 0; i < WIDTH; i++) {
    next_state[i] = poseidon2::sbox(add_rc[i]);
  }
  external_linear_layer<F>(next_state);
}

template <class F>
__DT_HOSTDEV__ __DT_INLINE__ void populate_internal_rounds(
  ...
  for (size_t r = 0; r < NUM_INTERNAL_ROUNDS; r++) {
    ...
    F add_rc = state[0] + F(F::to_monty(rc<F>(round, 0)));

    sbox_deg_3[r] = add_rc * add_rc * add_rc;

    state[0] = poseidon2::sbox(add_rc);
...
template <class F>
__DT_HOSTDEV__ __DT_INLINE__ void populate_perm(
...
template <class F>
__DT_HOSTDEV__ void event_to_row(const F input[WIDTH], F* input_row,
...
template <class F>
__DT_HOSTDEV__ void instr_to_row(const Poseidon2WideInstr<F>& instr,
