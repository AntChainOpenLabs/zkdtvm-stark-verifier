...
namespace dt_recursion_core_sys::alu_ext {
template <class F>
__DT_HOSTDEV__ void event_to_row(const ExtAluEvent<F>& event,
                                  ExtAluValueCols<F>& cols) {
  cols.vals = event;
}

template <class F>
__DT_HOSTDEV__ void instr_to_row(const ExtAluInstr<F>& instr,
