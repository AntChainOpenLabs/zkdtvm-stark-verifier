...
impl<F: Field> SymbolicFelt<F> {
    pub(crate) fn value(&self) -> F {
        match self {
            SymbolicFelt::Const(f) => *f,
            SymbolicFelt::Val(f) => f.val,
        }
    }
}
...
impl<F: Field, EF: ExtensionField<F>> SymbolicExt<F, EF> {
    pub fn value(&self) -> EF {
        match self {
            SymbolicExt::Const(ef) => *ef,
            SymbolicExt::Base(felt) => EF::from_base(felt.value()),
            SymbolicExt::Val(ext) => ext.val,
        }
    }
}
