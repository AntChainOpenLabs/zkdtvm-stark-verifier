...
use dt_recursion_core::runtime::{DIGEST_SIZE, HASH_RATE, PERMUTATION_WIDTH};
use p3_field::AbstractField;

macro_rules! poseidon2_op {
    (permute, $data:expr) => {{
        #[cfg(feature = "babybear")]
        {
            DslIr::Poseidon2PermuteBabyBear($data)
        }
        #[cfg(feature = "koalabear")]
        {
            DslIr::Poseidon2PermuteKoalaBear($data)
        }
    }};
    (absorb, $num:expr, $input:expr) => {{
        #[cfg(feature = "babybear")]
        {
            DslIr::Poseidon2AbsorbBabyBear($num, $input)
        }
        #[cfg(feature = "koalabear")]
        {
            DslIr::Poseidon2AbsorbKoalaBear($num, $input)
        }
    }};
    (finalize, $num:expr, $output:expr) => {{
        #[cfg(feature = "babybear")]
        {
            DslIr::Poseidon2FinalizeBabyBear($num, $output)
        }
        #[cfg(feature = "koalabear")]
        {
            DslIr::Poseidon2FinalizeKoalaBear($num, $output)
        }
    }};
    (compress, $data:expr) => {{
        #[cfg(feature = "babybear")]
        {
            DslIr::Poseidon2CompressBabyBear($data)
        }
        #[cfg(feature = "koalabear")]
        {
            DslIr::Poseidon2CompressKoalaBear($data)
        }
    }};
}
...
impl<C: Config> Builder<C> {
    ...
    pub fn poseidon2_permute(&mut self, array: &Array<C, Felt<C::F>>) -> Array<C, Felt<C::F>> {
        ...
        self.push_op(poseidon2_op!(permute, Box::new((output.clone(), array.clone()))));
        output
    }
    ...
    pub fn poseidon2_permute_mut(&mut self, array: &Array<C, Felt<C::F>>) {
        self.push_op(poseidon2_op!(permute, Box::new((array.clone(), array.clone()))));
    }
    ...
    pub fn poseidon2_absorb(
        ...
        self.push_op(poseidon2_op!(absorb, p2_hash_and_absorb_num, input.clone()));
    }
    ...
    pub fn poseidon2_finalize_mut(
        ...
        self.push_op(poseidon2_op!(finalize, p2_hash_num, output.clone()));
    }
    ...
    pub fn poseidon2_compress_x(
        ...
        self.push_op(poseidon2_op!(
            compress,
            Box::new((result.clone(), left.clone(), right.clone(),))
        ));
    }
