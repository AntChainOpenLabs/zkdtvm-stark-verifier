...
use dt_recursion_core::runtime::NUM_BITS;
...
#[allow(dead_code)]
pub fn to_bits_le(num: BigUint) -> Vec<u32> {
    let le_bytes = num.to_bytes_le();
    let mut ret = vec![];
    for byte in le_bytes {
        for bit in 0..8 {
            let bit_value = (byte >> bit) & 1;
            ret.push(bit_value as u32);
        }
    }
    ret
}
#[allow(dead_code)]
pub fn u32_to_le_bits(num: u32) -> Vec<u32> {
...
impl<C: Config> Builder<C> {
    ...
    pub fn num2bits_v_circuit(&mut self, num: Var<C::N>, bits: usize) -> Vec<Var<C::N>> {
        ...
        #[cfg(feature = "verify")]
        {
            let num_uint = num.val.as_canonical_biguint();
            let mut num_bits = to_bits_le(num_uint);
            assert!(bits > num_bits.len());
            num_bits.resize(bits, 0);
            assert_eq!(num_bits.len(), output.len());
            for i in 0..num_bits.len() {
                output[i].val = C::N::from_canonical_u32(num_bits[i]);
            }
        }
        output
    }
    ...
    pub fn num2bits_f_circuit(&mut self, num: Felt<C::F>) -> Vec<Var<C::N>> {
        let mut output: Vec<Var<C::N>> = Vec::new();
        for _ in 0..NUM_BITS {
            output.push(self.uninit());
        }
        #[cfg(feature = "verify")]
        {
            let mut num_bits = u32_to_le_bits(num.val.as_canonical_u32());
            num_bits.resize(NUM_BITS, 0);
            assert_eq!(num_bits.len(), output.len());
            for i in 0..num_bits.len() {
                output[i].val = C::N::from_canonical_u32(num_bits[i]);
            }
