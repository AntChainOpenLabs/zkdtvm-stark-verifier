...
impl<N> Var<N> {
    pub fn new(idx: u32, handle: *mut VarHandle<N>) -> Self
    where
        N: Field,
    {
...
impl<F> Felt<F> {
    pub fn new(id: u32, handle: *mut FeltHandle<F>) -> Self
    where
        F: Field,
    {
        #[cfg(not(feature = "verify"))]
...
impl<F, EF> Ext<F, EF> {
    pub fn new(id: u32, handle: *mut ExtHandle<F, EF>) -> Self
    where
        EF: Field,
    {
        #[cfg(not(feature = "verify"))]
        ...
        #[cfg(feature = "verify")]
        {
            Self { idx: id, handle, val: EF::zero() }
        }
    }
...
impl<C: Config> Variable<C> for Usize<C::N> {
    ...
    fn assign(&mut self, src: Self::Expression, builder: &mut Builder<C>) {
        match self {
            Usize::Const(_) => {
        ...
        match (lhs, rhs) {
            ...
            (SymbolicVar::Const(lhs), SymbolicVar::Val(rhs)) => {
                #[cfg(feature = "verify")]
                {
                    ...
                    let right = rhs.val;
                    if left != right {
                        log::debug!("CONSTRAINT UNSAT, left: {left}, right: [{right}]");
                    }
                    // assert_eq!(left, right, "Assertion failed at compile time");
                }
        ...
        match (lhs, rhs) {
            ...
            (SymbolicVar::Const(lhs), SymbolicVar::Val(rhs)) => {
                #[cfg(feature = "verify")]
                {
                    ...
                    // assert_ne!(left, right, "Assertion failed at compile time");
                }
            ...
            SymbolicExt::Base(src) => match src {
                SymbolicFelt::Const(src) => {
                    #[cfg(feature = "verify")]
                    {
                        // unsafe {
                        //     let self_mut = self as *const Self as *mut Self;
                        //     (*self_mut).val = C::EF::from_base(src);
                        // }
                        self.val = C::EF::from_base(src);
                    }
                ...
                SymbolicFelt::Val(src) => {
                    ...
                    {
                        let val = src.val;
                        self.val = C::EF::from_base(val);
                        // unsafe {
                        //     let self_mut = self as *const Self as *mut Self;
                        //     (*self_mut).val = C::EF::from_base(val);
                        // }
                    }
    ...
    fn assert_eq(
        ...
        match (lhs, rhs) {
            ...
            (SymbolicExt::Const(lhs), rhs) => {
                ...
                let mut rhs_value = Self::uninit(builder);
            ...
            (lhs, rhs) => {
                ...
                let mut lhs_value = Self::uninit(builder);
