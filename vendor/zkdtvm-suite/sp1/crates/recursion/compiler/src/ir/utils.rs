...
impl<C: Config> Builder<C> {
    ...
    pub fn select_f(&mut self, cond: Var<C::N>, a: Felt<C::F>, b: Felt<C::F>) -> Felt<C::F> {
        let mut c: Felt<<C as Config>::F> = self.uninit();
        #[cfg(feature = "verify")]
        {
            if cond.val != C::N::zero() {
                c.val = a.val;
            } else {
                c.val = b.val;
            }
    ...
    pub fn select_ef(
        ...
        let mut c: Ext<C::F, C::EF> = self.uninit();
        #[cfg(feature = "verify")]
        {
            if cond.val != C::N::zero() {
                c.val = a.val;
    ...
    /// A version of `exp_reverse_bits_len` that uses a loop (ExpReverseBitsLen precompile removed).
    pub fn exp_reverse_bits_len_fast(
        ...
        self.exp_reverse_bits_len(x, power_bits, bit_len)
    }
    ...
    pub fn ext_from_base_slice(&mut self, arr: &[Felt<C::F>]) -> Ext<C::F, C::EF> {
        ...
        let mut r: Ext<_, _> = self.eval(res);
        #[cfg(feature = "verify")]
        {
            r.val = res.value();
        }
    ...
    pub fn ext2felt_circuit(&mut self, value: Ext<C::F, C::EF>) -> [Felt<C::F>; D] {
        let mut felts = core::array::from_fn(|_| self.uninit());
        self.push_op(DslIr::CircuitExt2Felt(felts, value));
        #[cfg(feature = "verify")]
        {
            let decomposed_ext = value.val.as_base_slice();
            for i in 0..D {
                felts[i].val = decomposed_ext[i];
            }
        }
        felts
    }
