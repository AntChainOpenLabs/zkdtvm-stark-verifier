...
impl<C: Config> VarOperations<C::N> for UnsafeCell<InnerBuilder<C>> {
    fn add_var(ptr: *mut (), lhs: Var<C::N>, rhs: Var<C::N>) -> Var<C::N> {
        ...
        let res = {
            ...
            {
                Var::new(idx, lhs.handle)
            }
            #[cfg(feature = "verify")]
            {
                let res_val = lhs.val + rhs.val;
                Var { idx, handle: lhs.handle, val: res_val }
            }
        };
    ...
    fn sub_const_var(ptr: *mut (), lhs: C::N, rhs: Var<C::N>) -> Var<C::N> {
        ...
        let res = {
            #[cfg(not(feature = "verify"))]
            {
                Var::new(idx, rhs.handle)
            }
            #[cfg(feature = "verify")]
            {
                let res_val = lhs - rhs.val;
                Var { idx, handle: rhs.handle, val: res_val }
            }
        };
        inner.variable_count += 1;
...
impl<C: Config> FeltOperations<C::F> for UnsafeCell<InnerBuilder<C>> {
    fn add_felt(ptr: *mut (), lhs: Felt<C::F>, rhs: Felt<C::F>) -> Felt<C::F> {
        ...
        let res = {
            #[cfg(not(feature = "verify"))]
            {
                Felt::new(idx, lhs.handle)
            }
            #[cfg(feature = "verify")]
            {
                let res_val = lhs.val + rhs.val;
                Felt { idx, handle: lhs.handle, val: res_val }
            }
        };
        inner.variable_count += 1;
    ...
    fn sub_const_felt(ptr: *mut (), lhs: C::F, rhs: Felt<C::F>) -> Felt<C::F> {
        ...
        let res = {
            #[cfg(not(feature = "verify"))]
            {
                Felt::new(idx, rhs.handle)
            }
            #[cfg(feature = "verify")]
            {
                let res_val = lhs - rhs.val;
                Felt { idx, handle: rhs.handle, val: res_val }
            }
        };
        inner.variable_count += 1;
    ...
    fn div_felt(ptr: *mut (), lhs: Felt<C::F>, rhs: Felt<C::F>) -> Felt<C::F> {
        ...
        let res = {
            ...
            #[cfg(feature = "verify")]
            {
                let res_val = if !lhs.val.is_zero() {
                    lhs.val.try_div(rhs.val).expect("division by zero")
                } else {
                    C::F::zero()
                };
                Felt { idx, handle: lhs.handle, val: res_val }
    ...
    fn div_felt_const(ptr: *mut (), lhs: Felt<C::F>, rhs: C::F) -> Felt<C::F> {
        ...
        let res = {
            ...
            {
                let res_val = if !lhs.val.is_zero() {
                    lhs.val.try_div(rhs).expect("division by zero")
                } else {
                    C::F::zero()
                };
                Felt { idx, handle: lhs.handle, val: res_val }
    ...
    fn div_const_felt(ptr: *mut (), lhs: C::F, rhs: Felt<C::F>) -> Felt<C::F> {
        ...
        let res = {
            ...
            #[cfg(feature = "verify")]
            {
                let res_val = if !lhs.is_zero() {
                    lhs.try_div(rhs.val).expect("division by zero")
                } else {
                    C::F::zero()
                };
                Felt { idx, handle: rhs.handle, val: res_val }
...
impl<C: Config> ExtOperations<C::F, C::EF> for UnsafeCell<InnerBuilder<C>> {
    fn add_ext(ptr: *mut (), lhs: Ext<C::F, C::EF>, rhs: Ext<C::F, C::EF>) -> Ext<C::F, C::EF> {
        ...
        let res = {
            #[cfg(not(feature = "verify"))]
            {
                Ext::new(idx, lhs.handle)
            }
            #[cfg(feature = "verify")]
            {
                let res_val = lhs.val + rhs.val;
                Ext { idx, handle: lhs.handle, val: res_val }
            }
        };
        inner.variable_count += 1;
    ...
    fn add_felt_const_ext(
        ...
        let res = {
            ...
            #[cfg(feature = "verify")]
            {
                let res_val = rhs + lhs.val;
                Ext { idx, handle, val: res_val }
            }
        };
        let inner = inner.get_mut();
    ...
    fn sub_ext_const(ptr: *mut (), lhs: C::EF, rhs: Ext<C::F, C::EF>) -> Ext<C::F, C::EF> {
        ...
        let res = {
            #[cfg(not(feature = "verify"))]
            {
                Ext::new(idx, rhs.handle)
            }
            #[cfg(feature = "verify")]
            {
                let res_val = lhs - rhs.val;
                Ext { idx, handle: rhs.handle, val: res_val }
            }
        };
        inner.variable_count += 1;
