...
use dt_recursion_core::{air::RecursionPublicValues, D};
...
pub enum DslIr<C: Config> {
    ...
    /// Permutes an array of koala bear elements using Poseidon2.
    Poseidon2PermuteKoalaBear(Box<(Array<C, Felt<C::F>>, Array<C, Felt<C::F>>)>),
    /// Compresses two koala bear element arrays using Poseidon2.
    ...
    /// Finalize and return the hash digest of a specified koala bear hash instance.
    ...
    /// V2 Poseidon2 permutation for KoalaBear (circuit). The compiler decides whether
    /// to lower this to the wide or skinny chip based on its compile-time mode.
    ...
    CircuitExt2Felt([Felt<C::F>; D], Ext<C::F, C::EF>),
    /// Converts a slice of felts to an ext. Should only be used when target is a gnark circuit.
    CircuitFelts2Ext([Felt<C::F>; D], Ext<C::F, C::EF>),

    // Sumcheck specific instructions.
    /// Executes a field polynomial evaluation at x
    CircuitV2PolyEval(Felt<C::F>, Vec<Felt<C::F>>, Felt<C::F>),
    // Extension field exponentiation.
    /// Exponentiation. Output, base, exponent.
    CircuitV2ExtExpReverseBits(Ext<C::F, C::EF>, Ext<C::F, C::EF>, Vec<Felt<C::F>>),

    /// SumcheckRound: output, coeffs, challenge, claim.
    CircuitV2SumcheckRound(
        Ext<C::F, C::EF>,
        Vec<Ext<C::F, C::EF>>,
        Ext<C::F, C::EF>,
        Ext<C::F, C::EF>,
    ...
    /// PrefixSumChecks: result, x1_vec, x2_vec.
    CircuitV2PrefixSumChecks(Ext<C::F, C::EF>, Vec<Ext<C::F, C::EF>>, Vec<Ext<C::F, C::EF>>),
