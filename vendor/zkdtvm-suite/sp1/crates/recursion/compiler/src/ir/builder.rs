#![allow(unused_mut)]
use std::{cell::UnsafeCell, fmt::Debug, ptr};

use dt_primitives::types::RecursionProgramType;
...
impl<C: Config> Builder<C> {
    ...
    pub fn eval<V: Variable<C>, E: Into<V::Expression>>(&mut self, expr: E) -> V {
        let mut dst = V::uninit(self);
        // log::debug!("eval new variable {:?}", dst);
        dst.assign(expr.into(), self);
        // log::debug!("eval after assign, {:?}", dst);
        dst
    }
    ...
    pub fn assign<V: Variable<C>, E: Into<V::Expression>>(&mut self, mut dst: V, expr: E) {
        dst.assign(expr.into(), self);
    }
    ...
    pub fn lt(&mut self, lhs: Var<C::N>, rhs: Var<C::N>) -> Var<C::N> {
        let mut result: Var<<C as Config>::N> = self.uninit();
        #[cfg(feature = "verify")]
        {
            use p3_field::PrimeField;

            if lhs.val.as_canonical_biguint() < rhs.val.as_canonical_biguint() {
                result.val = C::N::one();
            }
        }
    ...
    pub fn felt2var_circuit(&mut self, felt: Felt<C::F>) -> Var<C::N> {
        let mut var: Var<<C as Config>::N> = self.uninit();
        #[cfg(feature = "verify")]
        {
            var.val = C::N::from_canonical_u32(felt.val.as_canonical_u32());
        }
