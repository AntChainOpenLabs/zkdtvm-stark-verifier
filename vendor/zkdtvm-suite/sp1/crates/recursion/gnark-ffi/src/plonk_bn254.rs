...
use crate::{
    ...
    PlonkBn254Proof, DT_CIRCUIT_VERSION,
};
use anyhow::Result;
...
impl PlonkBn254Prover {
    ...
    pub fn test<C: Config>(constraints: Vec<Constraint>, witness: Witness<C>) {
        ...
        tracing::info!("begin test plonk bn254");
        test_plonk_bn254(
            witness_file.path().to_str().expect("witness file to str failed"),
            constraints_file.path().to_str().expect("costraints file to str failed"),
        );
    }
    ...
    pub fn build<C: Config>(constraints: Vec<Constraint>, witness: Witness<C>, build_dir: PathBuf) {
        ...
        let dt_verifier_path = build_dir.join("DTVerifierPlonk.sol");
        let vkey_hash = Self::get_vkey_hash(&build_dir);
        let dt_verifier_str = include_str!("../assets/DTVerifierPlonk.txt")
            .replace("{DT_CIRCUIT_VERSION}", DT_CIRCUIT_VERSION)
            .replace("{VERIFIER_HASH}", format!("0x{}", hex::encode(vkey_hash)).as_str())
            .replace("{PROOF_SYSTEM}", "Plonk");
        fs::write(dt_verifier_path, dt_verifier_str).unwrap();
    }
