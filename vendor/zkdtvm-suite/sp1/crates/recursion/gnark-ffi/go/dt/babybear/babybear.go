...
func ZeroExt() ExtensionVariable {
	return ExtensionVariable{Value: [4]Variable{Zero(), Zero(), Zero(), Zero()}}
}
func One() Variable {
	return Variable{
...
func OneExt() ExtensionVariable {
	return ExtensionVariable{Value: [4]Variable{One(), Zero(), Zero(), Zero()}}
}
...
func (c *Chip) ExtPolyEval(x ExtensionVariable, poly []ExtensionVariable) ExtensionVariable {
	result := poly[0]
	if len(poly) == 1 {
		return result
	}
	for i := 1; i < len(poly); i++ {
		result = c.AddE(poly[i], c.MulE(result, x))
	}
	return result
}
func (c *Chip) ExtExpReverseBits(x ExtensionVariable, exp []Variable) ExtensionVariable {
	result := OneExt()

	for i := range exp {
		result = c.MulE(result, result)
		is_one := c.MulEF(x, exp[i])
		is_zero := c.SubEF(OneExt(), exp[i])
		multi := c.AddE(is_one, is_zero)
		result = c.MulE(result, multi)
	}

	return result
}

func (c *Chip) ExtDotProd(x_vec []ExtensionVariable, y_vec []ExtensionVariable) ExtensionVariable {
	result := c.MulE(x_vec[0], y_vec[0])
	if len(x_vec) == 1 {
		return result
	}
	for i := 1; i < len(x_vec); i++ {
		result = c.AddE(result, c.MulE(x_vec[i], y_vec[i]))
	}
	return result
}

func (c *Chip) ExtDotProdBase(x_vec []ExtensionVariable, y_vec []Variable) ExtensionVariable {
	result := c.MulEF(x_vec[0], y_vec[0])
	if len(x_vec) == 1 {
		return result
	}
	for i := 1; i < len(x_vec); i++ {
		result = c.AddE(result, c.MulEF(x_vec[i], y_vec[i]))
	}
	return result
}

func (c *Chip) EqPoly(x_vec []ExtensionVariable, y_vec []ExtensionVariable) ExtensionVariable {
	eq_one := c.MulE(x_vec[0], y_vec[0])
	eq_zero := c.MulE(c.SubE(OneExt(), x_vec[0]), c.SubE(OneExt(), y_vec[0]))
	result := c.AddE(eq_one, eq_zero)
	// result := c.AddE(c.MulE(x_vec[0], y_vec[0]),c.MulE(c.SubE(OneExt(), x_vec[i]), c.SubE(OneExt(), y_vec[i])))
	if len(x_vec) == 1 {
		return result
	}
	for i := 1; i < len(x_vec); i++ {
		eq_one := c.MulE(x_vec[i], y_vec[i])
		eq_zero := c.MulE(c.SubE(OneExt(), x_vec[i]), c.SubE(OneExt(), y_vec[i]))
		result = c.MulE(result, c.AddE(eq_one, eq_zero))
	}
	return result
}
