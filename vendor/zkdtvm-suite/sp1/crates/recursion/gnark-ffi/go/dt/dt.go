...
import (
	...
	"github.com/succinctlabs/dt-recursion-gnark/dt/babybear"
	"github.com/succinctlabs/dt-recursion-gnark/dt/poseidon2"
)
...
func (circuit *Circuit) Define(api frontend.API) error {
	...
	for _, cs := range constraints {
		...
		case "ExtPolyEval":
			coe := []babybear.ExtensionVariable{}
			for i := 0; i < len(cs.Args[1]); i++ {
				coe = append(coe, exts[cs.Args[1][i]])
			}
			exts[cs.Args[0][0]] = fieldAPI.ExtPolyEval(exts[cs.Args[2][0]], coe)
		// case "ExtExpReverseBits":
		// 	bits := []babybear.Variable{}
		// 	for i := 0; i < len(cs.Args[2]); i++ {
		// 		bits = append(bits, felts[cs.Args[2][i]])
		// 	}
		// 	exts[cs.Args[0][0]] = fieldAPI.ExtExpReverseBits(exts[cs.Args[1][0]], bits)
		case "ExtDotProd":
			x_vec := []babybear.ExtensionVariable{}
			y_vec := []babybear.ExtensionVariable{}
			for i := 0; i < len(cs.Args[1]); i++ {
				x_vec = append(x_vec, exts[cs.Args[1][i]])
			}
			for i := 0; i < len(cs.Args[2]); i++ {
				y_vec = append(y_vec, exts[cs.Args[2][i]])
			}
			exts[cs.Args[0][0]] = fieldAPI.ExtDotProd(x_vec, y_vec)
		case "ExtDotProdBase":
			x_vec := []babybear.ExtensionVariable{}
			y_vec := []babybear.Variable{}
			for i := 0; i < len(cs.Args[1]); i++ {
				x_vec = append(x_vec, exts[cs.Args[1][i]])
			}
			for i := 0; i < len(cs.Args[2]); i++ {
				y_vec = append(y_vec, felts[cs.Args[2][i]])
			}
			exts[cs.Args[0][0]] = fieldAPI.ExtDotProdBase(x_vec, y_vec)
		case "EqPoly":
			x_vec := []babybear.ExtensionVariable{}
			y_vec := []babybear.ExtensionVariable{}
			for i := 0; i < len(cs.Args[1]); i++ {
			...
			exts[cs.Args[0][0]] = fieldAPI.EqPoly(x_vec, y_vec)
		default:
