...
import (
	...
	"github.com/succinctlabs/dt-recursion-gnark/dt/trusted_setup"
)

// Modify the PlonkVerifier so that it works with the DTVerifier.
...
// Modify the Groth16Verifier so that it works with the DTVerifier.
...
func BuildPlonk(dataDir string) {
	...
	if !strings.Contains(dataDir, "dev") {
		...
		fmt.Println("generate srs")
		if err != nil {
			panic(err)
		}

		srsFile, err := os.Create(srsFileName)
		fmt.Println("create srs file");
		if err != nil {
			panic(err)
		}
		defer srsFile.Close()
		...
		_, err = srsLagrange.WriteTo(srsLagrangeFile)
		fmt.Println("write srs lagrange");
		if err != nil {
			panic(err)
		}
	}
