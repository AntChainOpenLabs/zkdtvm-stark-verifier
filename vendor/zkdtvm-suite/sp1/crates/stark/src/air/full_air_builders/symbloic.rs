...
use crate::air::PolyAirExtendable;
use p3_matrix::dense::{DenseMatrix, RowMajorMatrix};

use super::super::{FullAir, FullAirBuilder, PairCol};
...
impl<F: Field + PolyAirExtendable<D>, const D: usize> SymbolicAirBuilder<F, D> {
    ...
    pub fn generate_precomputed_lc_input(&mut self) {
        let values: Vec<_> =
            (0..self.precomputed_lc_output.len())
    ...
    pub fn get_max_degree(&self) -> usize {
        for (i, precomputed) in self.precomputed_lc_output.iter().enumerate() {
            assert!(precomputed.degree_multiple() == 1, "degree != 1 for precomputed_lc[{i}]")
        }
        ...
        for (i, gate) in self.gate.iter().enumerate() {
            assert!(gate.degree_multiple() > 1, "degree <= 1 for gate[{i}]");
    ...
    pub fn get_num_constraint(&self, batch_size: usize) -> usize {
        self.gate.len() + self.lookup_infos.len().div_ceil(batch_size) + 3
    }

    /// Build a symbolic representation from an AIR.
    #[allow(deprecated)]
    pub fn from_air<A: FullAir<Self>>(air: &A) -> Self {
        let mut builder = Self::new_empty();
        builder.with_preprocessed_width(air.preprocessed_trace().map(|m| m.width).unwrap_or(0));
        builder.with_main_width(air.width());
