...
use super::{interaction::AirInteraction, BinomialExtension, ChallengeExtension};
use crate::{
    air::public_values::{PublicValues, DT_PROOF_NUM_PV_ELTS},
...
/// The fixed width of a bit vector interaction message.
pub const BIT_VEC_WIDTH: usize = 16;

/// A trait which contains methods for bit vector interactions in an AIR.
pub trait BitVecAirBuilder: BaseAirBuilder {
    /// Sends a bit vector decomposition to be processed.
    fn send_bit_vec(
        &mut self,
        bits: &[impl Into<Self::Expr> + Clone],
        multiplicity: impl Into<Self::Expr>,
    ) {
        assert!(bits.len() <= BIT_VEC_WIDTH, "bit vector length must be <= {BIT_VEC_WIDTH}");
        let mut values: Vec<Self::Expr> = bits.iter().map(|b| b.clone().into()).collect();
        values.resize(BIT_VEC_WIDTH, Self::Expr::zero());
        self.send(
            AirInteraction::new(values, multiplicity.into(), InteractionKind::BitVec),
            InteractionScope::Local,
        );
    }

    /// Receives a bit vector decomposition to be processed.
    fn receive_bit_vec(
        &mut self,
        bits: &[impl Into<Self::Expr> + Clone],
        multiplicity: impl Into<Self::Expr>,
...
pub trait InstructionAirBuilder: BaseAirBuilder {
    ...
    fn receive_state(
        ...
        self.receive(
            AirInteraction::new(
                once(shard.into())
                    .chain(once(clk.into()).chain(once(pc.into())))
                    .collect::<Vec<_>>(),
                multiplicity.into(),
                InteractionKind::State,
            ),
...
pub trait ExtensionAirBuilder: BaseAirBuilder {
    ...
    fn assert_ext_eq<I: Into<Self::Expr>>(
        &mut self,
        left: ChallengeExtension<I>,
        right: ChallengeExtension<I>,
    ) {
    ...
    fn if_else_ext(
        ...
        ChallengeExtension(array::from_fn(|i| {
...
/// A trait that contains the common helper methods for building `zkDTVM recursion` and zkDTVM
/// machine AIRs.
...
/// A trait which contains all helper methods for building zkDTVM machine AIRs.
pub trait DTAirBuilder: MachineAirBuilder + ByteAirBuilder + InstructionAirBuilder {
    fn current_shard(&self) -> impl Into<Self::Expr> {
        let public_values_slice: [Self::PublicVar; DT_PROOF_NUM_PV_ELTS] =
            core::array::from_fn(|i| self.public_values()[i]);
        let public_values: &PublicValues<Word<Self::PublicVar>, Self::PublicVar> =
...
impl<AB: BaseAirBuilder> BitVecAirBuilder for AB {}
...
impl<AB: BaseAirBuilder + AirBuilderWithPublicValues> DTAirBuilder for AB {}
