...
impl<T: Clone + Add<Output = T> + Mul<Output = T>> Polynomial<T> {
    pub fn eval_with_powers<S>(&self, powers: &[S], zero_s: S) -> S
    where
        S: Clone + Add<Output = S> + Mul<T, Output = S>,
    {
        let mut result = zero_s;
        for (i, c) in self.coefficients.iter().enumerate() {
            result = result + powers[i].clone() * c.clone();
        }
        result
    }
}
...
impl<T: AbstractField> Polynomial<T> {
    #[must_use]
    pub fn square(self) -> Polynomial<T> {
        let n = self.coefficients.len();
        let mut result = vec![T::zero(); 2 * n - 1];
        for i in 0..n {
            result[2 * i] = self.coefficients[i].clone().square();
        }
        for i in 0..n {
            for j in i + 1..n {
                result[i + j] +=
                    (self.coefficients[i].clone() * self.coefficients[j].clone()).double();
            }
        }
        Polynomial::new(result)
    }
