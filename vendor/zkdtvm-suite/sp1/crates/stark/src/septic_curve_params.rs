...
//! (e.g., `BabyBear` with irreducible polynomial `z^7 - 2z - 5` and `KoalaBear` with `z^7 - 3z -
//! 5`).
...
/// Compute `β^7 − c₁·β − c₀` (the septic polynomial evaluated at β)
/// for the curve `y² = x³ + a·x + b·z^d` where `z` is the reduction
/// variable defined by the irreducible polynomial with coefficients
/// `POLY_COEFFS`.
pub fn compute_beta_septix<
    F: p3_field::Field,
    EF: p3_field::Field + p3_field::ExtensionField<F>,
    P: SepticCurveParams,
>(
    beta: EF,
) -> EF {
    let neg_const = (-P::POLY_COEFFS[0]) as u32;
    let neg_linear = (-P::POLY_COEFFS[1]) as u32;
    beta.exp_u64(7) - EF::from_canonical_u32(neg_linear) * beta - EF::from_canonical_u32(neg_const)
}
...
mod tests {
    ...
    #[test]
    fn test_babybear_params() {
        assert_eq!(BabyBearCurveParams::NAME, "BabyBear");
        assert_eq!(BabyBearCurveParams::CURVE_A, 2);
        assert_eq!(BabyBearCurveParams::CURVE_B_CONST, 26);
        assert_eq!(BabyBearCurveParams::POLY_COEFFS[0], -5);
        assert_eq!(BabyBearCurveParams::POLY_COEFFS[1], -2);
    }

    #[test]
    fn test_koalabear_params() {
        assert_eq!(KoalaBearCurveParams::NAME, "KoalaBear");
        assert_eq!(KoalaBearCurveParams::CURVE_A, 45);
        assert_eq!(KoalaBearCurveParams::CURVE_B_CONST, 41);
        assert_eq!(KoalaBearCurveParams::POLY_COEFFS[0], -5);
        assert_eq!(KoalaBearCurveParams::POLY_COEFFS[1], -3);
    }

    #[test]
    fn test_different_params() {
        // Verify that the two implementations have different parameters
        assert_ne!(
            BabyBearCurveParams::CURVE_A,
            KoalaBearCurveParams::CURVE_A,
            "Curve parameters should differ between implementations"
        );
        assert_ne!(
            BabyBearCurveParams::POLY_COEFFS[1],
            KoalaBearCurveParams::POLY_COEFFS[1],
            "Polynomial coefficients should differ"
        );
        assert_ne!(
            BabyBearCurveParams::CURVE_CUMULATIVE_SUM_START_X,
            KoalaBearCurveParams::CURVE_CUMULATIVE_SUM_START_X,
            "Cumulative sum start points should differ"
        );
    }
}
