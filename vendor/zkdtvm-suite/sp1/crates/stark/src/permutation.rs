use crate::{
    ...
    lookup::{Interaction, InteractionValueEncoding},
...
fn evaluate_encoded_values<F: PrimeField, EF: ExtensionField<F>>(
    interaction: &Interaction<F>,
    preprocessed_row: &[F],
    main_row: &[F],
) -> Vec<EF> {
    let base_values = interaction
        .values
        .iter()
        .map(|columns| columns.apply::<F, F>(preprocessed_row, main_row))
        .collect::<Vec<_>>();
    match interaction.value_encoding {
        InteractionValueEncoding::Base => base_values.into_iter().map(EF::from_base).collect(),
        InteractionValueEncoding::ExtensionBlocks { degree } => {
            assert_eq!(
                EF::D,
                degree,
                "interaction extension-block degree does not match permutation field"
            );
            base_values
                .chunks(degree)
                .map(|chunk| {
                    EF::from_base_fn(|limb| chunk.get(limb).copied().unwrap_or_else(F::zero))
                })
                .collect()
        }
    }
...
fn evaluate_encoded_expressions<'a, AB: MultiTableAirBuilder<'a>>(
    interaction: &Interaction<AB::F>,
    ...
    let base_values = interaction
        .values
        .iter()
        .map(|columns| columns.apply::<AB::Expr, AB::Var>(preprocessed_row, main_row))
        .collect::<Vec<_>>();
    match interaction.value_encoding {
        InteractionValueEncoding::Base => {
            base_values.into_iter().map(AB::ExprEF::from_base).collect()
        }
        InteractionValueEncoding::ExtensionBlocks { degree } => {
            assert_eq!(
                AB::ExprEF::D,
                degree,
                "interaction extension-block degree does not match constraint field"
            );
            base_values
                .chunks(degree)
                .map(|chunk| {
                    AB::ExprEF::from_base_fn(|limb| {
                        chunk.get(limb).cloned().unwrap_or_else(AB::Expr::zero)
                    })
                })
                .collect()
        }
    ...
    for (value, chunk) in row.iter_mut().zip(interaction_chunks) {
        *value = chunk
            ...
            .map(|(interaction, is_send)| {
                ...
                for (value, beta) in
                    evaluate_encoded_values::<F, EF>(interaction, preprocessed_row, main_row)
                        .into_iter()
                        .zip(betas)
                {
                    denominator += beta * value;
                }
...
pub fn generate_compressed_permutation_trace<F: PrimeField, EF: ExtensionField<F>>(
    ...
    // decompress produces a genuinely empty matrix and perm_empty checks (total_height() == 0)
    // work.
    ...
    AB: 'a,
{
    ...
    if !local_sends.is_empty() || !local_receives.is_empty() {
        ...
        for (entry, chunk) in perm_local[0..perm_width].iter().zip(interaction_chunks) {
            ...
            for (interaction, is_send) in chunk {
                ...
                for (element, beta) in
                    evaluate_encoded_expressions::<AB>(interaction, &preprocessed_local, main_local)
                        .into_iter()
                        .zip(betas.clone())
                {
                    rlc = rlc.clone() + beta * element;
                }
                rlcs.push(rlc);
    ...
    // Projective Global boundaries are authenticated by the fixed public claim. No physical
    // main-trace tail is interpreted as a second proof statement.
    let _ = commit_scope;
}
...
///   batch_size)` + 1 (extra alpha power for the cumulative sum constraint computed separately in
///   the sumcheck prover).
/// Global projective boundaries are constrained by the public claim, outside this local count.
...
#[cfg(all(test, feature = "koalabear", feature = "ext5"))]
mod encoding_tests {
    use p3_air::VirtualPairCol;
    use p3_field::{
        extension::QuinticTrinomialExtensionField, AbstractExtensionField, AbstractField,
    };
    use p3_koala_bear::KoalaBear;

    use super::{evaluate_encoded_values, Interaction, InteractionValueEncoding};
    use crate::{air::InteractionScope, InteractionKind};

    type F = KoalaBear;
    type EF = QuinticTrinomialExtensionField<F>;

    #[test]
    fn extension_payload_advances_by_seven_quintic_blocks() {
        let values = (0..34)
            .map(|value| VirtualPairCol::new(vec![], F::from_canonical_usize(value + 1)))
            .collect();
        let interaction = Interaction::new_with_encoding(
            values,
            InteractionValueEncoding::ExtensionBlocks { degree: 5 },
            VirtualPairCol::new(vec![], F::one()),
            InteractionKind::GlobalProjectiveChainV2,
            InteractionScope::Local,
        );
        let encoded = evaluate_encoded_values::<F, EF>(&interaction, &[], &[]);
        assert_eq!(encoded.len(), 7);
        for (block, element) in encoded.iter().enumerate() {
            assert_eq!(
                *element,
                EF::from_base_fn(|limb| {
                    let offset = block * 5 + limb;
                    if offset < 34 {
                        F::from_canonical_usize(offset + 1)
                    } else {
                        F::zero()
                    }
                })
