...
use p3_field::{
    AbstractExtensionField, AbstractField, Field, PrimeField32, PrimeField64, TwoAdicField,
};
...
impl<SC: StarkGenericConfig, A: MachineAir<Val<SC>>> Verifier<SC, A> {
        ...
        A: for<'a> Air<VerifierConstraintFolder<'a, SC>>,
    {
        ...
        let active_shape = derive_active_shape_v1(
            chips
                ...
                .map(|(chip, opening)| (chip.name(), chip.width(), opening.log_degree)),
        )
        .map_err(|_| VerificationError::CumulativeSumsError("invalid active shape"))?;
        ...
        let global_claim = {
            ...
            if owners > 1 {
                return Err(VerificationError::CumulativeSumsError("duplicate Global opening"));
            }
            validate_global_claim(&claim, owners == 1).map_err(|_| {
                VerificationError::CumulativeSumsError("Global claim/opening admission failed")
            })?;
            claim
        };
        ...
        let zeta = challenger.sample_ext_element::<Challenge<SC>>();
        ...
        // Compute the expected local cumulative sum from the dangling State interactions.
        let mut expected_local_sum =
            compute_expected_state_imbalance::<SC>(public_values, &permutation_challenges);
        expected_local_sum += compute_expected_global_claim_imbalance_v2(
            permutation_challenges[0],
            permutation_challenges[1],
            &global_claim,
        )
        .map_err(|_| {
            VerificationError::CumulativeSumsError("invalid projective Global-chain boundary")
        })?;
    ...
    fn verify_opening_shape(
        ...
        opening: &ChipOpenedValues<Val<SC>, Challenge<SC>>,
        ...
        if opening.permutation.local.len() !=
            chip.permutation_width() * <Challenge<SC> as AbstractExtensionField<Val<SC>>>::D
        {
        ...
        for slice in &opening.quotient {
            if slice.len() != <Challenge<SC> as AbstractExtensionField<Val<SC>>>::D {
    ...
    pub fn eval_constraints(
        chip: &MachineChip<SC, A>,
        opening: &ChipOpenedValues<Val<SC>, Challenge<SC>>,
        selectors: &LagrangeSelectors<Challenge<SC>>,
        alpha: Challenge<SC>,
        permutation_challenges: &[Challenge<SC>],
        public_values: &[Val<SC>],
    ) -> Challenge<SC>
        ...
        A: for<'a> Air<VerifierConstraintFolder<'a, SC>>,
    {
        ...
        let unflatten = |v: &[Challenge<SC>]| {
            v.chunks_exact(<Challenge<SC> as AbstractExtensionField<Val<SC>>>::D)
                .map(|chunk| {
                    chunk
                        .iter()
                        .enumerate()
                        .map(|(e_i, &x)| {
                            <Challenge<SC> as AbstractExtensionField<Val<SC>>>::monomial(e_i) * x
                        })
                        .sum()
                })
                .collect::<Vec<Challenge<SC>>>()
        };
    ...
    pub fn recompute_quotient(
        ...
        opening
            ...
            .map(|(ch_i, ch)| {
                // SAFETY: The opening shapes are already verified.
                assert_eq!(ch.len(), <Challenge::<SC> as AbstractExtensionField<Val<SC>>>::D);
                ch.iter()
                    .enumerate()
                    .map(|(e_i, &c)| {
                        zps[ch_i] *
                            <Challenge<SC> as AbstractExtensionField<Val<SC>>>::monomial(e_i) *
                            c
                    })
                    .sum::<Challenge<SC>>()
            })
