...
use crate::{
    air::{DTAirBuilder, InteractionScope, MachineAir, MultiTableAirBuilder},
...
pub struct Chip<F: Field, A> {
    ...
    /// The maximum constraint degree of the chip.
    pub max_constraint_degree: usize,
    /// The relative log degree of the quotient polynomial, i.e. `log2(max_constraint_degree - 1)`.
    pub log_quotient_degree: usize,
}

impl<F: Field, A: Copy> Chip<F, A> {
    /// Converts the chip to an extension field chip.
    pub fn field_ext<EF: ExtensionField<F>>(&self) -> Chip<EF, A> {
        Chip {
            air: self.air,
            sends: self.sends.iter().map(Interaction::field_ext).collect(),
            receives: self.receives.iter().map(Interaction::field_ext).collect(),
    ...
    A: BaseAir<F>,
{
        ...
        A: MachineAir<F> + Air<InteractionBuilder<F>> + Air<SymbolicAirBuilder<F>>,
    {
        ...
        let mut max_constraint_degree =
            get_max_constraint_degree_sc(&air, air.preprocessed_width(), PROOF_MAX_NUM_PVS);
        ...
        Self { air, sends, receives, max_constraint_degree, log_quotient_degree }
    }
    ...
    /// Returns the cost of a row in the chip for shard-splitting budget.
    ///
    /// Only counts `preprocessed + main` columns. Permutation and quotient columns are
    /// excluded following the same convention as zkDTVM Hypercube: the permutation trace
    /// can be computed on-the-fly during sumcheck without full materialization, so it
    /// should not count against the per-shard cell budget.
    #[inline]
    ...
    /// Returns the batch size for sumcheck-based proving system.
    #[inline]
    pub fn logup_batch_size(&self) -> usize {
        self.max_constraint_degree - 1
    }
}
    ...
    A: MachineAir<F>,
{
    ...
    fn global_boundary_owner(&self) -> Option<crate::global_d11::StableChipId> {
    ...
    ) -> Result<Option<crate::air::GlobalClaim<F>>, String> {
...
where
    ...
    AB: DTAirBuilder<F = F> + MultiTableAirBuilder<'a> + PairBuilder + 'a,
{
    ...
    // fn eval_sc(&self, builder: &mut AB) {
    //     // Evaluate the execution trace constraints.
    //     self.air.eval(builder);
    //     // Evaluate permutation constraints.
    //     let batch_size = self.logup_batch_size();
