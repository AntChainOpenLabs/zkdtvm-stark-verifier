...
pub fn debug_constraints<SC, A>(
    ...
    perm: &RowMajorMatrix<Challenge<SC>>,
    perm_challenges: &[Challenge<SC>],
    public_values: &[Val<SC>],
    local_cumulative_sum: &Challenge<SC>,
) where
    SC: StarkGenericConfig,
    Val<SC>: PrimeField32,
    A: MachineAir<Val<SC>> + for<'a> Air<DebugConstraintBuilder<'a, Val<SC>, Challenge<SC>>>,
{
    if perm.width > 0 {
