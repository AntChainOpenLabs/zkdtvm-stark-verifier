...
pub fn compute_powers_of_alpha<F: Field>(alpha: F, num_constraints: Vec<usize>) -> Vec<Vec<F>> {
...
pub mod selectors {
    ...
    ///
    /// Returns the active extension's packing (`EF::ExtensionPacking`), generic over
    /// the extension degree, so it works for both quartic and quintic challenges.
    pub fn get_first_sel_ext_packed<F: Field, EF: ExtensionField<F>>(
        offset: usize,
        first_value: EF,
        _total_height: usize,
        _stored_height: usize,
        ...
        if offset == 0 {
            return EF::ExtensionPacking::from_base_fn(|coeff_idx| {
    ...
    /// Returns the active extension's packing (`EF::ExtensionPacking`), generic over
    /// the extension degree, so it works for both quartic and quintic challenges.
    pub fn get_last_sel_ext_packed<F: Field, EF: ExtensionField<F>>(
        offset: usize,
        last_value: EF,
        total_height: usize,
        stored_height: usize,
        ...
        if stored_height < total_height {
            return EF::ExtensionPacking::zero();
        }
        // stored_height == total_height
        if total_height <= F::Packing::WIDTH {
            debug_assert_eq!(offset, 0);
            EF::ExtensionPacking::from_base_fn(|coeff_idx| {
                F::Packing::from_fn(|idx_in_packing| {
