...
use crate::{
    ...
    MachineChip, SCStarkMachine, StarkMachine, StarkProvingKey,
};

use crate::sumcheck::{keys::SCStarkProvingKey, trace::CompressedMatrix};
...
pub fn debug_interactions<SC: StarkGenericConfig, A: MachineAir<Val<SC>>>(
    ...
    let mut main = compressed.decompress();
...
#[allow(clippy::type_complexity)]
#[allow(clippy::needless_pass_by_value)]
pub fn debug_interactions_sumcheck<SC: SCStarkGenericConfig, A: MachineAir<Val<SC>>>(
    chip: &MachineChip<SC, A>,
    pkey: &SCStarkProvingKey<SC>,
    record: &A::Record,
    interaction_kinds: Vec<InteractionKind>,
    scope: InteractionScope,
) -> (BTreeMap<String, Vec<InteractionData<Val<SC>>>>, BTreeMap<String, Val<SC>>) {
    ...
    let mut main = compressed.decompress();
    let height = main.height();
    let mut decompressed_traces: Vec<_> =
        pkey.traces.iter().map(CompressedMatrix::decompress).collect();
    let mut preprocessed_trace =
        pkey.chip_ordering.get(&chip.name()).map(|&index| &mut decompressed_traces[index]);
...
/// and print out the ones for which the set of sends and receives don't match.
#[allow(clippy::needless_pass_by_value)]
pub fn debug_interactions_with_all_chips_sumcheck<SC, A, AE>(
    ...
    AE: MachineAir<Challenge<SC>>,
{
    ...
    for chip in chips.iter() {
        ...
        for shard in shards {
            ...
            eprintln!("{}", chip.name());
            let (_, count) = debug_interactions_sumcheck::<SC, A>(
                chip,
                pkey,
                shard,
