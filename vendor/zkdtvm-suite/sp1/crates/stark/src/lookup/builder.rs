...
impl<F: Field> MessageBuilder<AirInteraction<SymbolicExpression<F>>> for InteractionBuilder<F> {
    fn send(&mut self, message: AirInteraction<SymbolicExpression<F>>, scope: InteractionScope) {
        let value_encoding = message.value_encoding;
        let values =
        ...
        self.sends.push(Interaction::new_with_encoding(
            values,
            value_encoding,
            multiplicity,
            message.kind,
    ...
    fn receive(&mut self, message: AirInteraction<SymbolicExpression<F>>, scope: InteractionScope) {
        ...
        self.receives.push(Interaction::new_with_encoding(
            values,
            value_encoding,
            multiplicity,
            message.kind,
...
mod tests {
    ...
    use super::*;
    use crate::{air::DTAirBuilder, lookup::InteractionKind};
    ...
    impl<AB: DTAirBuilder> Air<AB> for LookupTestAir {
