...
pub(crate) fn create_local_command(
    ...
    // downloaded by 'dtup --c-toolchain'.
    if env::var("CC_riscv32im_succinct_zkvm_elf").is_err() {
        if let Some(home_dir) = home_dir() {
            let cc_path = home_dir.join(".dt").join("bin").join("riscv32-unknown-elf-gcc");
            if cc_path.exists() {
    ...
    // Also remove CARGO_PROFILE_* env vars to prevent the host cargo's profile
    // settings (e.g. trim-paths) from leaking into the child cargo process,
    // which may not support those unstable features.
    env::vars()
        .map(|v| v.0)
        .filter(|v| {
            v.starts_with("CARGO_FEATURE_") ||
                v.starts_with("CARGO_CFG_") ||
                v.starts_with("CARGO_PROFILE_")
        })
