...
/// Uses DT_DOCKER_IMAGE environment variable if set, otherwise constructs the image to use based
/// on the provided tag.
fn get_docker_image(tag: &str) -> String {
    std::env::var("DT_DOCKER_IMAGE").unwrap_or_else(|_| {
...
pub(crate) fn create_docker_command(
    ...
    // Add the zkDTVM program build arguments.
