...
use dt_prover::{components::SCCpuProverComponents, types::HashableKey, DTProver};
...
/// Build arguments for guest zkVM programs so the **scalar field** matches the host prover.
///
/// When the script crate is built with a `babybear` feature (e.g. `babybear_whir` with
/// `--no-default-features`), Cargo sets `CARGO_FEATURE_BABYBEAR` for this `build.rs`. Guest crates
/// should define `guest_babybear` / `guest_koalabear` features (see example `*-program`
/// `Cargo.toml` files) and pass the returned args to [`build_program_with_args`].
pub fn guest_program_build_args() -> BuildArgs {
    if std::env::var("CARGO_FEATURE_BABYBEAR").is_ok() {
        BuildArgs {
            no_default_features: true,
            features: vec!["guest_babybear".to_string()],
            ..Default::default()
        }
    } else {
        BuildArgs::default()
...
///
/// Set the `DT_SKIP_PROGRAM_BUILD` environment variable to `true` to skip building the program.
...
///
/// Note that this only works when using `dt_build::build_program` or
/// `dt_build::build_program_with_args` in a build script.
///
...
macro_rules! include_elf {
    ($arg:tt) => {{
        include_bytes!(env!(concat!("DT_ELF_", $arg)))
    }};
}
