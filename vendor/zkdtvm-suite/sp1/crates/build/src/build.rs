...
pub(crate) fn build_program_internal(path: &str, args: Option<BuildArgs>) {
    ...
    // Skip the program build if the DT_SKIP_PROGRAM_BUILD environment variable is set to true.
    let skip_program_build = std::env::var("DT_SKIP_PROGRAM_BUILD")
    ...
    if skip_program_build {
        ...
        println!(
            "cargo:warning=Build skipped for {} at {} due to DT_SKIP_PROGRAM_BUILD flag",
            root_package_name,
            current_datetime()
        );
        return;
    ...
    if let Err(err) = path_output {
        panic!("Failed to build zkDTVM program: {}.", err);
    }
...
/// Prints cargo directives setting relevant `DT_ELF_` environment variables.
fn print_elf_paths_cargo_directives(target_elf_paths: &[(String, Utf8PathBuf)]) {
    for (target_name, elf_path) in target_elf_paths.iter() {
        println!("cargo:rustc-env=DT_ELF_{}={}", target_name, elf_path);
    }
}
