...
use dt_primitives::consts::fd::LOWEST_ALLOWED_FD;

/// Context to run a program inside zkDTVM.
#[derive(Clone)]
pub struct DTContext<'a> {
    /// The registry of hooks invocable from inside zkDTVM.
    ///
    ...
    /// The IO options for the [`DTExecutor`].
    pub io_options: IoOptions<'a>,
}

impl Default for DTContext<'_> {
    fn default() -> Self {
        Self::builder().build()
    }
}

/// A builder for [`DTContext`].
pub struct DTContextBuilder<'a> {
...
impl Default for DTContextBuilder<'_> {
    fn default() -> Self {
        Self {
...
impl<'a> DTContext<'a> {
    /// Create a new context builder. See [`DTContextBuilder`] for more details.
    #[must_use]
    pub fn builder() -> DTContextBuilder<'a> {
        DTContextBuilder::new()
    }
}

impl<'a> DTContextBuilder<'a> {
    /// Create a new [`DTContextBuilder`].
    ///
    /// Prefer using [`DTContext::builder`].
    #[must_use]
    pub fn new() -> Self {
        DTContextBuilder::default()
    }

    /// Build and return the [`DTContext`].
    ///
    /// Clears and resets the builder, allowing it to be reused.
    pub fn build(&mut self) -> DTContext<'a> {
    ...
    ///
    /// Hooks may be invoked from within zkDTVM by writing to the specified file descriptor `fd`
    /// with [`dt_zkvm::io::write`], returning a list of arbitrary data that may be read
    /// with successive calls to [`dt_zkvm::io::read`].
    ///
    /// # Panics
    ...
    ///
    /// The verifier is used to sanity check `verify_dt_proof` during runtime.
...
/// The IO options for the [`DTExecutor`].
///
/// This struct is used to redirect the `stdout` and `stderr` of the [`DTExecutor`].
#[derive(Default)]
...
#[cfg(test)]
mod tests {
    use crate::{subproof::NoOpSubproofVerifier, DTContext};

    #[test]
    fn defaults() {
        let DTContext { hook_registry, subproof_verifier, max_cycles: cycle_limit, .. } =
            DTContext::builder().build();
    ...
    fn without_default_hooks() {
        let DTContext { hook_registry, .. } = DTContext::builder().without_default_hooks().build();
        assert!(hook_registry.unwrap().table.is_empty());
    }

    #[test]
    fn with_custom_hook() {
        let DTContext { hook_registry, .. } = DTContext::builder().hook(30, |_, _| vec![]).build();
        assert!(hook_registry.unwrap().table.contains_key(&30));
    }

    #[test]
    fn without_default_hooks_with_custom_hook() {
        let DTContext { hook_registry, .. } =
            DTContext::builder().without_default_hooks().hook(30, |_, _| vec![]).build();
    ...
    fn subproof_verifier() {
        ...
        let DTContext { subproof_verifier, .. } =
            DTContext::builder().subproof_verifier(&verifier).build();
