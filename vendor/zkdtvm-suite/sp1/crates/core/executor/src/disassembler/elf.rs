...
use dt_primitives::consts::{MAXIMUM_MEMORY_SIZE, SC_PRIME, WORD_SIZE};
use elf::{
    ...
    pub(crate) fn decode(input: &[u8]) -> eyre::Result<Self> {
        ...
        for segment in segments.iter().filter(|x| x.p_type == PT_LOAD) {
            ...
            if (segment.p_flags & PF_X) != 0 {
                if base_address.is_none() {
                    ...
                    let instr_len: u32 = WORD_SIZE
                        ...
                        .try_into()?;
                    let last_instruction_addr =
                        vaddr.checked_sub(instr_len).ok_or_eyre("instruction addr overflow")?;
                    eyre::ensure!(
                        vaddr == last_instruction_addr + instr_len,
                        "unsupported elf structure"
                    );
                }
            }
            ...
            for i in (0..mem_size).step_by(WORD_SIZE) {
                ...
                if addr >= SC_PRIME {
                    eyre::bail!(
                        "address [0x{addr:08x}] for guest programs must be less than [0x{SC_PRIME:08x}]"
                    );
                }
