...
pub use dt_primitives::consts::fd::*;
...
pub fn hook_ed_decompress(_: HookEnv, buf: &[u8]) -> Vec<Vec<u8>> {
    ...
    let modulus = dt_curves::edwards::ed25519::Ed25519BaseField::modulus();
...
mod bls {
    ...
    use dt_curves::{params::FieldParameters, weierstrass::bls12_381::Bls12381BaseField, Zero};
...
pub(crate) mod deprecated_hooks {
    ...
    #[must_use]
    pub fn hook_ed_decompress(_: HookEnv, buf: &[u8]) -> Vec<Vec<u8>> {
        let Ok(point) = dt_curves::curve25519_dalek::CompressedEdwardsY::from_slice(buf) else {
            return vec![vec![0]];
        };

        if dt_curves::edwards::ed25519::decompress(&point).is_some() {
            vec![vec![1]]
        } else {
            vec![vec![0]]
        }
