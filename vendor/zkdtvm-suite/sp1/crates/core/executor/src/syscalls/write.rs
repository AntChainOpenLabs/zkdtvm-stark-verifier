...
impl Syscall for WriteSyscall {
    ...
    fn execute(
        ...
        if fd == 1 {
            ...
            if std::env::var("DT_ALLOW_DEPRECATED_HOOKS")
                .map(|r| {
                    r.parse::<bool>().expect("failed to parse DT_ALLOW_DEPRECATED_HOOKS as bool")
                })
                .unwrap_or(true)
            {
                ...
                panic!(
                    "You are using reserved file descriptor {fd} that is not supported on zkDTVM versions >= v4.0.0. \
