...
pub use poseidon2_permute::Poseidon2PermuteEvent;
...
impl PrecompileEvents {
    ...
    #[must_use]
    pub fn is_precompile_empty(&self) -> bool {
        self.events.iter().all(|(syscall_code, events)| {
            matches!(
                syscall_code,
                SyscallCode::SHA_EXTEND | SyscallCode::SHA_COMPRESS | SyscallCode::KECCAK_PERMUTE
            ) || events.is_empty()
        })
    }

    #[must_use]
    pub fn is_sha_extend_empty(&self) -> bool {
        self.events.get(&SyscallCode::SHA_EXTEND).is_none_or(std::vec::Vec::is_empty)
    }

    #[must_use]
    pub fn is_sha_compress_empty(&self) -> bool {
        self.events.get(&SyscallCode::SHA_COMPRESS).is_none_or(std::vec::Vec::is_empty)
    }

    #[must_use]
    pub fn is_keccak_empty(&self) -> bool {
        self.events.get(&SyscallCode::KECCAK_PERMUTE).is_none_or(std::vec::Vec::is_empty)
    }
    ...
    pub fn all_precompile_events(&self) -> impl Iterator<Item = &(SyscallEvent, PrecompileEvent)> {
        self.events
            .iter()
    ...
    pub fn sha_extend_events(&self) -> impl Iterator<Item = &ShaExtendEvent> {
        self.events
            .get(&SyscallCode::SHA_EXTEND)
            .map(|events| events.iter())
            .unwrap_or_default()
            .map(|(_, event)| {
                let PrecompileEvent::ShaExtend(event) = event else {
                    unreachable!();
                };
                event
            })
    ...
    pub fn sha_compress_events(&self) -> impl Iterator<Item = &ShaCompressEvent> {
        self.events
            .get(&SyscallCode::SHA_COMPRESS)
            .map(|events| events.iter())
            .unwrap_or_default()
            .map(|(_, event)| {
                let PrecompileEvent::ShaCompress(event) = event else {
                    unreachable!();
                };
                event
            })
    ...
    pub fn keccak_events(&self) -> impl Iterator<Item = &KeccakPermuteEvent> {
        self.events
            .get(&SyscallCode::KECCAK_PERMUTE)
            .map(|events| events.iter())
            .unwrap_or_default()
            .map(|(_, event)| {
                let PrecompileEvent::KeccakPermute(event) = event else {
                    unreachable!();
                };
                event
            })
