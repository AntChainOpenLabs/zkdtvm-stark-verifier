...
pub trait ByteRecord {
    ...
    /// Adds bit range lookup
    fn add_bit_range_check(&mut self, value: u16, bit_width: u8) {
        self.add_byte_lookup_event(ByteLookupEvent {
            opcode: ByteOpcode::BitRange,
            a1: value,
            a2: bit_width,
            b: 0,
            c: 0,
    ...
    /// Adds a BitVec lookup event. Low byte → b, high byte → c.
    fn add_bit_vec_lookup(&mut self, value: u16) {
        let b = (value & 0xFF) as u8;
        let c = ((value >> 8) & 0xFF) as u8;
        self.add_byte_lookup_event(ByteLookupEvent {
            opcode: ByteOpcode::BitVec,
            a1: 0,
            a2: 0,
            b,
            c,
    ...
    /// bytes.
    fn add_u8_range_checks_field<F: Field>(&mut self, field_values: &[F]) {
        self.add_u8_range_checks(
            &field_values.iter().map(|x| x.as_u32() as u8).collect::<Vec<_>>(),
        );
    }
    ...
    /// Adds `ByteLookupEvent`s to verify that all the field elements in the input slice are indeed
    /// u16 values.
    fn add_u16_range_checks_field<F: Field>(&mut self, field_values: &[F]) {
        self.add_u16_range_checks(
            &field_values.iter().map(|x| x.as_u32() as u16).collect::<Vec<_>>(),
        );
    }

    /// Adds a `ByteLookupEvent` to compute the bitwise AND of the two input values.
    fn lookup_and(&mut self, b: u8, c: u8) {
        self.add_byte_lookup_event(ByteLookupEvent {
    ...
    /// Adds a `ByteLookupEvent` to compute the bitwise XOR of the two input values.
    fn lookup_xor(&mut self, b: u8, c: u8) {
        self.add_byte_lookup_event(ByteLookupEvent {
