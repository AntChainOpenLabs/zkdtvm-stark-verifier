...
impl Default for MemoryRecordEnum {
    fn default() -> Self {
        Self::Read(MemoryReadRecord::default())
    }
}
...
impl MemoryRecordEnum {
    ...
    ///downcast to memory read record
    #[must_use]
    pub fn read_record(self) -> Option<MemoryReadRecord> {
        if let MemoryRecordEnum::Read(record) = self {
            Some(record)
        } else {
            None
        }
    }
    ///downcast to memory write record
    #[must_use]
    pub fn write_record(self) -> Option<MemoryWriteRecord> {
        if let MemoryRecordEnum::Write(record) = self {
            Some(record)
        } else {
            None
        }
