use crate::{
    events::{AUIPCEvent, AluEvent, BranchEvent, CpuEvent, JumpEvent, MemInstrEvent, MemoryRecord},
...
pub fn emit_divrem_dependencies(executor: &mut Executor, event: AluEvent) {
    ...
    if c_neg == 1 {
        executor.record.add_events.push((CpuEvent::default(), AluEvent {
            pc: UNUSED_PC,
            opcode: Opcode::ADD,
            a: 0,
            b: event.c,
    ...
    executor.record.mul_events.push((CpuEvent::default(), lower_multiplication));
    ...
    executor.record.mul_events.push((CpuEvent::default(), upper_multiplication));
    ...
    if event.c != 0 {
        executor.record.lt_events.push((CpuEvent::default(), lt_event));
    }
}
...
pub fn emit_memory_dependencies(
    ...
    executor.record.add_events.push((CpuEvent::default(), add_event));
    ...
    if matches!(event.opcode, Opcode::LB | Opcode::LH) {
        ...
        if (most_sig_mem_value_byte >> 7) & 0x01 == 1 {
            ...
            executor.record.add_events.push((CpuEvent::default(), sub_event));
        }
    }
}
...
pub fn emit_branch_dependencies(executor: &mut Executor, event: BranchEvent) {
    ...
    executor.record.lt_events.push((CpuEvent::default(), lt_comp_event));
    executor.record.lt_events.push((CpuEvent::default(), gt_comp_event));
