...
///
/// While the zkDTVM targets the RISC-V ISA, it uses a custom instruction encoding that uses
/// a different set of opcodes. The main difference is that the zkDTVM encodes register
/// operations and immediate operations as the same opcode. For example, the RISC-V opcodes ADD and
/// ADDI both become ADD inside the zkDTVM. We utilize flags inside the instruction itself to
...
pub enum ByteOpcode {
    ...
    MSB = 7,
    /// range for 0..=15bit range check,
    BitRange = 8,
    /// Unsigned 16-bit Range Check.
    U16Range = 9,
    /// Bit vector decomposition for PolyAir.
    BitVec = 10,
}
