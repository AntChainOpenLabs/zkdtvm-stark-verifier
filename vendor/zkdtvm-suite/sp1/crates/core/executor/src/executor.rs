...
use clap::ValueEnum;
use dt_primitives::consts::SC_PRIME;
use dt_stark::{air::PublicValues, DTCoreOpts};
...
use crate::{
    ...
    events::{
        AUIPCEvent, AluEvent, BranchEvent, JumpEvent, MemInstrEvent, MemoryAccessPosition,
    ...
    memory::{Entry, Memory},
    pad_rv32im_event_counts_no_dependencies,
    record::{
        ALUTypeRecord, AddiRecord, BTypeRecord, ExecutionRecord, ITypeRecord, JTypeRecord,
        MemoryAccessRecord, RTypeRecord,
    },
...
/// An executor for the zkDTVM RISC-V zkVM.
///
...
pub struct Executor<'a> {
    ...
    /// Verifier used to sanity check `verify_dt_proof` during runtime.
...
pub struct LocalCounts {
    ...
    pub local_mem: usize,
    /// Separate counter for ADDI instructions (`Opcode::ADD` with `imm_c=true`).
    /// Needed because ADD and ADDI share the same opcode but go to different chips.
    pub addi_count: u64,
}
...
impl<'a> Executor<'a> {
    ...
    #[must_use]
    pub fn new(program: Program, opts: DTCoreOpts) -> Self {
        Self::with_context(program, opts, DTContext::default())
    }
    ...
    pub fn maybe_setup_profiler(&mut self, elf_bytes: &[u8]) {
        #[cfg(feature = "profiling")]
        {
            ...
            if let Some(trace_buf) = trace_buf {
                tracing::debug!("Profiling enabled");

                let sample_rate = std::env::var("TRACE_SAMPLE_RATE")
                    .ok()
                    .and_then(|rate| {
                        tracing::debug!("Profiling sample rate: {rate}");
                        rate.parse::<u32>().ok()
                    })
                    .unwrap_or(1);
    ...
    #[must_use]
    pub fn with_context(program: Program, opts: DTCoreOpts, context: DTContext<'a>) -> Self {
    ...
    #[must_use]
    pub fn recover(program: Program, state: ExecutionState, opts: DTCoreOpts) -> Self {
    ...
    pub fn mr(
        ...
        if !addr.is_multiple_of(4) || addr <= Register::X31 as u32 || addr >= SC_PRIME {
    ...
    fn emit_events(
        ...
        // Increment the CPU event counter and track start_pc / last_next_pc / last_exit_code.
        if self.record.cpu_events == 0 {
            self.record.start_pc = self.state.pc;
        }
        self.record.cpu_events += 1;
        self.record.last_next_pc = next_pc;
        self.record.last_exit_code = exit_code;

        if instruction.is_alu_instruction() {
            self.emit_alu_event(clk, instruction, a, b, c, op_a_0, &record);
        } else if instruction.is_memory_load_instruction() ||
            instruction.is_memory_store_instruction()
        {
            self.emit_mem_instr_event(clk, instruction, a, b, c, op_a_0, &record);
        } else if instruction.is_branch_instruction() {
            self.emit_branch_event(clk, instruction, a, b, c, op_a_0, next_pc, &record);
        } else if instruction.is_jump_instruction() {
            self.emit_jump_event(clk, instruction, a, b, c, op_a_0, next_pc, &record);
        } else if instruction.is_auipc_instruction() {
            self.emit_auipc_event(clk, instruction, a, b, c, op_a_0, &record);
        } else if instruction.is_ecall_instruction() {
            self.emit_syscall_event(clk, instruction, op_a_0, syscall_code, b, c, next_pc, &record);
        } else {
            unreachable!()
        }
    }
        ...
        match opcode {
            Opcode::ADD => {
                if instruction.imm_c {
                    // ADDI or LUI-as-ADD: b may be register or immediate
                    let rec = AddiRecord::new(clk, record, instruction);
                    self.record.addi_events.push((rec, event));
                } else {
                    // ADD: R-type (a=write, b=read, c=read)
                    let rec = RTypeRecord::new(clk, record, instruction);
                    self.record.add_events.push((rec, event));
                }
            }
            Opcode::SUB => {
                let rec = RTypeRecord::new(clk, record, instruction);
                self.record.sub_events.push((rec, event));
            }
            Opcode::XOR | Opcode::OR | Opcode::AND => {
                let rec = ALUTypeRecord::new(clk, record, instruction);
                self.record.bitwise_events.push((rec, event));
            }
            Opcode::SLL => {
                let rec = ALUTypeRecord::new(clk, record, instruction);
                self.record.shift_left_events.push((rec, event));
            }
            Opcode::SRL | Opcode::SRA => {
                let rec = ALUTypeRecord::new(clk, record, instruction);
                self.record.shift_right_events.push((rec, event));
            }
            Opcode::SLT | Opcode::SLTU => {
                let rec = ALUTypeRecord::new(clk, record, instruction);
                self.record.lt_events.push((rec, event));
            }
            Opcode::MUL | Opcode::MULHU | Opcode::MULHSU | Opcode::MULH => {
                let rec = RTypeRecord::new(clk, record, instruction);
                self.record.mul_events.push((rec, event));
            }
            Opcode::DIVU | Opcode::REMU | Opcode::DIV | Opcode::REM => {
                let rec = RTypeRecord::new(clk, record, instruction);
                self.record.divrem_events.push((rec, event));
            }
            _ => unreachable!(),
        }
    }
    ...
    fn emit_mem_instr_event(
        ...
        match opcode {
            // Load instructions: I-type (a=write, b=read, c=immediate)
            Opcode::LB | Opcode::LBU => {
                let rec = ITypeRecord::new(clk, record, instruction);
                self.record.load_byte_events.push((rec, event));
            }
            Opcode::LH | Opcode::LHU => {
                let rec = ITypeRecord::new(clk, record, instruction);
                self.record.load_half_events.push((rec, event));
            }
            Opcode::LW => {
                let rec = ITypeRecord::new(clk, record, instruction);
                self.record.load_word_events.push((rec, event));
            }
            // Store instructions: B-type (a=read, b=read, c=immediate)
            Opcode::SB => {
                let rec = BTypeRecord::new(clk, record, instruction);
                self.record.store_byte_events.push((rec, event));
            }
            Opcode::SH => {
                let rec = BTypeRecord::new(clk, record, instruction);
                self.record.store_half_events.push((rec, event));
            }
            Opcode::SW => {
                let rec = BTypeRecord::new(clk, record, instruction);
                self.record.store_word_events.push((rec, event));
            }
            _ => unreachable!(),
        }
    }
    ...
    fn emit_branch_event(
        ...
        let rec = BTypeRecord::new(clk, record, instruction);
        self.record.branch_events.push((rec, event));
    }
    ...
    fn emit_jump_event(
        ...
        match opcode {
            Opcode::JAL => {
                let rec = JTypeRecord::new(clk, record, instruction);
                self.record.jal_events.push((rec, event));
            }
            Opcode::JALR => {
                let rec = ITypeRecord::new(clk, record, instruction);
                self.record.jalr_events.push((rec, event));
            }
            _ => unreachable!(),
        }
    }
    ...
    fn emit_auipc_event(
        ...
        let rec = JTypeRecord::new(clk, record, instruction);
        self.record.auipc_events.push((rec, event));
    }
    ...
    fn emit_syscall_event(
        ...
        let syscall_event =
            self.syscall_event(clk, record.a, Some(op_a_0), syscall_code, arg1, arg2, next_pc);

        let rec = RTypeRecord::new(clk, record, instruction);
        self.record.syscall_events.push((rec, syscall_event));
    }
    ...
    fn execute_instruction(&mut self, instruction: &Instruction) -> Result<(), ExecutionError> {
        ...
        if !self.unconstrained {
            ...
            if instruction.opcode == Opcode::ADD && instruction.imm_c {
                self.local_counts.addi_count += 1;
            }
    ...
    fn execute_ecall(
        ...
        *syscall_count += 1;
        ...
        {
            let threshold = syscall.precompile_shard_limit() as u64;
    ...
    fn execute_cycle(&mut self) -> Result<bool, ExecutionError> {
        ...
        if !self.unconstrained {
            // Hard limit: if the cycle count reaches shard_size, force a split.
            let cpu_exit = self.max_syscall_cycles + self.state.clk >= self.shard_size;

            let height_threshold = self.opts.sharding_threshold.height_threshold;
            let element_threshold = self.opts.sharding_threshold.element_threshold;

            // Every N cycles, use estimate_riscv_cost to check if the shard should be split.
            let mut cost_exceeded = false;
            let mut split_reason = "";
            if self.state.global_clk.is_multiple_of(self.shape_check_frequency) {
                // Sync local_mem from the record estimator to exclude precompile memory
                // accesses, which are counted in precompile shards, not core shards.
                if let Some(estimator) = &self.record_estimator {
                ...
                let global_raw = self.event_counts[RiscvAirId::Global];
                let global_bound = height_threshold * 9 / 10;
                if global_raw >= global_bound {
                    cost_exceeded = true;
                    split_reason = "global_bound";
                }

                // Pad event counts to account for worst-case growth over the next check interval.
                let padded_event_counts = pad_rv32im_event_counts_no_dependencies(
                    self.event_counts,
                    self.shape_check_frequency,
                );

                // Estimate cost using the precise program size.
                let program_size = self.program.instructions.len() as u32;
                let cost_estimate =
                    estimate_riscv_cost(padded_event_counts, &self.costs, program_size);

                // Split the shard if max_height or total_cells exceeds the threshold.
                if cost_estimate.max_height > height_threshold {
                    tracing::debug!(
                        "Stopping shard {} because estimated max_height {} exceeds threshold {}. clk = {} pc = 0x{:x?}",
                        self.shard(),
                        cost_estimate.max_height,
                        height_threshold,
                        self.state.global_clk,
                        self.state.pc,
                    );
                    cost_exceeded = true;
                    split_reason = "max_height";
                } else if cost_estimate.total_cells > element_threshold {
                    tracing::debug!(
                        "Stopping shard {} because estimated total_cells {} exceeds threshold {}. clk = {} pc = 0x{:x?}",
                        self.shard(),
                        cost_estimate.total_cells,
                        element_threshold,
                        self.state.global_clk,
                        self.state.pc,
                    );
                    cost_exceeded = true;
                    split_reason = "total_cells";
                }
            }

            if cpu_exit || cost_exceeded {
                let instr_count = self.state.clk >> 2;
                let unique_mem_addrs = self.local_memory_access.len();
                let reason = if cpu_exit { "hard_cap" } else { split_reason };
                ...
                let program_size = self.program.instructions.len() as u32;
                let final_cost = estimate_riscv_cost(self.event_counts, &self.costs, program_size);

                tracing::info!(
                    "SHARD_SPLIT: shard={} instructions={} unique_addrs={} reason={} max_height={} total_cells={} height_thresh={} cells_thresh={}",
                    self.shard(),
                    instr_count,
                    unique_mem_addrs,
                    reason,
                    final_cost.max_height,
                    final_cost.total_cells,
                    height_threshold,
                    element_threshold,
                );

                let mut chip_counts: Vec<_> = self
                    .event_counts
                    .iter()
                    .filter(|(_, v)| **v > 0)
                    .map(|(k, v)| (format!("{k:?}"), *v, v.next_power_of_two()))
                    .collect();
                chip_counts.sort_by(|a, b| b.2.cmp(&a.2));
                for (name, count, padded) in chip_counts.iter().take(10) {
                    tracing::info!("  chip {:20} events={:>8} padded={:>8}", name, count, padded);
                }
                self.bump_record();
    ...
    pub fn execute_state(
        ...
        let proof_stream_sc = std::mem::take(&mut self.state.proof_stream_sc);
        ...
        self.state.proof_stream_sc = proof_stream_sc;
    ...
    pub fn execute(&mut self) -> Result<bool, ExecutionError> {
        ...
        for (i, record) in self.records.iter_mut().enumerate() {
            ...
            if record.cpu_events == 0 {
                ...
                // Non-execution shards have no State interactions.
                record.public_values.start_clk = 0;
                record.public_values.exit_clk = 0;
            } else {
                record.public_values.start_pc = record.start_pc;
                record.public_values.next_pc = record.last_next_pc;
                record.public_values.exit_code = record.last_exit_code;
                // Execution shards: clk starts at 0, each instruction adds 4 + num_extra_cycles.
                // For most instructions, num_extra_cycles = 0. For precompile ECALLs,
                // num_extra_cycles > 0 (e.g., SHA_EXTEND = 48).
                let extra_cycles: u32 =
                    record.syscall_events.iter().map(|(_, e)| e.syscall_code.num_cycles()).sum();
                record.public_values.start_clk = 0;
                record.public_values.exit_clk = 4 * record.cpu_events + extra_cycles;
    ...
    fn postprocess(&mut self) {
        ...
        for (fd, buf) in &self.io_buf {
            if !buf.is_empty() {
                match fd {
                    1 => {
                        tracing::debug!("stdout: {buf}");
                    }
                    2 => {
                        tracing::debug!("stderr: {buf}");
                    }
                    _ => {}
                }
            }
        ...
        if self.state.proof_stream_ptr != self.state.proof_stream_sc.len() {
            tracing::warn!(
                "Not all proofs were read. Proving will fail during recursion. Did you pass too
            many proofs in or forget to call verify_dt_proof?"
            );
        }
                ...
                self.executor_mode == ExecutorMode::Checkpoint)
        {
            ...
            for addr in 1..32 {
                let Some(record) = self.state.memory.registers.get(addr) else {
                    continue;
                };
                if self.print_report {
    ...
    fn estimate_riscv_event_counts(
        ...
        // Compute the number of events in the add chip (R-type ADD only).
        let total_add = opcode_counts[Opcode::ADD];
        let addi_count = local_counts.addi_count;
        event_counts[RiscvAirId::Add] = total_add.saturating_sub(addi_count);

        // Compute the number of events in the addi chip (I-type ADD with imm_c).
        event_counts[RiscvAirId::Addi] = addi_count;
        ...
        event_counts[RiscvAirId::MemoryLocal] = touched_addresses;
        ...
        // Compute the number of events in the jal chip.
        event_counts[RiscvAirId::Jal] = opcode_counts[Opcode::JAL];

        // Compute the number of events in the jalr chip.
        ...
        // No cross-chip dependencies in the new architecture.
        // Each chip is self-contained: DivRem no longer generates Mul/Lt events.
    }
...
mod tests {
    ...
    use dt_stark::DTCoreOpts;
    use dt_zkvm::syscalls::SHA_COMPRESS;
    ...
    fn test_fibonacci_program_run_with_max_cycles() {
        ...
        let program = fibonacci_program();
        let context = DTContext::builder().max_cycles(max_cycles).build();
        let mut runtime = Executor::with_context(program, DTCoreOpts::default(), context);
        runtime.run().unwrap();
    }
