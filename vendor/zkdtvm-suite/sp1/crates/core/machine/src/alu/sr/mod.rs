...
impl<F: Field> MachineAir<F> for ShiftRightChip {
    ...
    fn generate_trace(
        ...
        let mut values = zeroed_f_vec(nb_rows * NUM_SHIFT_RIGHT_COLS);
        values.chunks_mut(chunk_size * NUM_SHIFT_RIGHT_COLS).enumerate().par_bridge().for_each(
            |(i, rows)| {
                rows.chunks_mut(NUM_SHIFT_RIGHT_COLS).enumerate().for_each(|(j, row)| {
                    ...
                    let (record, event) = &input.shift_right_events[idx];
                    self.event_to_row(record, event, cols, &mut byte_lookup_events, shard);
                });
            },
        );

        let mut pad_row = [F::zero(); NUM_SHIFT_RIGHT_COLS];
        let pad_cols: &mut ShiftRightCols<F> = pad_row.as_mut_slice().borrow_mut();
        pad_cols.shift_by_n_bits[0] = F::one();
        pad_cols.shift_by_n_bytes[0] = F::one();

        let main = RowMajorMatrix::new(values, NUM_SHIFT_RIGHT_COLS);
        CompressedMatrix::new(main, PaddingRow::General(pad_row.to_vec()), padded_nb_rows)
    }
...
impl ShiftRightChip {
    ...
    fn event_to_row<F: Field>(
        ...
        // Initialize cols with basic operands and flags derived from the current event.
        {
            ...
            // cols.op_a_not_zero = F::from_bool(!event.op_a_0);
        ...
        // Bit shifting.
        {
            ...
            for i in 0..WORD_SIZE {
                debug_assert_eq!(cols.mem_ops.op_a_value()[i], cols.bit_shift_result[i].clone());
            }
            // Range checks.
    ...
    AB: DTCoreAirBuilder,
{
    fn eval(&self, builder: &mut AB) {
        ...
        // Calculate the number of bits and bytes to shift by from c.
        {
            ...
            builder.assert_eq(c_byte_sum, c_word[0]);
        ...
        // Byte shift the sign-extended b.
        {
            ...
            for i in 0..WORD_SIZE {
                sign_extended_b.push(b_word[i].into());
            }
        ...
        {
            for i in 0..WORD_SIZE {
                builder.when(op_a_not_0.clone()).assert_eq(a_word[i], local.bit_shift_result[i]);
            }
        }
        ...
        // Check that the flags are indeed boolean.
        {
            ...
            builder.assert_eq(local.is_real, local.is_sra + local.is_srl);
        ...
        let opcode = local.is_sra * AB::Expr::from_canonical_u32(Opcode::SRA as u32) +
            local.is_srl * AB::Expr::from_canonical_u32(Opcode::SRL as u32);
        ...
        //In our case, if is dummy row, op_a_zero -> 0, op_a_not_zero = 1 - 0,
        //constraint assert_eq!(a_word[0], local.bit_shift_result[0])) enable, [0;4] equals [0;4]
