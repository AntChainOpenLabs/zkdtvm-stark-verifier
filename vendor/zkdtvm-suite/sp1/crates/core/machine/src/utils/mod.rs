...
/// Aligns a power-of-2 row count so that its log2 is a multiple of `NUM_SKIP_ROUNDS`
/// when below `CHIP_LOG_HEIGHT_THRESHOLD`. This is required by the sumcheck skip-rounds
/// optimization which processes `NUM_SKIP_ROUNDS` variables at a time.
...
/// `2^fixed_power` after checking that `n <= 2^fixed_power`. If the fixed size is too small for `n`
/// (e.g. after dynamic shape fallback), returns the next power of two that fits `n` instead of
/// panicking.
pub fn next_power_of_two(n: usize, fixed_power: Option<usize>) -> usize {
    match fixed_power {
        Some(power) => {
            ...
            if n > padded_nb_rows {
                // Shape was too small (e.g. preset vs actual event count); use next power of two
                // that fits.
                ...
                tracing::debug!(
                    "fixed log2 rows too small: got {}, expected {}; using {}",
                    n,
                    padded_nb_rows,
                    fallback
                );
...
pub fn zeroed_f_vec<F: Field>(len: usize) -> Vec<F> {
    ...
    // SAFETY: F is a 4-byte field element (asserted above) where all-zero bytes is a valid
    // representation (the zero element). Vec<u32> and Vec<F> have identical layout.
