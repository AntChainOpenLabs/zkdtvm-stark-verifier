...
impl<F: Field> FixedRotateRightOperation<F> {
    pub fn populate(&mut self, record: &mut impl ByteRecord, input: u32, rotation: usize) -> u32 {
        let result = input.rotate_right(rotation as u32);
        ...
        let input = [input[num_bytes_to_rotate], input[(num_bytes_to_rotate + 1) % 2]];
    ...
    pub fn eval<AB: DTAirBuilder<F = F>>(
        ...
        let lower_bits: [AB::Expr; 2] = std::array::from_fn(|i| {
            let lower_bits =
                input[(num_bytes_to_rotate + i) % 2].clone() - cols.higher_bits[i] * multiplier;
        ...
        //     cols.value[1],
        //     cols.higher_bits[1].into() + lower_bits[0].clone() * multiplier,
        // );
        // builder.when(is_real).assert_eq(
        //     cols.value[0],
...
impl<F: Field> FixedRotateRightOperationU64<F> {
    pub fn populate(&mut self, record: &mut impl ByteRecord, input: u64, rotation: usize) -> u64 {
        ...
        let num_bytes_to_rotate = rotation / 8;
        let num_bits_to_rotate = rotation % 8;

        let input: [u32; 8] =
            std::array::from_fn(|i| ((input >> ((num_bytes_to_rotate + i) % 8)) & 0xFFu64) as u32);
        ...
        for i in 0..8 {
            ...
            if num_bits_to_rotate <= 6 {
                record.add_bit_range_check(higher_bits as u16, (8 - num_bits_to_rotate) as u8);
            }
        }
    ...
    /// Evaluates the u64 fixed rotate right.
    pub fn eval<AB: DTAirBuilder<F = F>>(
        cols: &FixedRotateRightOperation<AB::Var>,
        builder: &mut AB,
        input: [Word<impl Into<AB::Expr>>; 2],
        rotation: usize,
        ...
        let input = input.map(|input| input.0.map(|input| input.into()));
        let input_flatten = input.as_flattened();
        let is_real = is_real.into();

        let multiplier = AB::F::from_canonical_u32(1u32 << num_bits_to_rotate);
        let lower_bits: [AB::Expr; 8] = std::array::from_fn(|i| {
            let lower_bits = input_flatten[(num_bytes_to_rotate + i) % 8].clone() -
            ...
            // TODO: deal with this
            if num_bits_to_rotate == 8 {
                builder.assert_zero(cols.higher_bits[i]);
            } else if num_bits_to_rotate == 7 {
                builder.assert_bool(cols.higher_bits[i]);
            } else {
                builder.send_byte(
                    AB::F::from_canonical_u32(ByteOpcode::BitRange as u32),
                    cols.higher_bits[i],
                    AB::F::from_canonical_u32((8 - num_bits_to_rotate) as u32),
                    AB::F::zero(),
                    is_real.clone(),
                );
            }
        ...
        let multiplier = AB::F::from_canonical_u32(1u32 << (8 - num_bits_to_rotate));
        // builder.when(is_real).assert_eq(
        //     cols.value[1],
        //     cols.higher_bits[1].into() + lower_bits[0].clone() * multiplier,
        // );
        ...
        [
            Word(std::array::from_fn(|i| {
                cols.higher_bits[i].into() + lower_bits[i + 1].clone() * multiplier
            })),
            Word(std::array::from_fn(|i| {
                cols.higher_bits[4 + i].into() + lower_bits[(4 + i + 1) % 8].clone() * multiplier
            })),
        ]
    }
}
...
// ============================================================================
// PolyAir three-phase helpers for FixedRotateRightOperation
// ============================================================================
...
pub const fn fixed_rotate_right_num_interactions(rotation: usize) -> usize {
    let num_bits = rotation % 16;
    let per_limb = if num_bits == 0 || num_bits == 1 || num_bits == 15 { 1 } else { 2 };
    per_limb * 2
}

pub fn fixed_rotate_right_precompute_lc<AB: dt_stark::air::FullAirBuilder>(
    builder: &mut AB,
    cols: &FixedRotateRightOperation<AB::VarMaybeExt>,
    ...
    for i in 0..2 {
        let lower_bits = input[(num_bytes_to_rotate + i) % 2].clone() -
            cols.higher_bits[i].clone() * multiplier.clone();
...
pub fn fixed_rotate_right_gate_constraints<AB: dt_stark::air::FullAirBuilder>(
    builder: &mut AB,
    cols: &FixedRotateRightOperation<AB::VarMaybeExt>,
    input: CompactWord<AB::VarMaybeExt>,
    rotation: usize,
...
pub fn fixed_rotate_right_result<AB: dt_stark::air::FullAirBuilder>(
    cols: &FixedRotateRightOperation<AB::VarMaybeExt>,
    input: &CompactWord<AB::VarMaybeExt>,
    rotation: usize,
    ...
    let lower_bits: [AB::VarMaybeExt; 2] = std::array::from_fn(|i| {
        input[(num_bytes_to_rotate + i) % 2].clone() -
            cols.higher_bits[i].clone() * multiplier.clone()
    });

    let result_multiplier =
        AB::VarMaybeExt::from(AB::F::from_canonical_u32(1u32 << (16 - num_bits_to_rotate)));
    CompactWord([
        cols.higher_bits[0].clone() + lower_bits[1].clone() * result_multiplier.clone(),
        cols.higher_bits[1].clone() + lower_bits[0].clone() * result_multiplier,
    ])
}

pub fn fixed_rotate_right_lookup<AB: dt_stark::air::FullAirBuilder>(
    builder: &mut AB,
