...
pub struct DTStdin {
    ...
    pub ptr: usize,
    pub proofs: Vec<(DTReduceProof<CoreSC>, SCStarkVerifyingKey<CoreSC>)>,
}

impl DTStdin {
    /// Create a new `DTStdin`.
    pub const fn new() -> Self {
        Self { buffer: Vec::new(), ptr: 0, proofs: Vec::new() }
    }

    /// Create a `DTStdin` from a slice of bytes.
    ...
    pub fn write_proof(&mut self, proof: DTReduceProof<CoreSC>, vk: SCStarkVerifyingKey<CoreSC>) {
        self.proofs.push((proof, vk));
    }
}

pub mod proof_serde {
    use dt_stark::{MachineProof, StarkGenericConfig};
