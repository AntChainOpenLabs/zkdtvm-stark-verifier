use crate::{
    ...
    utils::{limbs_from_access, next_power_of_two, padded_rows_threshold, words_to_bytes_le},
};
...
pub(super) const NUM_COLS: usize = size_of::<U256x2048MulCols<u8>>();
...
pub(super) const WORDS_FIELD_ELEMENT: usize = WordsFieldElement::USIZE;
pub(super) const LO_REGISTER: u32 = Register::X12 as u32;
pub(super) const HI_REGISTER: u32 = Register::X13 as u32;
...
impl<F: Field> MachineAir<F> for U256x2048MulChip {
    ...
    fn generate_trace(
        ...
        let main = RowMajorMatrix::new(rows.into_iter().flatten().collect::<Vec<_>>(), NUM_COLS);
