...
use dt_stark::{
    air::{BaseAirBuilder, DTAirBuilder, InteractionScope, MachineAir, Polynomial},
...
use crate::{
    ...
    operations::field::{
        field_add_op::FieldAddOpCols, field_inner_product::FieldInnerProductCols,
        field_op::FieldOpCols, field_sqrt::FieldSqrtCols, range::FieldLtCols,
    },
    utils::{
        bytes_to_words_le_vec, limbs_from_access, limbs_from_prev_access, next_power_of_two,
        padded_rows_threshold,
    },
};
...
pub struct WeierstrassDecompressCols<T, P: FieldParameters + NumWords> {
    ...
    pub(crate) x_3_plus_b_plus_ax: FieldAddOpCols<T, P>,
    pub(crate) y: FieldSqrtCols<T, P>,
    pub(crate) neg_y: FieldAddOpCols<T, P>,
}
...
impl<E: EllipticCurve + WeierstrassParameters> WeierstrassDecompressChip<E> {
    ...
    pub(super) fn populate_field_ops<F: Field>(
        ...
        let x_2 = cols.x_2.populate(record, &x.clone(), &x.clone(), FieldOperation::Sqr);
    ...
    for WeierstrassDecompressChip<E>
{
    ...
    fn generate_trace(
        ...
        let main = RowMajorMatrix::new(rows.into_iter().flatten().collect::<Vec<_>>(), width);
    ...
    Limbs<AB::Var, <E::BaseField as NumLimbs>::Limbs>: Copy,
{
    fn eval(&self, builder: &mut AB) {
        ...
        local.x_2.eval(builder, &x, &x, FieldOperation::Sqr, local.is_real);
...
mod tests {
    ...
    fn test_weierstrass_bls_decompress() {
        ...
        for _ in 0..num_tests {
            ...
            let stdin = DTStdin::from(&compressed);
    ...
    fn test_weierstrass_k256_decompress() {
        ...
        for _ in 0..num_tests {
            ...
            let inputs = DTStdin::from(&compressed);
