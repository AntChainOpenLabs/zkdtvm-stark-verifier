...
use dt_stark::{air::MachineAir, sumcheck::trace::CompressedMatrix, Word};
...
impl<F: Field> MachineAir<F> for ShaCompressChip {
    ...
    fn generate_trace(
        ...
        rows.iter_mut().for_each(|row| {
            let cols: &mut ShaCompressCols<F> = row.as_mut_slice().borrow_mut();
            //is_real_last
            cols.is_real_last = cols.is_real * (F::one() - cols.is_last_row);
            //local.octet_num[0] + local.octet_num[9] * (AB::Expr::one() - local.octet[7])
            cols.degree_helper1 =
                cols.octet_num[0] + cols.octet_num[9] * (F::one() - cols.octet[7]);
            //local.octet_num[0] * local.octet[i]
            for i in 0..8 {
                cols.degree_helper2[i] = cols.octet_num[0] * cols.octet[i];
            }
        });
