...
use dt_core_executor::{
    events::MemoryAccessPosition, syscalls::SyscallCode, Opcode, Register::X5, DEFAULT_PC_INC,
};
use dt_stark::{
    air::{
        BaseAirBuilder, DTAirBuilder, InteractionScope, PublicValues, DT_PROOF_NUM_PV_ELTS,
    ...
    AB::Var: Sized,
{
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        let public_values_slice: [AB::PublicVar; DT_PROOF_NUM_PV_ELTS] =
        ...
        CPUState::<AB::F>::eval(
            ...
            local.cpu_state,
            local.next_pc.into(),
            AB::Expr::from_canonical_u32(DEFAULT_PC_INC) + local.num_extra_cycles,
            local.is_real.into(),
            execution_shard,
        );
        ...
        builder
            ...
            .assert_eq(local.next_pc, local.cpu_state.pc + AB::Expr::from_canonical_u32(4));
...
impl SyscallInstrsChip {
    ...
    pub(crate) fn eval_ecall<AB: DTAirBuilder>(
        ...
        builder.send_syscall(
            local.cpu_state.shard,
            local.cpu_state.clk::<AB>(),
            syscall_id,
            local.mem_ops.op_b_value().reduce::<AB>(),
            local.mem_ops.op_c_value().reduce::<AB>(),
            send_to_table,
        ...
        builder
            ...
            .assert_word_eq(*local.mem_ops.op_a_access.value(), zero_word);

        // When the syscall is not one of ENTER_UNCONSTRAINED or HINT_LEN, op_a shouldn't change.
        builder.when(local.is_real).when_not(is_enter_unconstrained + is_hint_len).assert_word_eq(
            *local.mem_ops.op_a_access.value(),
    ...
    pub(crate) fn eval_commit<AB: DTAirBuilder>(
        &self,
        builder: &mut AB,
        local: &SyscallInstrColumns<AB::Var>,
        ...
        for (i, bit) in local.index_bitmap.iter().enumerate() {
            builder
                .when(local.is_real)
                .when(*bit)
                .assert_eq(local.mem_ops.op_b_value()[0], AB::Expr::from_canonical_u32(i as u32));
        }
        ...
        for i in 0..3 {
            builder
                .when(local.is_real)
                .when(is_commit.clone() + is_commit_deferred_proofs.clone())
                .assert_zero(local.mem_ops.op_b_value()[i + 1]);
        }
        ...
        let digest_word = *local.mem_ops.op_c_value();
    ...
    pub(crate) fn eval_halt_unimpl<AB: DTAirBuilder>(
        &self,
        builder: &mut AB,
        local: &SyscallInstrColumns<AB::Var>,
        ...
        builder
            .when(local.is_halt)
            .assert_word_eq(*local.mem_ops.op_b_value(), local.operand_to_check);

        // Check that the `op_b_value` reduced is the `public_values.exit_code`.
        builder
            .when(local.is_halt)
            .assert_eq(local.mem_ops.op_b_value().reduce::<AB>(), public_values.exit_code);
    }

    /// Returns a boolean expression indicating whether the instruction is a HALT instruction.
    pub(crate) fn eval_is_halt_syscall<AB: DTAirBuilder>(
        &self,
        builder: &mut AB,
        local: &SyscallInstrColumns<AB::Var>,
    ) {
        // `is_halt` is checked to be correct in `eval_is_halt_syscall`.
        let prev_op_a_value = local.mem_ops.op_a_access.prev_value();
        // The syscall code is the read-in value of op_a at the start of the instruction.
        // let syscall_code = local.op_a_access.prev_value();
        let syscall_code = *prev_op_a_value;
    ...
    pub(crate) fn get_is_commit_related_syscall<AB: DTAirBuilder>(
        &self,
        builder: &mut AB,
        local: &SyscallInstrColumns<AB::Var>,
    ) -> (AB::Expr, AB::Expr) {
    ...
    pub(crate) fn get_num_extra_ecall_cycles<AB: DTAirBuilder>(
        &self,
        local: &SyscallInstrColumns<AB::Var>,
    ) -> AB::Expr {
        // The syscall code is the read-in value of op_a at the start of the instruction.
