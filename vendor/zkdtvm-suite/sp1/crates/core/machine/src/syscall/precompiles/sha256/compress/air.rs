...
use dt_stark::{
    air::{DTAirBuilder, InteractionScope},
    Word,
};
...
impl ShaCompressChip {
    fn eval_control_flow_flags<AB: DTAirBuilder>(
        ...
        builder
            .assert_eq(local.is_real_last, local.is_real * (AB::Expr::one() - local.is_last_row));
        builder.assert_eq(
            local.degree_helper1,
            local.octet_num[0] + local.octet_num[9] * (AB::Expr::one() - local.octet[7]),
        );
        for (i, var) in vars.iter().enumerate() {
            ...
            builder.when_transition().when(local.degree_helper1).assert_word_eq(*var, next_vars[i]);

            // When column is read from memory during init, is should be equal to the memory value.
            builder
                .when_transition()
                .when(local.degree_helper2[i])
                .assert_word_eq(*var, *local.mem.value());
            builder.assert_eq(local.degree_helper2[i], local.octet_num[0] * local.octet[i]);
        }
        ...
        builder.when_transition().when(local.is_real_last).assert_eq(local.shard, next.shard);
        builder.when_transition().when(local.is_real_last).assert_eq(local.clk, next.clk);
        builder.when_transition().when(local.is_real_last).assert_eq(local.w_ptr, next.w_ptr);
        builder.when_transition().when(local.is_real_last).assert_eq(local.h_ptr, next.h_ptr);

        // Assert that is_real is a bool.
        builder.assert_bool(local.is_real);

        // If this row is real and not the last cycle, then next row should also be real.
        builder.when_transition().when(local.is_real_last).assert_one(next.is_real);
    ...
    fn eval_finalize_ops<AB: DTAirBuilder>(
        &self,
        builder: &mut AB,
        local: &ShaCompressCols<AB::Var>,
    ) {
