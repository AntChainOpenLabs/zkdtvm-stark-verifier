...
use dt_stark::air::{DTAirBuilder, InteractionScope};
    ...
    AB: DTAirBuilder,
{
    fn eval(&self, builder: &mut AB) {
        ...
        //constraint degree helper
        builder.assert_eq(local.cycle_16_end.result * local.cycle_48[2], local.degree_helper);
        // Copy over the inputs until the result has been computed (every 48 rows).
        builder.when_transition().when_not(local.degree_helper).assert_eq(local.shard, next.shard);
        builder.when_transition().when_not(local.degree_helper).assert_eq(local.clk, next.clk);
        builder.when_transition().when_not(local.degree_helper).assert_eq(local.w_ptr, next.w_ptr);
