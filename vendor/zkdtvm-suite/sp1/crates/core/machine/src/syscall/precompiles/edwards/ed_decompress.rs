...
use dt_stark::{
    air::{BaseAirBuilder, DTAirBuilder, InteractionScope, MachineAir},
...
use crate::{
    ...
    utils::{limbs_from_access, limbs_from_prev_access, next_power_of_two, padded_rows_threshold},
};
...
impl<F: Field> EdDecompressCols<F> {
    ...
    pub(super) fn populate_field_ops<E: EdwardsParameters>(
        &mut self,
...
impl<V: Copy> EdDecompressCols<V> {
    pub fn eval<AB: DTAirBuilder<Var = V>, P: FieldParameters, E: EdwardsParameters>(
        &self,
        builder: &mut AB,
    ) where
        V: Into<AB::Expr>,
