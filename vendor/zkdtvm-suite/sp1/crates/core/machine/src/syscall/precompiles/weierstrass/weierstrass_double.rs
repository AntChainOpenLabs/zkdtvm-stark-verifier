...
use dt_stark::{
    air::{DTAirBuilder, InteractionScope, MachineAir},
...
use crate::{
    ...
    operations::field::{
        field_add_op::FieldAddOpCols, field_mul_digit::FieldMulDigitCols, field_op::FieldOpCols,
        range::FieldLtCols,
    },
...
pub struct WeierstrassDoubleAssignCols<T, P: FieldParameters + NumWords> {
    ...
    pub(crate) slope_denominator: FieldAddOpCols<T, P>,
    pub(crate) slope_numerator: FieldAddOpCols<T, P>,
    pub(crate) slope: FieldOpCols<T, P>,
    pub(crate) p_x_squared: FieldOpCols<T, P>,
    pub(crate) p_x_squared_times_3: FieldMulDigitCols<T, P>,
    pub(crate) slope_squared: FieldOpCols<T, P>,
    pub(crate) p_x_plus_p_x: FieldAddOpCols<T, P>,
    pub(crate) x3_ins: FieldAddOpCols<T, P>,
    pub(crate) p_x_minus_x: FieldAddOpCols<T, P>,
    pub(crate) y3_ins: FieldAddOpCols<T, P>,
...
impl<E: EllipticCurve + WeierstrassParameters> WeierstrassDoubleAssignChip<E> {
    ...
    fn populate_field_ops<F: Field>(
        ...
        let slope = {
            ...
            let slope_numerator = {
                let p_x_squared =
                    cols.p_x_squared.populate(blu_events, &p_x, &p_x, FieldOperation::Sqr);
            ...
            let slope_denominator =
                cols.slope_denominator.populate(blu_events, &p_y, &p_y, FieldOperation::Add);
        ...
        let x = {
            let slope_squared =
                cols.slope_squared.populate(blu_events, &slope, &slope, FieldOperation::Sqr);
            let p_x_plus_p_x =
    ...
    for WeierstrassDoubleAssignChip<E>
{
    ...
    fn generate_trace(
        ...
        let mut padded_nb_rows = input
            .fixed_log2_rows::<F, _>(self)
            .map(|x| 1 << x)
            .unwrap_or(std::cmp::max(real_nb_rows.next_power_of_two(), 4));
        padded_nb_rows = padded_rows_threshold(padded_nb_rows);
        let mut values = zeroed_f_vec(real_nb_rows * num_cols);
        let chunk_size = 64;
        ...
        let main = RowMajorMatrix::new(values, num_weierstrass_double_cols::<E::BaseField>());
    ...
    Limbs<AB::Var, <E::BaseField as NumLimbs>::Limbs>: Copy,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let slope = {
            // slope_numerator = a + (p.x * p.x) * 3.
            {
                local.p_x_squared.eval(builder, &p_x, &p_x, FieldOperation::Sqr, local.is_real);

                local.p_x_squared_times_3.eval(
                    builder,
                    &local.p_x_squared.result,
                    &AB::Expr::from_canonical_u8(3u8),
                    FieldOperation::Mul,
                    local.is_real,
                );
            ...
            // slope_denominator = 2 * y.
            local.slope_denominator.eval(builder, &p_y, &p_y, FieldOperation::Add, local.is_real);
        ...
        // x = slope * slope - (p.x + p.x).
        let x = {
            local.slope_squared.eval(builder, slope, slope, FieldOperation::Sqr, local.is_real);
...
pub mod tests {
    ...
    use test_artifacts::{
        BLS12381_DOUBLE_ELF, BN254_DOUBLE_ELF,
        SECP256K1_DOUBLE_ELF, /* SECP256R1_DOUBLE_ELF, */
    };
