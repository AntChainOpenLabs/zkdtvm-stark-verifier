...
use dt_stark::{
    air::{DTAirBuilder, InteractionScope, MachineAir, Polynomial},
...
use crate::{
    ...
    operations::field::{field_add_op::FieldAddOpCols, field_op::FieldOpCols},
...
pub struct WeierstrassAddAssignCols<T, P: FieldParameters + NumWords> {
    ...
    pub(crate) p_x_plus_q_x: FieldAddOpCols<T, P>,
    ...
    for WeierstrassAddAssignChip<E>
{
    ...
    fn generate_trace(
        ...
        let main = RowMajorMatrix::new(values, num_weierstrass_add_cols::<E::BaseField>());
    ...
    fn padding_row(&self) -> Vec<F> {
        let mut row = zeroed_f_vec(num_weierstrass_add_cols::<E::BaseField>());
        let cols: &mut WeierstrassAddAssignCols<F, E::BaseField> = row.as_mut_slice().borrow_mut();
...
mod tests {
    ...
    use test_artifacts::{
        /* BLS12381_ADD_ELF, */ BLS12381_DOUBLE_ELF, BLS12381_MUL_ELF,
        /* BN254_ADD_ELF, */ BN254_MUL_ELF,
