...
use crate::utils::{padded_rows_threshold, zeroed_f_vec};
...
impl<F: Field> MachineAir<F> for KeccakPermuteChip {
    ...
    fn generate_trace(
        ...
        let mut num_rows = (num_events * NUM_ROUNDS).next_power_of_two();
        num_rows = padded_rows_threshold(num_rows);
        let chunk_size = 8;
