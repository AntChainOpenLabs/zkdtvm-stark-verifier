...
/// Builds the program used for SHA extend tests (used by memory tests and extend tests).
#[must_use]
pub fn sha_extend_program() -> dt_core_executor::Program {
    use dt_core_executor::{syscalls::SyscallCode, Instruction, Opcode, Program};
    let w_ptr = 100;
...
pub mod extend_tests {
    ...
    use dt_stark::{air::MachineAir, CpuProver};
    ...
    use super::{sha_extend_program, ShaExtendChip};

    #[test]
    fn generate_trace() {
        let mut shard = ExecutionRecord::default();
        shard.add_events =
            vec![(RTypeRecord::default(), AluEvent::new(0, Opcode::ADD, 14, 8, 6, false))];
