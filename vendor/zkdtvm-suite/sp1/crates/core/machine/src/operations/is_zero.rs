...
pub fn is_zero_op_gate_constraints<AB: dt_stark::air::FullAirBuilder>(
    builder: &mut AB,
    a: AB::VarMaybeExt,
    inverse: AB::VarMaybeExt,
    result: AB::VarMaybeExt,
    is_real: AB::VarMaybeExt,
) {
    let one = AB::one_maybe();
    let is_zero = one.clone() - inverse * a.clone();
    builder.when(is_real.clone()).assert_zero(is_zero - result.clone());
    builder.when(is_real.clone()).assert_zero(result.clone() * (one - result.clone()));
    builder.when(is_real).when(result).assert_zero(a);
}
