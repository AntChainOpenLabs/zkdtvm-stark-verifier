...
impl<F: Field, P: FieldParameters> FieldInnerProductCols<F, P> {
    pub fn populate(
        &mut self,
...
// ============================================================================
// PolyAir three-phase helpers for FieldInnerProductCols
// ============================================================================
//
// FieldInnerProductCols has the same column structure as FieldOpCols
// (result, carry, witness all as Limbs), so the interaction pattern is identical.

/// Compute the number of interactions generated by a single `FieldInnerProductCols` instance.
///
/// Same as `field_op_num_interactions`: `P::NB_LIMBS + P::NB_WITNESS_LIMBS`.
pub const fn field_inner_product_num_interactions<P: FieldParameters>() -> usize {
    super::field_op::field_op_num_interactions::<P>()
}

/// Precompute lookup denominators for a `FieldInnerProductCols` instance.
///
/// Delegates to `field_op_precompute_lc` since the column layout is identical.
pub fn field_inner_product_precompute_lc<AB: dt_stark::air::FullAirBuilder, P: FieldParameters>(
    builder: &mut AB,
...
/// Declare multiplicities for a `FieldInnerProductCols` instance's range check lookups.
///
/// Delegates to `field_op_lookup` since the interaction pattern is identical.
pub fn field_inner_product_lookup<AB: dt_stark::air::FullAirBuilder, P: FieldParameters>(
    builder: &mut AB,
...
mod tests {
    ...
    impl<F: Field, P: FieldParameters> MachineAir<F> for FieldIpChip<P> {
