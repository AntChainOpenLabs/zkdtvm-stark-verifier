...
fn biguint_to_field<F: Field>(num: BigUint) -> F {
    ...
    let base = F::from_canonical_u64((1 << 32) % F::PO64);
...
pub fn compute_root_quotient_and_shift<F: Field>(
    ...
    for c in p_quotient.coefficients().iter() {
        debug_assert!(c.neg().as_u64() < offset_u64 || c.as_u64() < offset_u64);
    }
...
#[inline]
pub fn split_u16_limbs_to_u8_limbs<F: Field>(slice: &[F]) -> (Vec<F>, Vec<F>) {
    (
        slice.iter().map(|x| x.as_u64() as u8).map(|x| F::from_canonical_u8(x)).collect(),
        slice.iter().map(|x| (x.as_u64() >> 8) as u8).map(|x| F::from_canonical_u8(x)).collect(),
    )
}
