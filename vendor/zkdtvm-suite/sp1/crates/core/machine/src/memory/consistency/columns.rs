...
impl<F: AbstractField> MemoryReadCols<F> {
    pub fn dummy(value: u32) -> Self {
        Self { access: MemoryAccessCols::dummy(value) }
    }
}
/// Memory write access.
...
impl<F: AbstractField> MemoryReadWriteCols<F> {
    pub fn dummy(value: u32) -> Self {
        Self { prev_value: Word::from(0), access: MemoryAccessCols::dummy(value) }
    }
}
#[derive(AlignedBorrow, Default, Debug, Clone, Copy, Serialize, Deserialize)]
#[repr(C)]
...
impl<F: AbstractField> MemoryAccessCols<F> {
    pub fn dummy(value: u32) -> Self {
        Self {
            ...
            prev_clk: F::zero(),
            compare_clk: F::zero(),
            diff_16bit_limb: F::zero(),
            diff_12bit_limb: F::zero(),
        }
    }
}
