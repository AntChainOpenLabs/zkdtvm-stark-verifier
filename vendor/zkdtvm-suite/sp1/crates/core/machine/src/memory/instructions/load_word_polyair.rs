...
use super::{
    load_word::{LoadWordChip, LoadWordCols, NUM_LOAD_WORD_COLUMNS},
...
// ============================================================================
// Main column offsets within `LoadWordCols<u8>` (NUM_LOAD_WORD_COLUMNS = 56).
//
// Layout (#[repr(C)]):
//   [0]      cpu_state.shard
...
//   [13..18] mem_ops.op_a_access.access.{prev_shard, prev_clk, compare_clk, diff_16, diff_12}
...
//   [0]      is_real
//   [1]      cpu_state.shard
//   [2]      cpu_state.clk_16_28
//   [3]      cpu_state.clk_0_16
//   [4]      op_a_zero
//   [5]      addr_range_checker.most_sig_byte_lt_120
//   [6]      addr_ls_two_bits
//   [7]      aligned_address
//   [8]      most_sig_bytes_zero.inverse
//   [9]      most_sig_bytes_zero.result
//   [10..14] op_a_access.access.value
//   [14..19] op_a_access.access.{ts fields}
//   [19..23] op_b_access.access.value
//   [23..28] op_b_access.access.{ts fields}
//   [28..32] op_c_imm
//   [32..36] addr_word.value
//   [36..40] offset_bit
//   [40..44] memory_access.access.value
//   [44..49] memory_access.access.{ts fields}
// ============================================================================
...
pub struct LoadWordChipPolyAir;

impl<AB: FullAirBuilder> FullAir<AB> for LoadWordChipPolyAir
where
    AB::VarMaybeExt: Clone,
{
    ...
    fn precompute_lc(&self, builder: &mut AB) {
        ...
        let local: &LoadWordCols<AB::VarMaybeExt> = unsafe { core::mem::transmute(main.as_ptr()) };
        ...
        // =====================================================================
        {
            let lw_opcode = AB::VarMaybeExt::from(AB::F::from_canonical_u8(Opcode::LW as u8));
            itype_register_op_precompute_lc(
                builder,
                pc.clone(),
                lw_opcode,
    ...
    /// Gate constraints, ordered to match the original `Air<AB>::eval()` in `load_word.rs`.
    fn eval(&self, builder: &mut AB) {
        ...
        // ── load_word.rs L155: assert_bool(is_real) ────────────────────
        // All boolean constraints enforced by BitVec #23, except is_real
        ...
        // ── load_word.rs L158-165: CPUState::eval() ────────────────────
        ...
        // ── load_word.rs L167-175: ITypeRegisterOp::eval() ─────────────
        ...
        // ── load_word.rs L177-183: AddressOperation::eval() ────────────
        ...
        // ── load_word.rs L184-189: eval_memory_access → memory_timestamp_gate_constraints
        ...
        // ── load_word.rs L190: is_real == offset_bit[0] (word-aligned) ─
        builder.assert_zero(is_real.clone() - offset[0].clone());

        // ── load_word.rs L191-194: when(perform) a_word == mem_word ────
        let perform = is_real - local[RES_OP_A_ZERO].clone();
        for i in 0..4 {
            builder.when(perform.clone()).assert_zero(a_word[i].clone() - mem_word[i].clone());
        }
    }
...
// =============================================================================
// MachineAir implementation (delegation to LoadWordChip)
// =============================================================================

impl<F: Field> BaseAir<F> for LoadWordChipPolyAir {
    fn width(&self) -> usize {
        NUM_LOAD_WORD_COLUMNS
    }
}

impl<F: Field> MachineAir<F> for LoadWordChipPolyAir {
    type Record = ExecutionRecord;
    type Program = Program;

    fn name(&self) -> String {
        "LoadWordPolyAir".to_string()
    }
    ...
    fn generate_trace(
        ...
        LoadWordChip.generate_trace(input, output)
    }
        ...
        let blu_batches = input
            ...
            .map(|events| {
                ...
                for (record, event) in events {
                    let mut row = [F::zero(); NUM_LOAD_WORD_COLUMNS];
                    let cols: &mut LoadWordCols<F> = row.as_mut_slice().borrow_mut();
                    LoadWordChip.event_to_row(record, event, cols, &mut blu, shard);
    ...
    fn included(&self, shard: &Self::Record) -> bool {
        <LoadWordChip as MachineAir<F>>::included(&LoadWordChip, shard)
    }
}
...
mod tests {
    ...
    use super::{super::load_word::LoadWordChip, *};
    ...
    fn beta_powers() -> Vec<EF> {
        ...
        let n = <LoadWordChipPolyAir as FullAir<
            PrecomputeRowBuilder<'_, F, F, EF>,
        >>::required_max_beta_power(&LoadWordChipPolyAir);
    ...
    fn reserved_poly_matrix(
        air: &LoadWordChipPolyAir,
        main: &RowMajorMatrix<F>,
    ) -> RowMajorMatrix<EF> {
        let reserved_poly =
            <LoadWordChipPolyAir as FullAir<PrecomputeRowBuilder<'_, F, F, EF>>>::reserved_poly(
                air,
            );
    ...
    #[test]
    fn test_load_word_column_layout() {
        assert_eq!(
            NUM_LOAD_WORD_COLUMNS,
            size_of::<LoadWordCols<u8>>(),
            "LoadWordCols layout mismatch"
        );
    }

    #[test]
    fn test_load_word_first_and_nonfirst_round_evaluation_satisfied() {
        let air = LoadWordChipPolyAir;
    ...
    fn random_load_word_trace(log_n: usize, _seed: u64) -> RowMajorMatrix<F> {
        ...
        let last_row_start = (base_height - 1) * NUM_LOAD_WORD_COLUMNS;
        let last_row = &base.values[last_row_start..last_row_start + NUM_LOAD_WORD_COLUMNS];
        let mut values = Vec::with_capacity(target_height * NUM_LOAD_WORD_COLUMNS);
    ...
    fn perf_multi_round_sumcheck() {
        ...
        let main = random_load_word_trace(log_n, seed);
        let height = main.height();
        ...
        let constraint_reducer = reducer();
        let global = EF::zero();
        let reserved_poly_desc = <LoadWordChipPolyAir as FullAir<
            PrecomputeRowBuilder<'_, F, F, EF>,
        >>::reserved_poly(&air);
    ...
    fn bitvec_mult_matches_send_count() {
        ...
        assert!(!shard.load_word_events.is_empty(), "fixture must yield load_word events");

        let mut deps = ExecutionRecord::default();
        <LoadWordChipPolyAir as MachineAir<F>>::generate_dependencies(
            &LoadWordChipPolyAir,
            &shard,
            &mut deps,
        );
        ...
        let expected = shard.load_word_events.len();
        assert!(expected > 0, "test fixture must include load_word events");
        assert_eq!(
