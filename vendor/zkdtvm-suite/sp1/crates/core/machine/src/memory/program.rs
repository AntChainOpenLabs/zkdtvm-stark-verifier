...
use dt_stark::{
    air::{
        AirInteraction, DTAirBuilder, InteractionScope, MachineAir, PublicValues,
...
impl<F: PrimeField32> MachineAir<F> for MemoryProgramChip {
    ...
    fn generate_preprocessed_trace(&self, program: &Self::Program) -> Option<CompressedMatrix<F>> {
        ...
        let mut values = zeroed_f_vec(nb_rows * NUM_MEMORY_PROGRAM_PREPROCESSED_COLS);
        ...
        let main = RowMajorMatrix::new(values, NUM_MEMORY_PROGRAM_PREPROCESSED_COLS);
        Some(CompressedMatrix::new(
            main,
            PaddingRow::Zero { width: NUM_MEMORY_PROGRAM_PREPROCESSED_COLS },
            padded_nb_rows,
        ))
    }
    ...
    fn generate_trace(
        ...
        CompressedMatrix::new(
            main,
            PaddingRow::Zero { width: NUM_MEMORY_PROGRAM_MULT_COLS },
            padded_nb_rows,
        )
    }
...
where
    AB: DTAirBuilder + PairBuilder + AirBuilderWithPublicValues,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let public_values_slice: [AB::Expr; DT_PROOF_NUM_PV_ELTS] =
