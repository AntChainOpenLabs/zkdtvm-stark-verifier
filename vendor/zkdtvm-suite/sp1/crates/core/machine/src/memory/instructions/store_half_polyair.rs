...
use super::{
    ...
    store_half::{StoreHalfChip, StoreHalfCols, NUM_STORE_HALF_COLUMNS},
};
use crate::{
    adapter::{
        register::b_type::{
...
// ============================================================================
// Main column offsets (NUM_STORE_HALF_COLUMNS = 56). Same struct as StoreByte/StoreWord.
// ============================================================================
...
const COL_MEM_PREV_VALUE: usize = 42;
const COL_MEM_VALUE: usize = 46;
const COL_MEM_TS: usize = 50;
const COL_IS_REAL: usize = 55;

// ============================================================================
// Reserved-poly layout (RES_NUM_COLS = 53). Excludes cpu_state.pc, op_a, op_b.
// memory_access.prev_value retained: eval enforces value/prev_value equality
// for bytes outside the offset region.
// ============================================================================
...
const RES_MEM_PREV_VALUE: usize = 40;
const RES_MEM_VALUE: usize = 44;
const RES_MEM_TS: usize = 48;
const RES_NUM_COLS: usize = 53;

#[derive(Default, Clone, Copy)]
pub struct StoreHalfChipPolyAir;

impl<AB: FullAirBuilder> FullAir<AB> for StoreHalfChipPolyAir
where
    AB::VarMaybeExt: Clone,
{
    ...
    fn reserved_poly(&self) -> Vec<PairCol> {
        ...
        for i in 0..4 {
            cols.push(PairCol::Main(COL_MEM_PREV_VALUE + i));
        }
        for i in 0..4 {
    ...
    fn precompute_lc(&self, builder: &mut AB) {
        ...
        let local: &StoreHalfCols<AB::VarMaybeExt> = unsafe { core::mem::transmute(main.as_ptr()) };
        ...
        let opcode = AB::VarMaybeExt::from(AB::F::from_canonical_u8(Opcode::SH as u8));
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        let stored_word: [AB::VarMaybeExt; 4] =
            core::array::from_fn(|i| local[RES_MEM_VALUE + i].clone());
        let read_word: [AB::VarMaybeExt; 4] =
            core::array::from_fn(|i| local[RES_MEM_PREV_VALUE + i].clone());
        ...
        let mem_access = crate::memory::MemoryAccessCols::<AB::VarMaybeExt> {
            value: dt_stark::Word(stored_word.clone()),
        ...
        builder.assert_zero(is_real - offset[0].clone() - offset[2].clone());

        builder.when(offset[0].clone()).assert_zero(a_word[0].clone() - stored_word[0].clone());
        builder.when(offset[0].clone()).assert_zero(a_word[1].clone() - stored_word[1].clone());
        builder.when(offset[0].clone()).assert_zero(read_word[2].clone() - stored_word[2].clone());
        builder.when(offset[0].clone()).assert_zero(read_word[3].clone() - stored_word[3].clone());

        builder.when(offset[2].clone()).assert_zero(a_word[0].clone() - stored_word[2].clone());
        builder.when(offset[2].clone()).assert_zero(a_word[1].clone() - stored_word[3].clone());
        builder.when(offset[2].clone()).assert_zero(read_word[0].clone() - stored_word[0].clone());
        builder.when(offset[2].clone()).assert_zero(read_word[1].clone() - stored_word[1].clone());
    }
...
// =============================================================================
// MachineAir implementation (delegation to StoreHalfChip)
// =============================================================================

impl<F: Field> BaseAir<F> for StoreHalfChipPolyAir {
    fn width(&self) -> usize {
        NUM_STORE_HALF_COLUMNS
    }
}

impl<F: Field> MachineAir<F> for StoreHalfChipPolyAir {
    type Record = ExecutionRecord;
    type Program = Program;

    fn name(&self) -> String {
        "StoreHalfPolyAir".to_string()
    }
    ...
    fn generate_trace(
        ...
        StoreHalfChip.generate_trace(input, output)
    }
        ...
        let blu_batches = input
            ...
            .map(|events| {
                ...
                for (record, event) in events {
                    let mut row = [F::zero(); NUM_STORE_HALF_COLUMNS];
                    let cols: &mut StoreHalfCols<F> = row.as_mut_slice().borrow_mut();
                    StoreHalfChip.event_to_row(record, event, cols, &mut blu, shard);
    ...
    fn included(&self, shard: &Self::Record) -> bool {
        <StoreHalfChip as MachineAir<F>>::included(&StoreHalfChip, shard)
    }
}
...
mod tests {
    ...
    use super::{super::store_half::StoreHalfChip, *};
    ...
    fn beta_powers() -> Vec<EF> {
        ...
        let n = <StoreHalfChipPolyAir as FullAir<
            PrecomputeRowBuilder<'_, F, F, EF>,
        >>::required_max_beta_power(&StoreHalfChipPolyAir);
    ...
    fn reducer() -> Vec<EF> {
        // Gate constraints: 42 (41 original + 1 is_real boolean gate)
        // Lookup batch: ceil(23/3) = 8
        // Cumulative sum: 3
        const NUM_GATE_CONSTRAINTS: usize = 42;
    ...
    fn reserved_poly_matrix(
        air: &StoreHalfChipPolyAir,
        main: &RowMajorMatrix<F>,
    ) -> RowMajorMatrix<EF> {
        let reserved_poly =
            <StoreHalfChipPolyAir as FullAir<PrecomputeRowBuilder<'_, F, F, EF>>>::reserved_poly(
                air,
            );
    ...
    #[test]
    fn test_store_half_column_layout() {
        assert_eq!(
            NUM_STORE_HALF_COLUMNS,
            size_of::<StoreHalfCols<u8>>(),
            "StoreHalfCols layout mismatch"
        );
    }

    #[test]
    fn test_store_half_first_and_nonfirst_round_evaluation_satisfied() {
        let air = StoreHalfChipPolyAir;
    ...
    fn random_store_half_trace(log_n: usize, _seed: u64) -> RowMajorMatrix<F> {
        ...
        let last_row_start = (base_height - 1) * NUM_STORE_HALF_COLUMNS;
        let last_row = &base.values[last_row_start..last_row_start + NUM_STORE_HALF_COLUMNS];
        let mut values = Vec::with_capacity(target_height * NUM_STORE_HALF_COLUMNS);
    ...
    fn perf_multi_round_sumcheck() {
        ...
        let main = random_store_half_trace(log_n, seed);
        let height = main.height();
        ...
        let constraint_reducer = reducer();
        let global = EF::zero();
        let reserved_poly_desc = <StoreHalfChipPolyAir as FullAir<
            PrecomputeRowBuilder<'_, F, F, EF>,
        >>::reserved_poly(&air);
    ...
    fn bitvec_mult_matches_send_count() {
        ...
        assert!(!shard.store_half_events.is_empty(), "fixture must yield store_half events");

        let mut deps = ExecutionRecord::default();
        <StoreHalfChipPolyAir as MachineAir<F>>::generate_dependencies(
            &StoreHalfChipPolyAir,
            &shard,
            &mut deps,
        );
        ...
        let expected = shard.store_half_events.len();
        assert!(expected > 0, "test fixture must include store_half events");
        assert_eq!(
