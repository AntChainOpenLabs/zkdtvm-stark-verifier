...
use super::{
    ...
    store_byte::{StoreByteChip, StoreByteCols, NUM_STORE_BYTE_COLUMNS},
};
use crate::{
    adapter::{
        register::b_type::{
...
// ============================================================================
// Main column offsets within `StoreByteCols<u8>` (NUM_STORE_BYTE_COLUMNS = 56).
// Same struct as StoreWord/StoreHalf.
// ============================================================================
...
// Excludes (precompute-only): cpu_state.pc, op_a, op_b.
// memory_access.prev_value is retained because eval enforces
// `value[i] == prev_value[i]` for bytes outside the offset.
// ============================================================================
...
pub struct StoreByteChipPolyAir;

impl<AB: FullAirBuilder> FullAir<AB> for StoreByteChipPolyAir
where
    AB::VarMaybeExt: Clone,
{
    ...
    fn precompute_lc(&self, builder: &mut AB) {
        ...
        let local: &StoreByteCols<AB::VarMaybeExt> = unsafe { core::mem::transmute(main.as_ptr()) };
        ...
        let opcode = AB::VarMaybeExt::from(AB::F::from_canonical_u8(Opcode::SB as u8));
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        let mem_value: [AB::VarMaybeExt; 4] =
            core::array::from_fn(|i| local[RES_MEM_VALUE + i].clone());
        let prev_value: [AB::VarMaybeExt; 4] =
        ...
        let mem_access = crate::memory::MemoryAccessCols::<AB::VarMaybeExt> {
            value: dt_stark::Word(mem_value.clone()),
        ...
        // store byte consistency
        let stored_byte = mem_value[0].clone() * offset[0].clone() +
            mem_value[1].clone() * offset[1].clone() +
            mem_value[2].clone() * offset[2].clone() +
            mem_value[3].clone() * offset[3].clone();
        builder.when(is_real.clone()).assert_zero(stored_byte - a_word[0].clone());

        // For each byte: if offset_bit[i] != 1, then value[i] == prev_value[i]
        for i in 0..4 {
            builder
                .when(is_real.clone() - offset[i].clone())
                .assert_zero(mem_value[i].clone() - prev_value[i].clone());
        }
    }
...
// =============================================================================
// MachineAir implementation (delegation to StoreByteChip)
// =============================================================================

impl<F: Field> BaseAir<F> for StoreByteChipPolyAir {
    fn width(&self) -> usize {
        NUM_STORE_BYTE_COLUMNS
    }
}

impl<F: Field> MachineAir<F> for StoreByteChipPolyAir {
    type Record = ExecutionRecord;
    type Program = Program;

    fn name(&self) -> String {
        "StoreBytePolyAir".to_string()
    }
    ...
    fn generate_trace(
        ...
        StoreByteChip.generate_trace(input, output)
    }
        ...
        let blu_batches = input
            ...
            .map(|events| {
                ...
                for (record, event) in events {
                    let mut row = [F::zero(); NUM_STORE_BYTE_COLUMNS];
                    let cols: &mut StoreByteCols<F> = row.as_mut_slice().borrow_mut();
                    StoreByteChip.event_to_row(record, event, cols, &mut blu, shard);
    ...
    fn included(&self, shard: &Self::Record) -> bool {
        <StoreByteChip as MachineAir<F>>::included(&StoreByteChip, shard)
    }
}

#[cfg(test)]
mod tests {
    use super::{super::store_byte::StoreByteChip, *};
    ...
    fn beta_powers() -> Vec<EF> {
        ...
        let n = <StoreByteChipPolyAir as FullAir<
            PrecomputeRowBuilder<'_, F, F, EF>,
        >>::required_max_beta_power(&StoreByteChipPolyAir);
    ...
    fn reserved_poly_matrix(
        air: &StoreByteChipPolyAir,
        main: &RowMajorMatrix<F>,
    ) -> RowMajorMatrix<EF> {
        let reserved_poly =
            <StoreByteChipPolyAir as FullAir<PrecomputeRowBuilder<'_, F, F, EF>>>::reserved_poly(
                air,
            );
    ...
    #[test]
    fn test_store_byte_first_and_nonfirst_round_evaluation_satisfied() {
        let air = StoreByteChipPolyAir;
    ...
    fn random_store_byte_trace(log_n: usize, _seed: u64) -> RowMajorMatrix<F> {
        ...
        let last_row_start = (base_height - 1) * NUM_STORE_BYTE_COLUMNS;
        let last_row = &base.values[last_row_start..last_row_start + NUM_STORE_BYTE_COLUMNS];
        let mut values = Vec::with_capacity(target_height * NUM_STORE_BYTE_COLUMNS);
    ...
    fn perf_multi_round_sumcheck() {
        ...
        let main = random_store_byte_trace(log_n, seed);
        let height = main.height();
        ...
        let constraint_reducer = reducer();
        let global = EF::zero();
        let reserved_poly_desc = <StoreByteChipPolyAir as FullAir<
            PrecomputeRowBuilder<'_, F, F, EF>,
        >>::reserved_poly(&air);
    ...
    fn bitvec_mult_matches_send_count() {
        ...
        assert!(!shard.store_byte_events.is_empty(), "fixture must yield store_byte events");

        let mut deps = ExecutionRecord::default();
        <StoreByteChipPolyAir as MachineAir<F>>::generate_dependencies(
            &StoreByteChipPolyAir,
            &shard,
            &mut deps,
        );
        ...
        let expected = shard.store_byte_events.len();
        assert!(expected > 0, "test fixture must include store_byte events");
        assert_eq!(
