...
pub const NUM_STORE_BYTE_COLUMNS: usize = size_of::<StoreByteCols<u8>>();

#[derive(AlignedBorrow, Default, Debug, Clone, Copy)]
#[repr(C)]
pub struct StoreByteCols<T> {
...
impl<F: Field> MachineAir<F> for StoreByteChip {
    ...
    fn generate_trace(
        ...
        let nb_rows = input.store_byte_events.len();
        ...
        let mut values = zeroed_f_vec(nb_rows * NUM_STORE_BYTE_COLUMNS);
        ...
        let blu_events = values
            ...
            .map(|(i, rows)| {
                ...
                rows.chunks_mut(NUM_STORE_BYTE_COLUMNS).enumerate().for_each(|(j, row)| {
                    ...
                    let (record, event) = &input.store_byte_events[idx];
        ...
        let main = RowMajorMatrix::new(values, NUM_STORE_BYTE_COLUMNS);
        CompressedMatrix::new(
            main,
            PaddingRow::Zero { width: NUM_STORE_BYTE_COLUMNS },
            padded_nb_rows,
        )
    }
    ...
    AB::Var: Sized,
{
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        let opcode = AB::F::from_canonical_u8(Opcode::SB as u8);
        ...
        builder.when(local.is_real).assert_eq(stored_byte, a_word[0].into());
        for i in 0..WORD_SIZE {
            //if offset_bit != i, then mem_access.value[i] == mem_access.prev_value[i]
            builder.when(local.is_real - local.address_operation.offset_bit[i]).assert_eq(
                local.memory_access.value()[i].into(),
                local.memory_access.prev_value()[i].into(),
            );
        }
    }
}
