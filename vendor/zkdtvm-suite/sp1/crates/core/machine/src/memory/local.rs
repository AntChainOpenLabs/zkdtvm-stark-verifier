...
/// The columns for the MemoryLocal chip. Each row contains a single memory local entry.
pub type MemoryLocalCols<T> = SingleMemoryLocal<T>;
...
impl<F: Field> MachineAir<F> for MemoryLocalChip {
    ...
    fn num_rows(&self, input: &Self::Record) -> Option<usize> {
        ...
        let num_rows = padded_rows_threshold(next_power_of_two(nb_rows, size_log2));
        Some(num_rows)
    }
    ...
    fn generate_trace(
        ...
        let chunk_size = std::cmp::max(real_nb_rows / num_cpus::get(), 0) + 1;
        let mut values = zeroed_f_vec(real_nb_rows * NUM_MEMORY_LOCAL_INIT_COLS);

        let mut chunks =
            values.chunks_mut(chunk_size * NUM_MEMORY_LOCAL_INIT_COLS).collect::<Vec<_>>();
        ...
        let main = RowMajorMatrix::new(values, NUM_MEMORY_LOCAL_INIT_COLS);
        CompressedMatrix::new(
            main,
            PaddingRow::Zero { width: NUM_MEMORY_LOCAL_INIT_COLS },
            padded_nb_rows,
        )
    }
...
mod tests {
    ...
    use crate::{
        memory::MemoryLocalChip, programs::tests::*, riscv::RiscvAir, utils::setup_logger,
    };
    use dt_core_executor::{ExecutionRecord, Executor};
    use dt_stark::{
        air::{InteractionScope, MachineAir},
        baby_bear_poseidon2::BabyBearPoseidon2,
        debug_interactions_with_all_chips, DTCoreOpts, InteractionKind,
    };
    ...
    fn get_test_execution_record() -> ExecutionRecord {
        use dt_core_executor::events::{MemoryLocalEvent, MemoryRecord};
    ...
    fn test_generate_trace_ffi_eq_rust() {
        ...
        let trace: RowMajorMatrix<BabyBear> =
            chip.generate_trace(&record, &mut ExecutionRecord::default()).decompress();
    ...
    fn generate_trace_ffi(input: &ExecutionRecord, height: usize) -> RowMajorMatrix<BabyBear> {
        ...
        use crate::{
            memory::{MemoryLocalCols, NUM_MEMORY_LOCAL_INIT_COLS},
            utils::zeroed_f_vec,
        };
        ...
        chunks.par_iter_mut().enumerate().for_each(|(i, rows)| {
            rows.chunks_mut(NUM_MEMORY_LOCAL_INIT_COLS).enumerate().for_each(|(j, row)| {
                ...
                if idx < events.len() {
                    let cols: &mut MemoryLocalCols<F> = row.borrow_mut();
                    // SAFETY: FFI call to optimized C/assembly trace generation.
                    // cols points to a valid, mutable, properly-aligned MemoryLocalCols<F>.
                    unsafe {
                        crate::sys::memory_local_event_to_row_babybear(events[idx], cols);
                    }
                }
            });
        });
