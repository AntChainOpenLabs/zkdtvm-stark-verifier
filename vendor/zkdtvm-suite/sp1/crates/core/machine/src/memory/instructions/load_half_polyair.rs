...
use super::{
    load_half::{LoadHalfChip, LoadHalfCols, NUM_LOAD_HALF_COLUMNS},
...
// ============================================================================
// Main column offsets within `LoadHalfCols<u8>` (NUM_LOAD_HALF_COLUMNS = 61).
// Same layout as LoadByteCols except `selected_byte` is `selected_half: [T;2]`.
// ============================================================================
...
const COL_SELECTED_HALF: usize = 55;
const COL_MSB: usize = 57;
const COL_IS_LH: usize = 58;
const COL_IS_LHU: usize = 59;
const COL_IS_REAL: usize = 60;

// ============================================================================
// Reserved-poly slice layout (RES_NUM_COLS = 54).
// Excludes (precompute-only): cpu_state.pc, op_a, op_a_access.prev_value, op_b.
// ============================================================================

const RES_IS_REAL: usize = 0;
const RES_IS_LH: usize = 1;
const RES_IS_LHU: usize = 2;
...
const RES_SELECTED_HALF: usize = 12;
const RES_MSB: usize = 14;
const RES_OP_A_VALUE: usize = 15;
const RES_OP_A_ACCESS_TS: usize = 19;
const RES_OP_B_VALUE: usize = 24;
const RES_OP_B_ACCESS_TS: usize = 28;
const RES_OP_C_IMM: usize = 33;
const RES_ADDR_WORD: usize = 37;
const RES_OFFSET_BIT: usize = 41;
const RES_MEM_VALUE: usize = 45;
const RES_MEM_TS: usize = 49;
const RES_NUM_COLS: usize = 54;

#[derive(Default, Clone, Copy)]
pub struct LoadHalfChipPolyAir;

impl<AB: FullAirBuilder> FullAir<AB> for LoadHalfChipPolyAir
where
    AB::VarMaybeExt: Clone,
{
    ...
    fn reserved_poly(&self) -> Vec<PairCol> {
        ...
        cols.push(PairCol::Main(COL_IS_LH));
        cols.push(PairCol::Main(COL_IS_LHU));
        ...
        cols.push(PairCol::Main(COL_SELECTED_HALF));
        cols.push(PairCol::Main(COL_SELECTED_HALF + 1));
    ...
    fn precompute_lc(&self, builder: &mut AB) {
        ...
        let local: &LoadHalfCols<AB::VarMaybeExt> = unsafe { core::mem::transmute(main.as_ptr()) };
        ...
        let opcode = local.is_lh.clone() *
            AB::VarMaybeExt::from(AB::F::from_canonical_u8(Opcode::LH as u8)) +
            local.is_lhu.clone() *
                AB::VarMaybeExt::from(AB::F::from_canonical_u8(Opcode::LHU as u8));
        ...
        // #23: MSB lookup
        // =====================================================================
        msb_precompute_lc(builder, local.msb.clone(), local.selected_half[1].clone());

        // #24: BitVec [is_lh, is_lhu, offset_bit[0..3], most_sig_bytes_zero.result]
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        let is_lh = local[RES_IS_LH].clone();
        let is_lhu = local[RES_IS_LHU].clone();
        ...
        // All boolean constraints enforced by BitVec #24, except is_real
        ...
        builder.assert_zero(is_real.clone() - is_lh.clone() - is_lhu);
        ...
        // selected_half selection
        let selected_half_lo = local[RES_SELECTED_HALF].clone();
        let selected_half_hi = local[RES_SELECTED_HALF + 1].clone();
        let low_byte =
            offset[0].clone() * mem_word[0].clone() + offset[2].clone() * mem_word[2].clone();
        let high_byte =
            offset[0].clone() * mem_word[1].clone() + offset[2].clone() * mem_word[3].clone();
        builder.assert_zero(is_real.clone() - offset[0].clone() - offset[2].clone());
        builder.assert_zero(low_byte - selected_half_lo.clone());
        builder.assert_zero(high_byte - selected_half_hi.clone());

        // padding_byte + assert_word_eq
        let perform = is_real - local[RES_OP_A_ZERO].clone();
        let padding =
            local[RES_MSB].clone() * is_lh * AB::VarMaybeExt::from(AB::F::from_canonical_u8(0xff));
        builder.when(perform.clone()).assert_zero(a_word[0].clone() - selected_half_lo);
        builder.when(perform.clone()).assert_zero(a_word[1].clone() - selected_half_hi);
...
// =============================================================================
// MachineAir implementation (delegation to LoadHalfChip)
// =============================================================================

impl<F: Field> BaseAir<F> for LoadHalfChipPolyAir {
    fn width(&self) -> usize {
        NUM_LOAD_HALF_COLUMNS
    }
}

impl<F: Field> MachineAir<F> for LoadHalfChipPolyAir {
    type Record = ExecutionRecord;
    type Program = Program;

    fn name(&self) -> String {
        "LoadHalfPolyAir".to_string()
    }
    ...
    fn generate_trace(
        ...
        LoadHalfChip.generate_trace(input, output)
    }
        ...
        let blu_batches = input
            ...
            .map(|events| {
                ...
                for (record, event) in events {
                    let mut row = [F::zero(); NUM_LOAD_HALF_COLUMNS];
                    let cols: &mut LoadHalfCols<F> = row.as_mut_slice().borrow_mut();
                    LoadHalfChip.event_to_row(record, event, cols, &mut blu, shard);

                    // PolyAir-only: emit BitVec for each real row.
                    // Payload: [is_lh, is_lhu, offset_bit[0..3], most_sig_bytes_zero.result]
                    ...
                    let is_lh = (event.opcode == Opcode::LH) as u16;
                    let is_lhu = (event.opcode == Opcode::LHU) as u16;
                    let value: u16 = is_lh |
                        (is_lhu << 1) |
    ...
    fn included(&self, shard: &Self::Record) -> bool {
        <LoadHalfChip as MachineAir<F>>::included(&LoadHalfChip, shard)
    }
}

#[cfg(test)]
mod tests {
    use super::{super::load_half::LoadHalfChip, *};
    ...
    fn beta_powers() -> Vec<EF> {
        ...
        let n = <LoadHalfChipPolyAir as FullAir<
            PrecomputeRowBuilder<'_, F, F, EF>,
        >>::required_max_beta_power(&LoadHalfChipPolyAir);
    ...
    fn reducer() -> Vec<EF> {
        // Gate constraints: 41 (40 original + 1 is_real boolean gate)
    ...
    fn reserved_poly_matrix(
        air: &LoadHalfChipPolyAir,
        main: &RowMajorMatrix<F>,
    ) -> RowMajorMatrix<EF> {
        let reserved_poly =
            <LoadHalfChipPolyAir as FullAir<PrecomputeRowBuilder<'_, F, F, EF>>>::reserved_poly(
                air,
            );
    ...
    #[test]
    fn test_load_half_first_and_nonfirst_round_evaluation_satisfied() {
        let air = LoadHalfChipPolyAir;
    ...
    fn random_load_half_trace(log_n: usize, _seed: u64) -> RowMajorMatrix<F> {
        ...
        let last_row_start = (base_height - 1) * NUM_LOAD_HALF_COLUMNS;
        let last_row = &base.values[last_row_start..last_row_start + NUM_LOAD_HALF_COLUMNS];
        let mut values = Vec::with_capacity(target_height * NUM_LOAD_HALF_COLUMNS);
    ...
    fn perf_multi_round_sumcheck() {
        ...
        let main = random_load_half_trace(log_n, seed);
        let height = main.height();
        ...
        let constraint_reducer = reducer();
        let global = EF::zero();
        let reserved_poly_desc = <LoadHalfChipPolyAir as FullAir<
            PrecomputeRowBuilder<'_, F, F, EF>,
        >>::reserved_poly(&air);
    ...
    fn bitvec_mult_matches_send_count() {
        ...
        assert!(!shard.load_half_events.is_empty(), "fixture must yield load_half events");

        let mut deps = ExecutionRecord::default();
        <LoadHalfChipPolyAir as MachineAir<F>>::generate_dependencies(
            &LoadHalfChipPolyAir,
            &shard,
            &mut deps,
        );
        ...
        let expected = shard.load_half_events.len();
        assert!(expected > 0, "test fixture must include load_half events");
        assert_eq!(
