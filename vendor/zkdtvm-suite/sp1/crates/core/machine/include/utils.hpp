...
__DT_HOSTDEV__ __DT_INLINE__ array_t<uint8_t, 4> u32_to_le_bytes(uint32_t n) {
    return {
        (uint8_t)(n >> 8 * 0),
        (uint8_t)(n >> 8 * 1),
        (uint8_t)(n >> 8 * 2),
...
__DT_HOSTDEV__ __DT_INLINE__ array_t<uint8_t, 8> u64_to_le_bytes(uint64_t n) {
    return {
        (uint8_t)(n >> 8 * 0),
        (uint8_t)(n >> 8 * 1),
        (uint8_t)(n >> 8 * 2),
...
__DT_HOSTDEV__ __DT_INLINE__ std::tuple<uint8_t, uint8_t>
...
template<class F>
__DT_HOSTDEV__ __DT_INLINE__ void
...
template<class F>
__DT_HOSTDEV__ __DT_INLINE__ uint32_t
...
template<class F>
__DT_HOSTDEV__ __DT_INLINE__ void word_from_le_bytes(
...
namespace opcode_utils {
    ...
    __DT_HOSTDEV__ __DT_INLINE__ bool is_branch(Opcode opcode) {
        switch (opcode) {
            case Opcode::BEQ:
            case Opcode::BNE:
            case Opcode::BLT:
    ...
    __DT_HOSTDEV__ __DT_INLINE__ bool is_jump(Opcode opcode) {
        switch (opcode) {
            case Opcode::JAL:
            case Opcode::JALR:
                return true;
